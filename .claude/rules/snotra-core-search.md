...
- **search_with_options の分割構造（#436）**: 候補準備 `prepare_query_plan`（→ `QueryPlan`）／incremental 可否 `decide_incremental`（`prev_*` は read のみ）／1 エントリ採点 `score_one_entry`（`#[inline]`・bitmask pre-filter を先頭固定）／結果組立 `heap_into_results`。`prev_*` の write は orchestrator が fold 後・sort 前に行う
