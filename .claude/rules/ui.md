...
- **await 後の状態復元は staleness チェック必須**: `latestRun` primitive の `isStale()`（world 世代カウンタを内包）で「await 中に状態が変わっていないか」を検証してから復元する。lane タスクは `searchLane.run()` の ctx から `isStale`/`requestId` を受け取り、非 lane コード（モード遷移・起動）は `searchLane.invalidate()` で world を進めて in-flight を supersede する
