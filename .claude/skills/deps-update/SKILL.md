...
- **Rust の toolchain も見る**（`cargo` または空のとき・#1161）。`rust-toolchain.toml` が版を固定して
  ...
  - **`channel` を上げたら `rustup component add rust-analyzer` を打つ。** 自動取得されるのは
    `components` に挙がったものだけであり、rust-analyzer はそこに無い（足すと CI も毎 run 取りに
    行くため、意図して外してある）。`.claude/lsp/` の LSP プラグインは PATH の shim 越しにこれを
    起動するので、**入っていないと findReferences が使えず、しかもどの検査も赤くならない**
    （2026-08-24 に新しい toolchain が実体化して以降、11 時間気づかれなかった・実測）。
    セッションが既に LSP の起動失敗を数え切っていると、component を入れてもそのセッションでは
    復帰しない。**Claude Code を再起動すれば戻る**（2026-08-25 に両方向とも実測）。
    再起動の直後は索引が冷えており、最初の 1 回は「見つからない」を返しうる——
    クラッシュとは別物なので、温まるまで待って測り直す
