...
各サブディレクトリの `CLAUDE.md`「モジュール構成」が、実際の **production ファイル**一覧と一致しているか検証する。

**目的（この検査が保証するもの）**: production モジュールの**追加・削除と責務索引の同期**——「モジュール構成」表が production ファイルの網羅的な索引であり続けることを保証する。**テストの網羅性検査ではない**（テストは production モジュールの検証手段であって責務単位ではない）。
...
**テストファイルを個別列挙の対象外にする理由**: テストファイル（`*.test.ts` / `*.test.tsx`）は対応する production モジュールの**検証手段**であって、モジュール構成表の**責務単位ではない**。テストの構成・方針は `ui/CLAUDE.md`「テスト基盤」節が方針として受け持つ（個別ファイルは列挙しない）。この除外は**ファイル種別（`*.test.*`）に基づく規則**であり、現存する特定ファイルのハードコードではない——テストの増減に自動追従する。除外パターンは「何がテストか」の SSOT（`vitest.config.ts` の `ui/src/**/*.test.{ts,tsx}`）と一致させる。Rust 各 crate はインライン `#[cfg(test)] mod tests` 規約で独立テストファイルを持たないため、この除外が実質効くのは UI のみ。

**テスト専用の helper / fixture の扱い**: production から import されないテスト専用モジュール（`__tests__/` 配下・`*.fixture.ts` 等）を module として管理したくなった場合は、production の「モジュール構成」表ではなく `ui/CLAUDE.md`「テスト基盤」節に責務を記す（現状そのようなファイルは存在しない・種別ベースの規則で判断する）。

手順:
1. 各 `CLAUDE.md`「モジュール構成」を読み、記載されたファイル名を抽出する
2. 実際のファイルを列挙し、**テストファイル（`*.test.{ts,tsx}`）を母集団から除外**する（除外後の production ファイルのみを照合する）。列挙は `Glob` ツールで行う——`git ls-files` の pathspec `**` は `ui/src` 直下のファイル（`main.tsx` 等）を取りこぼすため、件数が load-bearing な本検査では使わない
3. 差分を報告:
   - **記載あり・実ファイルなし**: 削除されたのに CLAUDE.md が未更新
   - **実ファイルあり・記載なし**: 追加されたのに CLAUDE.md が未更新
4. **証拠を添える**（`AGENTS.md`「派生コピー同士の一致を完全性の証拠にしない」・Check 7 と同じ証跡の規律）: 除外したファイル種別（`*.test.{ts,tsx}`）と、除外を行う **UI の照合母集団**（production 件数・除外したテスト件数）を、最終報告の**サマリーに「根拠」として**残す（下のサマリー雛形の根拠行）。**「発見事項なし」でも件数を添える**（証跡の無い「差分ゼロ」は、照合していないのと区別が付かない）。除外の無い Rust 対象は件数の明示を要さない

## Check 2 — docs/architecture.md にファイル単位モジュール表が再導入されていないか

`docs/architecture.md` はファイル単位のモジュール一覧を**持たない**設計。責務宣言の正準は各ファイルの `//!` / TSDoc で、各サブディレクトリの `CLAUDE.md` がファイル名の索引と横断不変条件を持ち（#562）、architecture.md は位置づけ・主要な型・横断パターン・データフローのみを記す。二重管理がドリフトの温床になるため、ファイル単位のモジュール表は撤去された経緯がある。
