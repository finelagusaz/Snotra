...
⚠ **「連続」はサイクルで数える。表の行数で数えてはならない。** 観測の無いサイクル末が挟まっていれば連続は切れており、行が 2 つ並んでいても連続 2 回ではない。**数えるときは行の日付とサイクル境界（`RETROSPECTIVE.md` の更新）を突き合わせる**——2026-08-25 に実際に踏んだ: 記録は 2 行ともゼロだったが、あいだの 2 サイクルに観測が無く、連続は成立していなかった。行数だけを見ると撤去の根拠が一段甘くなる。
