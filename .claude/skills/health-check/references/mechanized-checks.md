...
| Check 3 — `AGENTS.md` ドキュメント参照の実在性 | G-references（#587） | 対象はガバナンス文書群全体に一般化された |
| Check 4 — `SPEC.md` セクション番号の連続性 | G-spec-sections（#587） | 番号連続性に加え、リポジトリ内の `SPEC §N.x` 参照の実在も検査対象 |
| Check 6 — `docs/development-principles.md` 参照の実在性 | G-references（#587） | Check 3 と同様に一般化された |
