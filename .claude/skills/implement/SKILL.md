...
`docs/build-commands.md` の「変更後の検証チェックリスト」を SSOT として、該当するカテゴリ A〜F をすべて実行する。失敗した場合、修正して失敗したステップから再実行する。**該当の判定は拡張子ではなく、変更が触れるコードパスの意味で行う**（`.claude/rules/src-tauri.md`・#558）——カテゴリ C・D はファイルの種類ではなく変更が何をするかで決まるため、拡張子で写像すると一度も到達しない。**カテゴリ D はエージェントが実行できない**（対話入力・目視を要する）——該当する変更では実行を試みず、目視項目を PR 本文へ引き渡して人間に委ねる。
