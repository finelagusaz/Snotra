...
`npm run race:boundaries` を走らせる（**コマンド本体と `--base` / `--include-removed` の指定は `docs/build-commands.md`「Windows/macOS/Linux で実行可能」が SSOT**——二重メンテを避けるためここには書かない）。

**このツールが母集団を決める**（種別 ①〜⑧ の定義・検出パターン・0 件の明示を含む出力仕様は `scripts/race-boundaries.mjs` が SSOT）。
