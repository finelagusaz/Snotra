...
- **計画レビュー**（`workspace/plan.md` 等）: 対象は**計画の記述そのもの**である。計画が述べる設計上の操作（spawn する・送る・listen する・共有状態を足す）をテキストから直接同定する。**実装前のコードへの grep は必ず 0 件になる**——それを根拠に「非該当」と報告してはならない
...
| ④ | managed state の読み書き（**ロック取得順序・保持区間は本スキルの対象外**——`src-tauri/CLAUDE.md` のロック最小化パターンが正本） | `try_state`, `Mutex`, `Atomic`, `\.lock\(` |
...
- **①〜⑦の全種について grep を実行し、0 件であることをコマンドとともに示す**。①②③だけ見て「spawn も channel も無い」と書くのは非該当判定ではない
