...
- **対象は $ARGUMENTS が指す差分の全ファイル・全 hunk** である。目についた 1 ファイルを「対象」と自己申告しない
- **変更したシンボルの呼び出し元も grep する**。純粋核（`spawn`/`channel`/`listen`/`Mutex` を 1 つも含まないファイル）であっても、その関数が drain ループの staleness 判定に使われているなら**それは境界の要石であり非該当ではない**。定義ファイルの grep が空振りすることと、並行性に無関係であることは別である

3 つすべてを満たしたときだけ「並行境界なし（非該当）」と報告する。
...
**grep 結果を根拠にするときは総ヒット件数を明記し、全件について個別に判定する。** 1 件だけ貼って残りを黙って無視するのは、形式を満たした未確認である。
