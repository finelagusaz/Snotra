...
開発サイクルの振り返りを行う。質問せず自律的に進める——例外はスキル自体の更新で、その採否だけはユーザーに諮る（Step 2）。
...
2. **内容の種別で器が決まるものを先に振り分ける（第二軸）**: **手順**（ステップ列として実行できる）→ スキル（規範文書に手順本文を書かない）。**否定の知識**（なぜ B を却下したか）→ `docs/adr/ADR-<slug>.md`。**失敗の一次証跡**（何が起きたか）→ GitHub issue（`RETROSPECTIVE.md` は揮発でよい）。**意図（仕様）** → `SPEC.md`
3. **特定ファイル/パスを触ると効くか** → `.claude/rules/<X>.md`（対象を読むと自動配送される）。既存 rule の `paths:` に該当するなら本文へ追記、しなければ新設（新設時はカナリア要否を判断）
4. **特定の局面・作業で効くか** → 該当スキル（`/symmetric-check`・`/plan-review`・`/state-check`・`/persistence-check`・`/dry-check`・`/race-check` 等）の本文へ。`AGENTS.md`「条件別チェック」表には「トリガー → 参照先」の 1 行だけ足す（本文の SSOT は skill 側）
5. **汎用の設計・デバッグ原則か** → `docs/development-principles.md`
6. **モジュール固有** → 各サブディレクトリ `CLAUDE.md`（`src-tauri/`・`snotra-core/`・`snotra-settings/`・`snotra-egui-runtime/`）／**Claude Code 固有**（コミュニケーション原則・スキル表・委譲/worktree 運用） → ルート `CLAUDE.md`
7. **どのトリガーにも紐付かない、書く・判断するたびに効く横断原則だけ** → `AGENTS.md`「検証の作法」コア。**ここだけが常時ロードに値する**
...
- **サイクル/PR を越えて生き残るタスク**（follow-up・別リファクタ・繰越・要設計）→ **GitHub issue を起票**する（`gh issue create`）。迷ったら永続側＝issue を選ぶ
  - **起票の前に、同じ「はい」で動き出すものを 1 件へ束ねる**——判定は関連の強さではなく「人間が 1 度承認すれば束ねた全部が着手可能になるか」である（#1112 と #1113 は 1 つの是非で code と規範の双方が動き PR #1117 一本で閉じた。同じサイクルの #1114 は別の是非ゆえ別 PR になった）。**束ねたら、その 1 つの是非を issue のタイトルに置き、所見は個別の見出しとして本文へ残す**——タイトルが違えば人間はそこで分割でき、本文に残っていれば片方が黙って落ちない。**迷ったら分ける**（重複した判断は人間の目に見えるが、束ね損なった所見は沈黙する）
...
- **各節（`###` 配下）には「次のサイクルで効く一点」だけを書く**——`RETROSPECTIVE.md` は次サイクルで上書きされるので、読んだ量の証明や経緯の再話は要らない。**ただし節を短く保つことは、Step 3 の教訓配置を省く理由にはならない**（上書きで消えるのはここの記述であって、トリガーに括り付けた教訓ではない。節を短くするために教訓の抽出そのものを 1 件へ絞ると、`AGENTS.md`「RETROSPECTIVE.md の運用」の不変条件が狙った失敗様態に戻る）
- 前回の内容は上書きする（追記しない）
- 実行タスク・未完了項目は書かない（Step 4 で issue / PR へ振り分け済み）
- **他文書の見出しを引くときは正準形 `` `<対象>`「<見出し>」 `` で書く**（`.claude/rules/governance-docs.md`）。`RETROSPECTIVE.md` は G-heading-refs の母集団に入っており、上書きのたびに参照の形が決め直される——ここで散文形に戻すと、着地の照合が毎サイクル失われる
