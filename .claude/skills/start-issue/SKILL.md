...
**ただしこの免除は、plan-review が「独立レビュー不成立」と報告したエントリには及ばない**——そのレイヤーの ①〜⑤ は検証されていない。5b で自ら確認するか `/plan-review` を再実行し、**不成立が残るならその事実と代替検証を `workspace/plan.md` 末尾の「セルフレビュー」セクションに記す**（本ファイル末尾の引き渡し契約「レビュー済みの計画」が偽になるのを防ぐ）。
