...
- **沈黙が「合格」なのは `selectChecks` に検査が割り当てられたファイルだけである**（#497・機構ではなく規範ゆえ前提を忘れれば false green が再発する）。`*.md` 全般・`SPEC.md`・`scripts/` 配下の非 TS ファイル・`.github/workflows/`・`Cargo.lock` の沈黙は「何も走らなかった」である（`scripts/*.ts` は「include 対象外」の一行が出るため沈黙しない）。**検査ではない reminder が別経路で出るが（`.md` の依存参照・#1140、編集に帰属する索引と参照実在の不整合・#1139。一覧は `docs/hooks.md`「検査ではない reminder」が正本）、その不在も「問題が無い」を意味しない**——依存参照は純追記でもフェンスに分断された節でも出ず、索引と参照実在は削除・`governanceDocs` 外の `.md`・`mod` 宣言を見ない（射程の穴は `docs/hooks.md` が名指しする）。決定的な項目（参照実在・索引・スキル表・SPEC 番号・rules glob・コマンド写像）は PR CI の `governance-check` job（`skip-ci` 非対象・#587）が事後に捕捉し、その検査対象外（責務の妥当性等の意味的整合）は**受容する残余**である
...
- **委譲はコンテキストを継承しない** — メインエージェントの system prompt にしか無い事実（メモリ領域の絶対パス等）はプロンプトへ明示的に渡す——渡し忘れると、見えないものを「無いもの」として報告される
  - **渡せないものもある: issue 添付の画像は `gh issue view` に現れず、サブエージェントは読めない**——一次証拠に画像が含まれる論点は委譲で決着しない（#836 で独立再導出 3 体のうち 1 体が散文だけを見て逆の結論を出し、スクリーンショットを見たメイン側が裁定した）
  - **`allowed-tools` はインライン実行のスキルを拘束しない**（実測）——frontmatter に `Agent` が無いことを根拠に「このスキルは委譲しない」と推論してはならない
...
- **レビューの委譲では、⚠️（確信の持てない所見）を返り値の列挙に含めさせ、前段の裁定は結論ではなく争点の形で渡す** — 返り値仕様（✅/❌・件数のみ）から外れた所見はファイルにしか残らず会話に載らない（#755/#801 の挙動後退で 0/2）。「裁定済み・理由つき」で受け取った下流は再導出せず逐語追認する
  - **前向きの適合検査だけでなく、「差分が消した行の不変条件を名指しし、再確立地点を探す」逆向きの監査を 1 枠置く**——当該後退を捕まえた唯一の枠組みである（2026-08-04 実測）
  - **所見が正しくても、そこに添えられた機序の説明は独立に誤りうる**——採るのは所見であって説明ではない。#1056 では、レビュアの「検知器が届いていない」は正しかったが機序の一般化（「両側に同じ誤りが在るから A/B では捕まらない」）は誤りで、実際は規模が理由だった（変異は片側の経路にしかない）。**規範文書へ書く前に機序まで一次証拠で裁定する**——写せば、正しい指摘の隣に未測定の主張が着地する
  - **枠組みだけでなく道具も分ける**——#745 では `git log -S` / `blame` を逆向きの監査 1 枠にだけ渡し、それが**主エージェントの誤認（根本原因は 2 変数分解ではなく実装漏れ）を訂正した唯一の経路**になった。他 3 枠はコードの現在形しか見ず原理的に到達できない

## コミュニケーション原則

- **実行にバイアスをかける** — 具体的な計画や修正指示が既にあるなら、事前の全体探索もプランモードも挟まず最初の Edit/Write から着手する。コミット・PR 作成の指示は確認なしに即実行する（ただし「`main` へ直接コミット・プッシュしない」）。不明点があれば焦点を絞った 1 つの質問をしてから実装に移る
