...
- **レビューの委譲では、⚠️（確信の持てない所見）を返り値の列挙に含めさせ、前段の裁定は結論ではなく争点の形で渡す** — 返り値仕様（✅/❌・件数のみ）から外れた所見はファイルにしか残らず会話に載らない（#755/#801 の挙動後退で 0/2）。「裁定済み・理由つき」で受け取った下流は再導出せず逐語追認する。観点には前向きの適合検査だけでなく**「差分が消した行の不変条件を名指しし、再確立地点を探す」逆向きの監査を 1 枠**置く——当該後退を捕まえた唯一の枠組みである（2026-08-04 実測）
