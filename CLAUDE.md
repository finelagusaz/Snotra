...
- 比較関数とデータ構造を組み合わせる実装（例: `Ord` + `Reverse` + `BinaryHeap`）では、実装前に「先頭要素が最良/最悪のどちらを表すか」を一文で明示する
- パフォーマンス最適化では、実装前に「意味を変えない不変条件」を箇条書きで定義し、性能改善と意味変更を分離して扱う
- 挙動変更がない前提の最適化では、変更前の代表入力/出力をベースライン化し、変更後との差分検証を必須とする
- 境界条件（例: `k=0/1`、同点順位、走査順、空白種別）を実装前に列挙し、最低1件ずつ検証ケースを用意する
...
- 修正報告時は、追加/更新したテスト名と「どの不変条件を検証したか」をセットで記載する
...
- `snotra-core/src/search.rs` の順位ロジックを変更する場合は、入力順を変えても `top-k` 結果が不変であることを検証する
- `snotra-core/src/search.rs` の比較/選別ロジックを変更する場合は、`BinaryHeap` 先頭要素の意味（最良/最悪）がテストで固定されていることを確認する
- `snotra-core/src/query.rs` の正規化を変更する場合は、タブ・全角スペース・NBSP を含む入力が `' '` に統一されることを検証する
- `snotra-core/src/query.rs` の正規化を変更する場合は、同じ関数を2回適用しても結果が変わらないこと（冪等性）を検証する
