...
- 検索結果ウィンドウの同期は3イベントで扱う: `results-data-changed`（結果配列変更）、`results-selection-changed`（選択のみ変更、配列不要）、`results-visibility-changed`（非表示、配列不要）。選択変更時の不要な配列シリアライズを避けるための分割設計
