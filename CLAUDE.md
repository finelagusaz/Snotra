...
- **`selectChecks` に載っていないファイルの沈黙は「何も走らなかった」であり、合格ではない**（#497）。`*.md` 全般・`SPEC.md`・`scripts/`・`.github/workflows/`・`Cargo.lock`・`docs/build-commands.md` 自身がこれに当たる。**エージェントはこの 2 種類の沈黙を区別できない** — 定期検知（`/health-check` の Check 1/3/5/6/8/9/10）が補うが、編集時の即時性は無い。これは**受容する残余**である
