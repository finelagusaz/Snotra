...
- **事後の捕捉は PR CI の `governance-check` job が担う**（`skip-ci` 非対象・#587）——決定的な項目（参照実在・索引・スキル表・SPEC 番号・rules glob・コマンド写像）はそこで捕捉され、その検査対象外（責務の妥当性等の意味的整合）は**受容する残余**である
...
- **記録への信頼で動く——気づいたその場で記録へ書く** — 記憶ではなく AGENTS.md・CLAUDE.md・スキル・RETROSPECTIVE.md への記録がチームの連続性を作る
...
- **委譲はコンテキストを継承しない——メインエージェントの system prompt にしか無い事実（メモリ領域の絶対パス等）はプロンプトへ明示的に渡す** — 渡し忘れると、見えないものを「無いもの」として報告される
  ...
  - **frontmatter に `Agent` が無いことを根拠に「このスキルは委譲しない」と推論しない** — `allowed-tools` はインライン実行のスキルを拘束しない（実測）
...
- **レビューの委譲では、⚠️（確信の持てない所見）を返り値の列挙に含めさせ、前段の裁定は結論ではなく争点の形で渡す** — 返り値仕様（✅/❌・件数のみ）から外れた所見はファイルにしか残らず会話に載らない（#755/#801 の挙動後退で 0/2）。「裁定済み・理由つき」で受け取った下流は再導出せず逐語追認する
  ...
  - **所見が正しくても、そこに添えられた機序の説明は独立に誤りうる——規範文書へ書く前に機序まで一次証拠で裁定する**——採るのは所見であって説明ではない。#1056 では、レビュアの「検知器が届いていない」は正しかったが機序の一般化（「両側に同じ誤りが在るから A/B では捕まらない」）は誤りで、実際は規模が理由だった（変異は片側の経路にしかない）。写せば、正しい指摘の隣に未測定の主張が着地する
  - **所見も機序も正しいのに、採るべき対処が「直す」ではないことがある**——**直した版を実データへ当ててから決める**。#992 で「述語がこの形の折れを見ない」は所見も機序も正しかったが、射程を広げた版を当てると**実ツリーの 7 行すべてが誤検出**になった（正しい対処は死角として宣言することだった）。**指摘の正しさは、修正の正しさを含意しない。**
