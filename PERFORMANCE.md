...
3. 計算量を下げる（中〜大規模データ向け）
   ...
   - **重い判定を lock の外へ出すときは、述語の向きが挙動を決める。** 判定は lock を離した時点の snapshot に基づくので、**「残す集合」で書くと窓の間に増えた要素が落ち、「落とす集合」で書くと残る**。同じ削減でも前者は挙動変更・後者は挙動維持になり、**この違いを知らないと「lock の外へ出す案」ごと否定してしまう**（反復 12 で実際にそうなっていた）。実例は「採用: アイコン剪定の判定を lock の外へ出す」
4. 描画のマイクロ最適化を行う（仕上げ）
   - 文字幅計測や省略文字列生成のキャッシュ
   - 無意味な再スクロール/再レイアウト抑制
...
|---|---|---:|---|
| アイコン剪定の照合を二分探索へ（`absent_paths`） | 判定は **1,000 対 312,625 の非対称**（キャッシュは `icon_cache_cap` で頭打ち）。`sorted_by_path` は**既定 config では真**（`include_path_env` の既定が `false`・旗を下ろすのは `extend_with_roots` だけ・どちらも実測） | `path_into` が 312,625 → **約 19,000 回**（K·log₂N）、**312,625 回の SipHash 引きが消える**。**すべて未実測** | **PATH 有効時は旗が偽**（`extend_with_roots` が下ろす・剪定はその後）なので、現行の全件走査を fallback に残すことになる。**E 形（原文の篩 + FxHash・-20 ms）が要求した「`IndexTree` へ末尾成分の API を増やす」は要らない**——`absent_paths` は `index_tree.rs` の中に居るので `sorted_by_path` と `path_into` をそのまま読める。**採る理由が立つのは背景再構築の壁時計を削りにいく局面だけ**（判定は lock の外に在るので、それ以外では E を却下したのと同じ理由が当たる） |
...
### 採用: アイコン剪定の判定を lock の外へ出す（lock 保持 36 ms → snapshot と除去の一瞬・反復 12）

`drain_index` は索引 312,625 件のフルパスを組み直しながら `IconCacheState` の lock を握り
続けていた。**測るべきは CPU ではなく lock の保持時間である**——その間、表示中のアイコン
取得（`commands::icon::load_icon_pngs` の Step 1/3）は待つ。

**この節の直前まで、候補表はこの変種を明示的に否定していた**（「stale な集合で新規挿入を
捨てうる＝挙動変更」）。**否定が成り立つのは述語を「残す集合」で書いた場合だけである**:

| 述語の向き | 窓の間に挿入されたキー |
|---|---|
| 残す集合（当時の retain_paths が取っていた形） | 残す集合に**居ない**ので落ちる |
| 落とす集合（`IconCache::remove_paths`） | 落とす集合にも**居ない**ので残る |

**保証と非保証（旧実装との異同を含む）の正本は `IconCache::remove_paths` の doc とする。**
述語の向きを書き留めた `concurrent_insert_during_prune_window_survives` の射程も、そのテストの
doc が持つ。

**買ったのは lock 保持であって確保回数ではない。** 走査の中の `String` 確保は消えたが、
`IconCache::keys` がキャッシュの全キー（高々 cap）を clone する側へ移り、ヒット数 ≤
キャッシュ件数ゆえ**回数はむしろ増える**。キーが 0 件なら全件走査ごと、`dead` が空なら
2 回目の lock ごと省く。

**受容する残余は `IconCache::remove_paths` の doc が正本とする**（件数も内訳もそちらを読む
こと——ここへ数え上げを写すと、残余が増えた日にこの行だけが偽になる）。**lock の構造そのもの
を守る検知器が無いこと**もそこに含めてある。
...
——判定はここが正本とする。**再び測る値打ちが出るのは 2 通りある**: 剪定が lock の中へ
戻るとき（保持時間が再び効く）と、背景再構築の壁時計そのものを削りにいくとき（同じ候補表の
`reject_existing` の rayon 並列化と同じ動機。E 形の -20 ms はそのとき再評価の対象になる）。
...
**E（-20 ms）は在るが採らない。** 判定が lock の外へ出た今、それは背景スレッドの CPU で
しかなく、`IndexTree` へ原文の末尾成分を取る API 面を増やす額に見合わない。
