...
### 採用: PATH スキャンの問いを反転（186 → 54 ms・反復 9）
...
| 指標 | before | after |
|---|---:|---:|
| **PATH スキャン（壁時計）** | 186 ms（186/193/197） | **58 ms**（58/59/64・-128・-69%） |
| **PATH スキャン（確保回数）** | 314,395 | **2,066**（**-99.3%**） |
| 区間の peak | 48.10 MiB | **0.03 MiB**（-48.07） |
| 返り値 | +101 件 | +101 件（**不変**） |
| 索引の常駐 / ブロック | 35.44 MiB / 411,413 | 35.44 MiB / 411,413（**不変**） |

**3 区間の和を削減幅として書かない。** 消えるのは PATH 区間の 128 ms だけで、ロードと構築は
この変更の対象外である（実測はぶれの範囲に収まる——ロード 120〜167 ms・構築 54〜69 ms）。
和を取ると対象外のぶれが削減幅に混ざる。

**受け入れは壁時計ではなく確保回数で判定した。** 壁時計は同じコードで 54〜64 ms とばらつく
（±17%）が、確保回数は 6 回測って 6 回とも 2,066 だった。
...
計画段階では索引側へ問う形を想定していたが、**`scan_path_env` は `Engine::new_from_cache`
より前に走る**（`main.rs:196` 対 `:217`）——その時点で `PathStore` はまだ存在しない。
問う形にするには PATH マージを engine 構築の後ろへ動かす必要があり、`extend_cached_masks`
も一緒に動く。反転は `scan_path_dirs` の内部で閉じるので、そちらを採った。

#### 残余: 約 49 ms（天井）

after の 58 ms のうち列挙が 9 ms なので、残りは 312,691 件のファイル名を小文字化して
ハッシュを引く**純粋な計算**である。確保は 2,066 まで落ちきっており、ここから先は篩を
もっと粗くする（ファイル名長のビットマスク等）しかない。**取れる額の上限が約 49 ms** で
あることを測ったうえで、複雑さに見合わないと判断して見送った。

**「候補の表現が複数あるなら全部測る」を省いたのではない。** 反復 8 では 2 案を実装して
比べたが、ここでは**天井そのものを測った**——列挙 9 ms は表現を変えても消えないので、
どの代替表現も 49 ms 以上は取れない。個別に実装して比べる必要が無いのはそのためである。
