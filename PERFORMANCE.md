...
**「改善」ではなく「同一」である。** 挿入側は抽出を lock の外で行い挿入で lock を取り直すので、
lock を握り続けていた頃も剪定中の挿入は retain の**後**に着地していた。**これは実装を読んで
導いた主張であって、測ってはいない。**

検知器は `icon.rs` の `concurrent_insert_during_prune_window_survives` である。**測るのは
述語の向きであって、スレッドと mutex の等価性ではない**（テストは単一スレッドで lock を
取らない）。否定した変種そのもの——呼び出し側を `retain(|k, _| alive.contains(k))` へ
差し替える形——を注入すると、狙った断言「窓の間に挿入されたキーは落ちてはならない」で
落ちることを実測した。**この変異は集合の一致だけを見るテストを素通りする**（`dead` の
扱いを変えないため）。**lock の構造そのものを守る検知器は無い**——2 回の lock 取得を
1 回へ戻す退行は全テスト緑のまま通る（受容する残余）。

副産物: 述語の反転で「生きた集合を積む」形が要らなくなり、ヒットのたびの `String` 確保が
消えた。キーが 0 件なら全件走査ごと、`dead` が空なら 2 回目の lock ごと省く。

**受容する残余は `IconCache::remove_paths` の doc が正本とする**（剪定を生き延びたキーに
有界な片づけ経路が無いこと・窓の間の世代交代・終了時 flush との競合の 3 つ）。1 つ目は
**lock を握っていた頃からの性質である**——当時も lock 待ちした挿入は `retain` の後に
着地して判定を受けなかった。
