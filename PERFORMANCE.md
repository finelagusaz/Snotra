...
**旧実装との異同は、ここでは主張しない。** 3 巡のレビューで**軸ごとに違う答えになり、書く
たびに強すぎるか弱すぎるかへ振れた**（挿入の着地順・世代交代・FIFO 退避・終了時 flush）。
**保証と非保証の正本は `IconCache::remove_paths` の doc とする。**

検知器は `icon.rs` の `concurrent_insert_during_prune_window_survives` である。**測るのは
述語の向きであって、スレッドと mutex の等価性ではない**（射程と変異注入の手順はそのテストの
doc）。

副産物: 走査の中で `String` を確保しなくなった（述語の反転で「生きた集合を積む」形が要らなく
なった）。**ただし確保が消えたわけではない**——`IconCache::keys` がキャッシュの全キー
（高々 cap）を clone する側へ移り、ヒット数 ≤ キャッシュ件数ゆえ**回数はむしろ増える**。
買ったのは lock 保持であって確保回数ではない。キーが 0 件なら全件走査ごと、`dead` が空なら
2 回目の lock ごと省く。
