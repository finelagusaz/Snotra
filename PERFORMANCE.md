...
**この表の絶対値は日をまたいで再現しない**——`index_load` 8 ms・`tauri_init` 3 ms は 2026-08-10 の
同機・同 config で**どちらのバイナリでも出ない**（下の「#1023 を挟んだ同日 A/B」）。区間どうしの
比（どこが主役か）は保つが、**値の差を改修の効果と読むには同日 A/B が要る**。
...
- **絶対値の表は単独では比較材料にならない。** 改修の効果を語るなら同日 A/B を取る——
  上の 2026-08-09 の表は「その日の絶対値」の記録であって、今日との差は改修の効果ではない
