...
### 採用: PATH スキャンの問いを反転（186 → 約 58 ms・確保 -99.3%・反復 9）
...
#### A/B（同日・同セッション・各 3 回の最小値）

| 指標 | before | after |
|---|---:|---:|
| **PATH スキャン（壁時計）** | 186 ms（186/193/197） | **54〜64 ms**（9 標本・-約 130 ms・-約 69%） |
| **PATH スキャン（確保回数）** | 314,395 | **2,066**（**-99.3%**） |
| 区間の peak | 48.10 MiB | **0.03 MiB**（-48.07） |
| 返り値 | +101 件 | +101 件（**不変**） |
| 索引の常駐 / ブロック | 35.44 MiB / 411,413 | 35.44 MiB / 411,413（**不変**） |

**3 区間の和を削減幅として書かない。** 消えるのは PATH 区間の約 130 ms だけで、ロードと構築は
この変更の対象外である（実測はぶれの範囲に収まる——ロード 119〜167 ms・構築 54〜69 ms）。
和を取ると対象外のぶれが削減幅に混ざる。

**受け入れは壁時計ではなく確保回数で判定した。** 壁時計は**同じコードで 54〜64 ms**（±17%）
とばらつくので単一値を掲げても意味を持たない。確保回数は 9 回測って 9 回とも 2,066 だった。
削減の主張はそちらに載せてある。
...
いる前提（ASCII 高速路でフルパスと末尾セグメントが別分岐を通りうること）は同関数の doc
が正本**。逆向き——別ディレクトリの同名ファイルが通り抜けること——は篩の仕様である。

**検知器は 3 本とも「外して落ちるか」を実測した**（反復 8 で 3 本中 1 本が落ちなかった教訓）:

| 改変 | 落ちたテスト |
|---|---|
| フルパス比較を無条件 true に | `scan_path_dirs_keeps_candidate_that_only_shares_a_file_name`（＋ clippy の `key is never read`） |
| 篩から正規化を外す | `scan_path_dirs_skips_existing_paths_written_in_other_notations` |
| 返り値を逆順に | `scan_path_dirs_preserves_enumeration_order` |

どちらの誤りも**結果が「それらしく」出る**——前者は起動できるはずの exe が黙って減り、
後者は重複エントリが索引へ入る。既存の 6 本はどちらも捕まえていなかった。
...
after の 54〜64 ms のうち列挙が 9 ms なので、残りは 312,691 件のファイル名を小文字化して
ハッシュを引く**純粋な計算**である。確保は 2,066 まで落ちきっているので、**取れる額の
上限は約 49 ms**。

**「篩をもっと粗くするしかない」と書いてはならない**（この節の初稿がそう書いていた）。
残っている道は 3 つあり、粗い篩はそのうち唯一**成立しないもの**である。

| 道 | 見込み | 判定 |
|---|---:|---|
| 走査を rayon で並列化する | -35 ms（`entries_digest` の実測 3.6 倍からの見積もり） | **次の反復の候補**。定常確保ゼロ・read-only の参照で、反復 6 で digest が効いた条件と揃う |
| `by_file_key` を `FxHashMap` へ | -3〜5 ms（見積もり） | 次の反復の候補（依存は既にある） |
| 篩をファイル名長のビットマスク等で粗くする | — | **不成立**。長さは正規化で保存されない（`char::to_lowercase` は UTF-8 長を変える）ので非 ASCII 側で偽陰性が出る。健全化には別実装の分岐が要り、`snotra-core/CLAUDE.md`「`normalize_entry_key` の冪等性契約」が名指しで禁じている |

**「候補の表現が複数あるなら全部測る」を省いたのではない。** 反復 8 では 2 案を実装して
比べたが、ここでは**天井そのものを測った**——列挙 9 ms は表現を変えても消えない。粗い篩は
額ではなく健全性で落ちる。
