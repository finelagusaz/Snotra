...
#### 潰し済みかどうかを型で区別する（結果を壊しうる唯一の点）

`assemble` の共有判定は「渡ってくる `lower_names` は全要素が `Some`」を前提にしている。
**潰し済みの列をそこへ流すと、`lower_file_names[i]` と `lower_names[i]` がどちらも `None` の
ときに「一致」と読まれる**——`entry_view` はその旗を見て file name として `lower_name` を
返すので、**file name 成分を持たないエントリが file name 経由でマッチする**。実データでは
フォルダ 256,262 件すべてが `None` であり、影響は小さくない。しかも検索結果は「それらしく」
出るため挙動テストでは捕まらない。

ゆえに `CachedLower::{Collapsed, Raw}` → `DerivedStrings::{Collapsed, Measured}` と型で分けた。
取り違えはコンパイルを通らない。**判定そのものは `query::measure_derived_sharing` の 1 か所**で、
記録側・追記側（`extend_cached_masks`）・適用側が同じ関数を通ることだけが、ディスクとメモリで
潰れ方が一致する根拠である。
