...
lock を握り続けていた頃も剪定中の挿入は retain の**後**に着地していた。

検知器は `icon.rs` の `concurrent_insert_during_prune_window_survives` で、**窓そのもの**を
検査する（集合の一致だけを見るテストはこの変異を素通りする）。否定した変種そのもの
——`retain(|k, _| alive.contains(k))`——を注入して落ちることを実測した。

副産物: 述語の反転で「生きた集合を積む」形が要らなくなり、ヒットのたびの `String` 確保が
消えた。キーが 0 件なら全件走査ごと省く（**初回起動がまさにその場合**）。
...
——判定はここが正本で、再び測る必要が生じるのは剪定が lock の中へ戻るときだけである。
