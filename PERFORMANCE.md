...
篩を抜けた少数だけがフルパスの正規化キーまで進む。両辺は `normalize_file_name_key_into`
という**同じ 1 つの手順**（`trim` → 末尾セグメント → `normalize_entry_key_into`）を通る。

正規化は「全体 `trim` → 文字単位の写像（小文字化と `/` → `\`）」であり、写像は新たな `\` を
生まないのでセグメント境界を保つ。ゆえに `normalize_entry_key(a) == normalize_entry_key(b)`
ならファイル名キーも必ず一致する。**逆は成り立たない**——別ディレクトリの同名ファイルは
通り抜けるので、通した候補はフルパスで確かめる。
