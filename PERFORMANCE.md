...
**読むときの但し書きが 1 つある。`update_us` には trace の書き込みが含まれる**（1 本あたり
約 10 ms。stderr をファイルへリダイレクトした同期 write）。実質的な処理が無い区間でも
trace 間が 10〜18 ms 空くことを実測した。**絶対値を実挙動と読んではならない**——A/B の両側へ
等しく乗るので差分として読む。この汚染は #1004 で smoke へフレーム予算の不変条件を置く案を
取り下げた理由でもある（`docs/superpowers/specs/2026-08-10-search-worker-design.md`）。
