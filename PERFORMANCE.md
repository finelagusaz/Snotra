...
| 経路 | main | アリーナ | 判定 |
|---|---|---|---|
| `users`（区切り無し） | 98–105 µs | 99–178 | 重なる（上振れ 2 回） |
| `c:\` | 16,532–17,022 | 16,731–17,060 | 重なる |
| **`c:\users`** | **11,505–11,520** | **12,104–12,226** | **重ならない（+5.3%）** |
...
**ノイズでは説明できない。** main 側の `c:\users` は 11,505 / 11,517 / 11,520 とばらつきが
0.13% しかなく、+5.3% はその外にある。

**機序は切り分けていない。** `String` の範囲添字が両端で UTF-8 文字境界を検査することを疑い、
`get_unchecked` 版を実装して測ったが**回復しなかった**（`c:\users` の p50 11,905–12,364 で
main と重ならないまま、ばらつきだけが広がった）。ゆえに `unsafe` は採っていない
——**「安全のために遅い形を選んだ」のではなく、速くならなかったから採らなかった。**
パスクエリ全走査そのもの（`measure_path_query_sweep_cost` の導出列）は両側でレンジが重なるので、
`PathCursor` の組み立てではなく scoring 側のどこかである、までしか言えない。
