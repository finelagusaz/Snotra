...
| 述語の向き | 窓の間に挿入されたキー |
|---|---|
| 残す集合（`retain_paths(alive)`） | alive に**居ない**ので落ちる ＝ 挙動変更 |
| 落とす集合（`remove_paths(dead)`） | dead にも**居ない**ので残る ＝ 挙動同一 |

**挿入の軸に限れば「改善」ではなく「同一」である。** 挿入側は抽出を lock の外で行い挿入で
lock を取り直すので、lock を握り続けていた頃も剪定中の挿入は retain の**後**に着地していた。
**これは実装を読んで導いた主張であって、測ってはいない。** **他の軸では同一でない**——
窓の間の世代交代は**この差分で新しく生まれた形**である（旧実装では `invalidate_icon_cache` が
同じ lock を要求するため剪定の途中に着地できなかった）。全体は下の残余の正本を読むこと。

検知器は `icon.rs` の `concurrent_insert_during_prune_window_survives` である。**測るのは
述語の向きであって、スレッドと mutex の等価性ではない**（変異注入の手順はそのテストの doc）。
**lock の構造そのものを守る検知器は無い**——2 回の lock 取得を 1 回へ戻す退行は全テスト緑の
まま通る（受容する残余）。
...
こと——ここへ数え上げを写すと、残余が増えた日にこの行だけが偽になる）。
...
シナリオに依存しない形であり、その 2 行の振れ（36.6/38.0・30.3/35.0）がこの計器の雑音でも
ある。**B と A の差 2% は雑音の中にある。**
