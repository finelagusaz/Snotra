...
- **インデックスを再構築しただけではキャッシュを掃除しない**（#996 で撤去。索引に無いキーは下記の FIFO 上限まで残りうる）。キャッシュを丸ごと破棄するのは次の 2 つの場合に限る
  - 背景再スキャンでエントリ集合が変わったとき（`RescanOutcome::Changed`）: メモリ内キャッシュと `icons.bin` の両方を無効化し、次回検索時に再抽出する
