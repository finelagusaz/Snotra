...
- 通常起動時はハイブリッド方式:
  ...
  - 再構築が予定されている場合（設定変更・手動再構築いずれの契機でも）、差分スキャンの結果は差し替えず本式の再構築に譲る
- 設定画面から手動再構築可能
...
- **索引の再構築は個別キーの照合剪定をしない**（#996 で撤去。索引に無いキーだけを狙って間引く経路は無く、下記の FIFO 上限に委ねる）。一方、キャッシュを**丸ごと**破棄する経路は別にあり、次の 3 つの場合に限る
  - **索引の再構築が始まったとき**（初回構築・`/s`・設定変更のいずれの契機でも）: メモリ内キャッシュと `icons.bin` の両方を無効化し、次回検索時に再抽出する
  - 背景再スキャンでエントリ集合が変わったとき（`RescanOutcome::Changed`）: メモリ内キャッシュと `icons.bin` の両方を無効化する。**この経路は撤去予定**（背景再スキャンの撤去と合わせて #1001 で消す）——撤去後は 1 つ目の契機に一本化される
...
- 背景再スキャン（§3.3）がキャッシュヒットの起動ごとに 2 行（`start` / `end`）を追記する。**`start` は走査の前に書く**ため、`start` だけ在って `end` が無い行は「完走せずにプロセスが終わった起動」を表す
- **§13.2 の共通保存仕様は適用しない**（ヘッダを持たず、追記であり、原子的置換もしない）
- **消えても壊れても書けなくてもアプリの振る舞いは変わらない。** 読み手は人間と計測ハーネスであり、アプリはこのファイルを読まない
- およそ 200 行を保ち、超過分は古い方から捨てる（剪定後に `start`/`end` の 2 行が追記されるため、実際の行数は一時的に 200 を超えうる）
