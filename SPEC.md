...
- 実装事実（現在どう動いているか）は `src/*.rs` を参照する
...
- Rust + egui（`eframe`）で GUI を構築し、Windows 固有機能は `windows` クレートで実装
...
- 左カーソルキー:
  ...
  - 通常検索モード: 選択アイテムの親フォルダ内容で候補を置換し、フォルダ展開モードに入る
...
- ホットキー: 保存時に再登録（失敗時は旧設定維持）
- 検索方式/最大件数: 保存直後に即時反映
- 見た目設定: 保存時に即時再描画（必要ならウィンドウ再作成）
- インデックス条件/アイコン設定:
  - 保存後に再構築確認ダイアログを表示
  - ユーザー承認時に再構築実行
...
- フォーカス喪失時の自動非表示（設定で切替）
- ホットキーでのトグル動作（設定で切替）

### 7.2 ウィンドウ位置

- ドラッグ移動位置を記憶し次回表示時に再利用
- タイトルバー有無に関わらず保存位置を利用

### 7.3 タイトルバー

- 設定で表示/非表示切替
- 非表示時はフレームレス（`WS_POPUP`）
