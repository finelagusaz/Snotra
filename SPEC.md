...
- **抽出のキーは結果行ごとに定まる**（#1133）。既定は行の `path`。`path` がファイルを指さない行は**抽出そのものを行わず**、単色の placeholder を描く。表示文字列とは別のファイルを指す行は、そのファイルのパスをキーにする（種別ごとのマッピングは §19.5）
  - **列挙失敗行（§6.6）はこの規則の対象外である**。`path` が実在ディレクトリを指していても抽出せず、placeholder も描かない（アイコン形の装飾をエラーメッセージに添えない）
...
- **高さは行高の整数倍ちょうどで、余りを持たない。** 行の描画側が行間の余白（egui の `item_spacing.y`）を 0 にして**行ピッチを行高に一致させている**ことと対であり、片方だけ変えると最終行が切れるか、最大表示件数 + 1 行目の頭が覗く。かつて式にあった `+ 8px` は 1 ウィンドウ時代の結果リスト**内側**の余白を #646 PR2 がウィンドウ高へ持ち込んだもので、ウィンドウを分けた時点で「ウィンドウの高さ = スクロール領域の高さ」になり余白としては機能していなかった
...
- **この高さは `results` に描くすべてのビューへ一様に適用される**——通常結果・フォルダ展開・ツール選択メニュー（§18.4）・インスタントコマンド行（§19）。行数が状況で変わってもウィンドウの高さは変わらない
- **0 件のときはウィンドウを出さない**（§8.6 の連言「結果が空でない」）。最大表示件数が 0（`config.toml` の手編集でのみ到達する）のときも出さない
- **最大表示件数が 0 で隠れている行は起動にも使えない**（#1106）。Enter・Shift+Enter・クリックは起動もツール選択への入場も行わない。**止めるのは操作だけで、行データと選択は消さず**、フォルダ展開（→ / ←）は止めない。**この規則が係るのは最大表示件数が 0 の側だけである**——0 件（§8.6 の連言「結果が空でない」）は起動する行そのものが無い。設定を戻す手段は残る: `/o` は Enter を経ず完全一致で走るため、このゲートを通らない（§15.1）
- **作業領域の下端でクランプしない**（#835 で #675 を撤去）。収まらない分は画面外へはみ出す。メインウィンドウを作業領域の最下端へ置くと `results` が丸ごと画面外へ出て 1 行も見えないことがあるが、**位置を動かせば直るものとして受容する**——表示位置によってウィンドウの大きさが変わらないことを優先した（#835 の人間裁定）
- **画面外へ出た行はスクロールでは手前に来ない。** ウィンドウの高さが件数によらず一定である以上、候補が最大表示件数以下ならスクロール可能量はゼロだからである（ウィンドウの内側でのスクロールが効くのは候補が最大表示件数を**超えた**ときだけ）。#675 が「あふれた行はスクロールで到達する」と書けたのは、ウィンドウが作業領域に合わせて縮むことでオーバーフローが生じていたためであり、#835 でその生成源ごと消えた
- **「位置を動かせば直る」が成り立つのは、ウィンドウが作業領域に収まる場合に限る。** 最大表示件数を作業領域より高くなる値に設定すると（既定の行高 36.7px・1080p @125% なら 22 行あたりが交差点。設定画面の上限は 50）、下端の行はメインウィンドウをどこへ動かしても見えず、上記のとおりスクロールでも到達できない。**受容する残余であり、復帰手段は設定値を小さくすることだけである**（#835）。**設定画面の上限を下げて塞ぐ案は却下した**——収まる値は解像度・DPI・タスクバーの配置で変わり単一の定数にできないため（`docs/adr/ADR-results-fixed-height.md`）
...
- Windows のサインイン時に Snotra を起動する（スタートアップ登録。「起動時にウィンドウを表示」とは別概念で、`config.toml` に保存されない。正本は §7.7）
- 言語（日本語 / English）
...
- **スタートアップ登録には影響しない**（`config.toml` の外にあるため。§7.7）
...
- **スタートアップ登録だけは `config.toml` を経由しない**（§7.7）。設定アプリがレジストリへ直接書いて即時に効き、本体は関与しないため `config_watcher` は発火しない
...
`[全般]` タブの「スタートアップ」節から、Windows のサインイン時に本体を起動するかを切り替える。**本節がこの機能の正本であり、他節はここを参照する**（`§7.7` で grep すれば参照元は数え上げずに辿れる）。

- 状態の正本は **OS のレジストリ**であり、`config.toml` ではない: `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` の値名 `Snotra`（REG_SZ）
- 値は**引用符で括った `snotra.exe` の絶対パス**（例: `"C:\...\Snotra\snotra.exe"`）。引数は付けない。引用符が要るのは、既定のインストール先が `%LOCALAPPDATA%` 配下でパスにユーザー名を含み、ユーザー名は空白を含みうるため
- 本体（`snotra`）はこの値を読まない。読むのは Windows のサインイン処理だけである
- **値名 `Snotra` は凍結する**——変えると既存の登録が孤児になる（古い名前の値が残り、新しい名前で二重に登録される）
- **Save ボタンを経由せず即時に効く。** チェックボックスの切り替えでレジストリへ書き、結果をタブ内にインライン表示する。`config.toml` を書かないため `config_watcher` も発火しない（§7.5）
- チェックを入れる操作は、既に別のパスで登録されていても**現在のパスで上書きする**（移動したポータブル版の陳腐化したエントリはこれで治る）
- チェックを外す操作は**冪等**である。値が既に存在しなくても成功として扱う
- 昇格（管理者権限）を要求しない（`HKCU` のため）
- **アンインストールするとこの値も消える。設定アプリでも本体でもなく、NSIS のアンインストーラが消す**——Tauri の NSIS テンプレートが `Section Uninstall` で `DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "${PRODUCTNAME}"` を呼び、`${PRODUCTNAME}` は `src-tauri/tauri.conf.json` の `"productName"` から来る（2026-09-02 実測: `npx tauri build --bundles nsis` が生成した `installer.nsi` で `!define PRODUCTNAME "Snotra"`・`@tauri-apps/cli` 2.11.4）。**値名 `Snotra` の凍結は、この一致にも効いている**
- **updater 経由の更新では消えない。** `tauri-plugin-updater` 2.10.1 はインストーラを `/UPDATE` 付きで起動し、テンプレートの削除はその指定を受けたときは走らないよう書かれている（2026-09-02 実測）。更新をまたいでスタートアップ登録は保たれる

**受容する残余**:

- **タスクマネージャーの「スタートアップ アプリ」で無効化されても検知しない。** Windows は `Run` 値を消さず `Explorer\StartupApproved\Run` へ無効マークを置く（2026-09-01 実測: 無効化で先頭バイトが `02` → `03` になり、`Run` 値はそのまま残った）。ゆえにチェックが入ったまま実際には起動しない状態が起こりうる。チェックボックスの意味論は「`Run` 値が存在する」であって「実効的に有効」ではない
  - **追随させない理由は、読めないからではなく、そこが利用者の領分だからである。** 無効化はタスクマネージャーで利用者が明示した意思であり、設定アプリがそれを読み替えたり上書きしたりするものではない
  - **チェックを外しても `StartupApproved` のレコードは残る**（消すのは `Run` 値だけ）。Windows 自身が同じ振る舞いをする——`Run` に居ないアプリのレコードが残っている例が実機にある。**解除の失敗ではない**
- **設定アプリの起動中に外部からレジストリを変えても追随しない。** 読むのは起動時と操作直後だけである
- **ポータブル版のフォルダを移動すると、登録済みの絶対パスが死ぬ。** チェックを入れ直せば治るが、利用者には気づく契機が無い
- **新しいインストーラを手で重ねて実行し、再インストールページで「先にアンインストールする」を選ぶと、値は消える。** その経路の旧アンインストーラは `/UPDATE` を受け取らないため、上の削除が走る。updater 経由の更新とは違う経路であり、**「更新では消えない」は updater 経由についてのみ言える**
- **削除が効くのは `RUN_VALUE_NAME` と `"productName"` が一致している間だけである。** どちらかを変えても、コンパイルもテストも緑のまま、アンインストール時の削除だけが静かに空振りする。**`tauri.conf.json` は厳格な JSON でコメントを書けず、注意書きは Rust 側にしか置けない**ため、この結合を守る機構は置いていない（#1211 で検討し、差分を最小にする判断で見送った）
- **上流 Tauri がテンプレートから削除行を外しても気づけない。** テンプレートはこのリポジトリの中に無く、依存更新の差分にも現れない。実測しうるのは `tauri build` が生成した `installer.nsi` を読むときだけである
- **ポータブル版（ZIP）にはアンインストーラが無いため、この経路では消えない。** 値は利用者が設定アプリのチェックを外すか、手でレジストリから消すまで残る

## 8. ウィンドウ動作

### ウィンドウの名称と役割

本書と実装で使う呼称を固定する（#670）。**1 つのウィンドウに 1 つの呼称**——「検索ウィンドウ / 入力ウィンドウ / 検索バーウィンドウ」のような同義語は、どれが同じウィンドウを指すのか読者に判定させる。

| 呼称 | コード上のラベル | 役割 |
|---|---|---|
| **メインウィンドウ** | `main` | ホットキーで最初に現れるウィンドウ。検索文字列の入力・キーボード操作に加え、バー直下の **status 行**（インデックス構築案内 / 一時通知 / 起動中）と **updater トースト**を積む。UI で唯一フォーカスを持つウィンドウ |
| **検索結果ウィンドウ** | `results` | 検索結果とフォルダ列挙の行を描く従属ウィンドウ。フォーカスを取らず、可視性・サイズ・位置はメインウィンドウが駆動する |
...
- コード上のウィンドウラベルは英字 `main` / `results` を正とする（Win32・tao・トレースが参照するため）
...
- **キーボードによるウィンドウ移動（`Alt+Space` → `M` → 矢印キー）は、実測では移動中に拘束されない——ただしドラッグと違い、拘束しないことが機構で保証されてはいない。** 復帰は確定（`Enter`）後の最初のフレームで 1 度だけ働く（#1194 実測・13 反復）。**保証の強さが違う理由は機構の違いにある**: ドラッグはポインタが押されている間を除外するが、キーボード移動にポインタ押下は伴わない。除外しているのは**フレームが来ないこと**であり、OS のモーダル移動ループ中は本体がイベント駆動で待つため通常フレームが回らない。**ループ中にフレームが走る条件が生じれば、そのフレームでは引き戻される**——それがこの記述を偽にする条件である。なお #1173 は逆の観測（移動中も拘束される）を記録しており、条件の違いは未特定である
...
- タイトルバーは常に非表示（生成時に `main`/`results` 両ウィンドウとも装飾なしで作る）
...
- メインウィンドウ（`main`）と検索結果ウィンドウ（`results`）は起動時のセットアップで不可視のまま生成する。`results` はフォーカスを取らない従属ウィンドウとし、可視性・サイズ・位置は `main` の毎フレーム更新が駆動する。`main` の hide 時は `results` も同時に隠す（§4.7・§8.2 参照・#646 PR2）。**この hide 同期と組になる show 側のゲートは §8.6「検索結果ウィンドウの可視性」にある**——hide 同期だけでは閉じない
...
- `NormalMode -> ToolSelectionMode` の `Shift+Enter` は **§4.7 の表示ゲートと §4.5 の「最大表示件数が 0」の規則にも従う**——**独立した 2 つの規則であり、どちらか一方でも行を隠していれば入場しない**（規則の正本はそれぞれ §4.7 と §4.5）。前者はインデックス構築中に隠れている通常結果、後者は最大表示件数が 0 で 1 行も描かれていない場合である。**ゲートが効くのは行の起動・入場であって、スラッシュコマンドの実行ではない**——後者は完全一致した時点で Enter を経ずに走る（§15.1）。ただし `/r` が出す履歴行は通常結果なので、その**起動**には §4.7 が効く。また構築中にコマンドが効くかは各コマンド側の `indexing` ガードが別に決めており、設定オープンも索引再構築も同じ `ERR_INDEXING_IN_PROGRESS` で拒否される（上の `/o` の行）。**拒否されてもクエリのクリアは走る。** 索引再構築ではさらに非表示も走るため、見た目が成功時と区別が付かない（意図的無音・#434）。設定オープンでは設定ウィンドウが現れないことと status 行の構築中案内が理由の提示になる
...
- 検索結果ウィンドウ（`results`）の表示/非表示は上図の状態ノードではなく、`SearchVisible` に従属する直交軸である（下記「検索結果ウィンドウの可視性」）。ウィンドウの生成・hide 同期は §8.5 参照（#646 PR2）
...
```
results 可視 ⇔ main 可視 ∧ 結果が空でない ∧ 通常結果を隠していない ∧ ウィンドウ高さ > 0
```

| 連言項 | 正本 |
|---|---|
| 結果が空でない / 通常結果を隠していない（インデックス構築中の carve-out） | §4.7 |
| ウィンドウ高さ > 0（最大表示件数分の固定高。**0 になるのは最大表示件数が 0 のときだけ**——件数では 0 にならない・#835） | §4.5 |
| main 可視 | 下記 |
...
ウィンドウごとに「いつ生まれ、いつ現れ、いつ消え、いつ壊れるか」を 1 か所に置く（#670）。個々の制約は §8.1〜§8.5 が詳しい。

**メインウィンドウ / 検索結果ウィンドウ**（本体プロセス内・2 ウィンドウとも同じ生存期間）
...
- メインウィンドウ（`main`）と検索結果ウィンドウ（`results`）は `window_gap`（既定 4px・`[visual]` の config キー・live-read）だけ離して配置し、両ウィンドウに DWM 角丸（`DWMWA_WINDOW_CORNER_PREFERENCE` = `DWMWCP_ROUND`）を適用する。Windows 11（build 22000+）でのみ有効な best-effort で、Windows 10 では角丸なしの矩形のまま残る（#646 PR2 決定4）
...
- **スタートアップ登録は export / import のどちらにも含まれない**（`config.toml` の外にあるため。§7.7）。含めると、機体ごとに異なる絶対パスと OS 状態を持ち運ぶことになる
...
- 起動中であることは検索バー直下の status 行に示す。失敗・結果不明のときは一時通知を出す（描画面の規則は `SPEC.md`「4.7 結果表示制御（2 ウィンドウ構成）」）
...
- 結果行のアイコン（キーの規則は §3.4）: exec 種別は `exe`（環境変数を展開したもの）のアイコンを表示する。URL 種別は取得しない
...
- インデックス構築中でもインスタントコマンドは使用可能（インデックスに依存しないため）
  ...
  - インスタントコマンドモード中の結果表示はインデックス構築中かどうかを見ない（`SPEC.md`「4.7 結果表示制御（2 ウィンドウ構成）」の 2 軸モデルから導出）
...
- 検索バー直下にトースト行高（= バー高と同値・既定 43px）の**単一行**として、モード
  ...
  `SPEC.md`「4.7 結果表示制御（2 ウィンドウ構成）」の規則（位置はバー高で決まり行の出没に追従しない・
