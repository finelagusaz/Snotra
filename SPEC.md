...
- **「位置を動かせば直る」が成り立つのは、窓が作業領域に収まる場合に限る。** 最大表示件数を作業領域より高くなる値に設定すると（既定の行高 36.7px・1080p @125% なら 22 行あたりが交差点。設定画面の上限は 50）、下端の行はメインウィンドウをどこへ動かしても見えず、上記のとおりスクロールでも到達できない。**受容する残余であり、復帰手段は設定値を小さくすることだけである**（#835）
