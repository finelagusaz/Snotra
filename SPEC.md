...
- 設定の読み込み失敗時の扱い（`config_watcher`）:
  ...
  - 一時的・環境的な失敗（権限/ロック/共有違反, `LoadOutcome::ReadFailed`）: **実行中の設定を維持し、何も適用しない**（既定値で上書きせず、再インデックスや履歴剪定も走らせない）。`config.toml` は無傷なので次の保存イベントで正規の変更を拾う。live-read 化した履歴剪定が既定値で走るとデータ損失になるため（#348）
