...
- パスキー: lowercase 化とパス区切り統一のみを適用し、アクセント折りたたみは行わない（パスは識別子であり、`Résumé.lnk` と `Resume.lnk` は別ファイル）
- クエリキー: パスキーと同じ正規化に加えてアクセント折りたたみ（é→e 等）を適用し、アクセント違いのクエリは同一バケットに統合する。正規化の実装契約（関数名・正規化ステップの内訳）は `snotra-core/CLAUDE.md`「history.rs のキー正規化に関するチェックリスト」を正本とする
