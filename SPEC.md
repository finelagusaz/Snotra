...
- **タスクマネージャーの「スタートアップ アプリ」で無効化されても検知しない。** Windows は `Run` 値を消さず `Explorer\StartupApproved\Run` へ無効マークを置くため、チェックが入ったまま実際には起動しない状態が起こりうる。チェックボックスの意味論は「`Run` 値が存在する」であって「実効的に有効」ではない
