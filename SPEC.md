...
- **この規則で隠れている行は起動にも使えない**（#1077）。隠すのは表示だけで**行データと選択は保持する**ため、隠れた行が残り続ける。インデックス構築中に隠れた通常結果に対する Enter・Shift+Enter・クリックは、起動もツール選択への遷移も行わない（§8.6 の `Shift+Enter [tools >= 2]` はこのゲートに従う）。止めるのは不可逆な起動だけであり、構築が終われば同じ行がそのまま使える。フォルダ展開（→ / ←）は起動ではないので止めない
