...
- show 時は、表示直後に描かれる行（status 行・toast 行があればその分）まで含めた高さへ揃えてから位置を決める。結果件数による `main` の伸縮はしない（#646 PR2 決定6・#755 / #801）
...
- 検索バー直下にトースト行高（= バー高と同値・既定 43px）の**単一行**として、モード
  ...
  揃えてから位置を決める（#755 / #801）。ただし別スレッドの非同期な状態変化（updater の
  進捗）がこの揃えと最初のフレームの間に挟まった回は、その 1 フレームだけ高さがずれ、
  同じフレームの動的高さ算出で直る（固着はしない）
