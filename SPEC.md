...
- 検索結果ウィンドウ（`results`）の高さは**常に最大表示件数分を確保する**（`最大表示件数 × 行高 + 8px`）。ヒット数が最大表示件数未満でも高さは変わらず、超過時はスクロールバーを表示する（#835 で #646 PR2 決定 7 の「実件数フィット」を覆した。理由は `docs/adr/ADR-results-fixed-height.md`）
