...
- **キーボードによるウィンドウ移動（`Alt+Space` → `M` → 矢印キー）も、移動中は拘束しない。** 復帰は確定（`Enter`）後の最初のフレームで 1 度だけ働く——ドラッグの「離したら戻る」と同じ保証である。**ただし機構が違う**: ドラッグは `!any_down()` が除外するが、キーボード移動にポインタ押下は伴わない。除外しているのは**フレームが来ないこと**であり、OS のモーダル移動ループ中は本体がイベント駆動で待つため通常フレームが回らない（§8.4 の「不可視中は `update()` が走らない」と同じ、回る契機が無いという理由）。**これは機構による保証ではない**——ループ中にフレームが走る条件が生じれば、そのフレームでは引き戻されうる（#1194 実測。#1173 は逆の観測を記録しており、条件の違いは未特定である）
