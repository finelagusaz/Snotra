...
- `results` ウィンドウは起動時に事前生成する（検索レイテンシのため）。`about` と `settings` はオンデマンド生成・クローズ時破棄（メモリ削減のため）
- `/o` スラッシュコマンドで開く
- タブ切り替えUI
...
- `results` は起動時のセットアップで事前生成（`visible: false`）。初回表示時は既存インスタンスを `show` する
- `about` と `settings` はオンデマンド生成。`open_about` / `open_settings` 呼び出し時に `ensure_*_window` で生成し、閉じる操作で破棄する（アイドル時のメモリ消費を削減）
- `platform.rs` の Win32 メッセージループスレッドは `results` ウィンドウ事前生成より前に spawn し、Win32 初期化（`RegisterClassExW` / `CreateWindowExW`）とウィンドウ生成を並列実行する（起動時間の短縮）
- トレイアイコンの表示はリスナー登録完了後に行う。`about` / `settings` はオンデマンド生成のため、トレイ表示時点で存在する必要はない
