...
- **プレースホルダゆえ、フォルダ内の絞り込み文字列が空のときだけ表示される**。1 文字でも入力すると消え、消すと戻る（受容する残余）。左右カーソルキーは絞り込みをクリアする（§6.1）ので、階層を移動した直後は必ず表示される
...
- 入力欄の幅に収まらないパスは末尾が `…` で省略される（受容する残余）。日本語 UI では接尾辞「 内を検索...」が先に削られ、英語 UI ではパスの末尾（＝いま居るフォルダ名）から削られる
