...
- 入力欄の幅に収まらないパスは末尾が `…` で省略される（受容する残余）。日本語 UI では接尾辞「 内を検索...」が先に削られ、英語 UI ではパスの末尾（＝いま居るフォルダ名）から削られる。**深い階層では現在のフォルダ名が読めなくなるが、`window_width`（§7.2）を広げれば解決するため、アプリ側に省略の機構は持たない**（既定幅 600px・日本語 UI での実測: パス 71 字は全部見え、94 字で削られる）
