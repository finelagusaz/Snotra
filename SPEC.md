...
**ただしこの 2 つでは十分でない。** 述語は毎フレーム再評価されるが、評価と `results` への `ShowWindow` の間には位置決め・リサイズの Win32 呼び出しが挟まり、`main` の hide は**別スレッド**（ホットキーは Win32 メッセージループスレッド）からその隔たりへ割り込める——「評価時は `main` 可視、適用時は `main` hidden」という並びが成立する。ゆえに含意 `results 可視 ⇒ main 可視` は次の 3 点で守る:
...
- クエリキー: アクセント折りたたみを適用し、アクセント違いのクエリ（例: `café` と `cafe`）は同一の履歴バケットとして扱う。正規化ステップの内訳（関数名・パス区切り統一の適用有無等）は `snotra-core/CLAUDE.md`「history.rs のキー正規化に関するチェックリスト」を正本とする
