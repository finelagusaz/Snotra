...
- パスキー: アクセント折りたたみは行わない（パスは識別子であり、`Résumé.lnk` と `Resume.lnk` は別ファイル）
- クエリキー: アクセント折りたたみを適用し、アクセント違いのクエリ（例: `café` と `cafe`）は同一の履歴バケットとして扱う
- 両キーの正規化ステップの内訳（関数名・パス区切り統一の適用有無等）は `snotra-core/CLAUDE.md`「history.rs のキー正規化に関するチェックリスト」を正本とする
