...
- **キーボードによる窓移動（`Alt+Space` → `M` → 矢印キー）は、ドラッグと違って拘束される。** ポインタが押されないため復帰が毎フレーム働き、移動中もバー矩形は作業領域の内側へ引き戻される——**移動先が作業領域の外になる位置には確定できない**（#1173 実測）。単一モニターでは「外へ出せない」に留まるが、複数モニター環境では上と同じ理由で隣のモニターへ渡れないと見込まれる
