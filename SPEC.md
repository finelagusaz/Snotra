...
可視性を**変える**操作（`main` の show / hide・`results` の raw show / hide）がイベントループスレッド上でしか呼べなくなった（`!Send` な証人型を引数に要求する・#880 サイクル段 2）ことで、評価と適用の間へ別スレッドが割り込む並びは構築できなくなった。事後検査と hide の権威性は不要になり、同段で撤去した——上の 3 点のうち残るのは事前ゲートだけである。§8.5 の hide 側の同期は本節冒頭のとおり別軸のまま必要である。
