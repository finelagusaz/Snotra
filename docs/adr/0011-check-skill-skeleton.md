...
この却下は、本サイクルで 3 度目の同じ失敗様態でもある。ADR-0003 却下 4 の「手を抜く読者の逃げ道を塞ぐ修正は、忠実な読者の実行可能性を削る方向に働く」を、設計レベルで再現した。**義務を足す方向の 1 文は、規模を問わず同じ罠を持つ。**
...
- **`/norm-review` を 2 巡回した。** 1 巡目は手を抜く読者 29 件・規則を全部守る読者 18 件（合格条件 K1〜K5 が 5/5 突破）。**2 巡目は較正して規則を全部守る読者だけを回し、4 件**（件数ではなく「忠実に実行すると詰まる箇所」だけを探させた——較正の根拠は `docs/adr/0012-two-class-reader-discrimination.md`）。件数はいずれも読者が書いたファイルの番号を数え直して立てた
