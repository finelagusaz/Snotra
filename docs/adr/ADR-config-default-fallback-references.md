...
3. **確保・OS 呼び出しを伴う既定は `unwrap_or_else` で受ける**
...
- **`snotra-settings` の `PRESETS` を既定値から導出する**: 却下。`PRESETS` は `&'static str` の `const` であり、`String` を返す `VisualConfig::default()` から const 文脈で導けない。**置換で消せない写しはテストで固定する**——UI が使う述語（`preset_matches`）をそのまま呼ぶ形にした（自前で 5 色を比較すると、UI が守る不変条件より厳しい主張になる）。
