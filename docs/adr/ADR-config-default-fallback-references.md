...
- **`window_coordinator.rs` の `inner_size()` 失敗時**: **config の実値へ倒す**。主たる読み元は OS の現在サイズのままである（この折り畳みは高さだけを変え、幅は現状を保つ意図なので）。変えたのは落とし先だけで、上の却下理由が名指ししていた欠陥——`window_width = 900` のユーザーで問い合わせが失敗すると窓が 600 へ縮む——がこれで閉じる。読み口は `read_window_width`（`read_metrics` と同層）。**却下理由の (b)「issue の該当項がコードを読む人に届かなくなる」は、届いた結果ここに決定が書かれたことで役目を終えた**
- **`launcher_controller.rs` の `Language::Ja` 固定**: **OS ロケールへ倒す**。`ja` で始まらないロケールと取得失敗はいずれも英語で、これは `SPEC.md`「7.6 起動時の設定初期化」が既に宣言している挙動である——つまり固定 `Ja` は仕様からの逸脱であり、この変更は仕様変更ではなく修正である。**`default_language()` を `pub` にはしない**（上の却下 1 を維持）——`GeneralConfig::default().language` を経由する
