...
- **`window_coordinator.rs` の `inner_size()` 失敗時**: **config の実値へ倒す**。変えたのは落とし先だけで、上の却下理由が名指ししていた欠陥——`window_width = 900` のユーザーで問い合わせが失敗すると窓が 600 へ縮む——がこれで閉じる。読み口は `read_window_width` に**一本化**し、`view.rs` の `window_width` もそこへ委譲する（読みと落とし先を独立実装に分けない——同型の乖離が `read_metrics` の doc に 52.0/43.0 として記録されている）。**却下理由の (b)「issue の該当項がコードを読む人に届かなくなる」は、届いた結果ここに決定が書かれたことで役目を終えた**
  - **この折り畳みが `inner_size()` を主たる読み元に据え続けてよいかは、ここでは決めていない。** 幅について config が正本であること（`view.rs` の `window_width` の doc が記録する、config_watcher との 2 次元 read-modify-write race の除去）と、main 窓が `resizable(false)` であることを併せると、OS 往復そのものが不要になりうる。**その撤去は挙動の変更**（hidden 中に幅を変えた場合の show 時スナップの有無が変わる）なので、落とし先の決定とは別に扱う
