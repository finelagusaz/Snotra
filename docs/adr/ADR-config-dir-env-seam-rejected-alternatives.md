...
（3 本の検証スクリプトが seed を書く理由も失効していない。共通する理由が「parse 失敗を避けるため」から
「ファイル自体が無いと first-run になり、`Config::default()` の探索パスシードで実マシンを索引しうるため」へ
変わった。そこから先の理由は 3 本で別々である——各スクリプトのコメントを正本とする。）
