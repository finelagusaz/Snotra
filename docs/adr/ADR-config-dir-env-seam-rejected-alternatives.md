...
**その後（#824）**: この `#[serde(default)]` は**付いた**。ただし**上の却下は当時の判断として
今も正しい**——却下したのは「検証スクリプトの都合で」という動機であり、#824 が根拠にしたのは
`SPEC.md`「13.1 設定データ」が既に宣言していた「欠損キーはデフォルト補完」への追随、つまり
製品側の独立した理由である。**「部分的な破損を黙って既定値で塗り潰さない」という保全方針も
緩んでいない**——欠落セクションを既定で埋めても `config.toml` は書き換わらず（欠落は
`apply_migrations()` の変更に寄与しないので save されない）、真の構文エラー・非 UTF-8 は
従来どおり `.bak` 退避経路を踏む。変わったのは「欠落を破損と見なすか」であって、
破損の扱いではない。**却下の判断そのものは書き換えない**——当時の文脈を残すのが ADR の目的である。
（3 本の検証スクリプトが seed を書く理由も失効していない。共通する理由は「parse 失敗を避けるため」から
「ファイル自体が無いと first-run になるため」へ変わった。その共通部分は
`scripts/lib/SnotraSmoke.psm1` の `New-SnotraVerificationProfile` を正本とし、そこから先の
3 本で別々の理由は各スクリプトのコメントを正本とする。）
