...
## 旧 ADR の案 G との関係（却下理由の 2 つが偽になった）

**本決定は `ADR-config-read-exception-discriminator` の案 G（規範を型・構造で強制する）の一部を実現している。** 同 ADR は案 G を 3 つの理由で却下したが、**そのうち 2 つは本サイクルで偽になった**。

1. 「**弁別子が否定形の述語だから型で表せない**」——弁別子そのものが消えたので、前提を失った。
2. 「`Engine::config()` の可視性を狭めるのは `src-tauri` に正当な呼び出しが残るため不可である」——その呼び出しを 4 件とも移したので、成り立たなくなった。
3. （なお「`Engine` から config を外へ出すのは既に限界まで済んでいる」は今も真である。書きが `Mutex<Engine>` の内側に残るのは `complete_index_drain` の原子性のためで、本決定はそこを触っていない。）

**凍結された ADR は直さない**（`ADR-adr-frozen-history`）ので、事実は生きた層であるここへ書く——同 ADR 自身が案 I で採った作法である。**ゆえに「案 A〜G は却下されたまま」と読んではならない。**
