...
- **ソース述語を「`Resolve-SnotraCargoExecutable` を呼ぶ `.ps1` すべて」へ広げる**: **一部を採用した。**
  当初は 3 本まとめて「変数名も導出の形も違うため誤検出になる」として外したが、**実測するとこの理由は
  1 本にしか当たらなかった**——`visual-input-metrics.ps1` は変数名も導出の形も対象 2 本と同一で、
  3 つの述語を無改造で通る。ゆえに射程へ入れた（あれもアプリを起動して測る道具であり、別コピーを
  測れば同じ欠陥クラスに当たる）。残る 2 本の除外は**それぞれ別の理由**で正当である:
  `run-pester.ps1` は `-ExePath` に既定値を持たず（明示 override 専用ゆえ入口の述語が当たらない）、
  `visual-check-colors.ps1` は変数名が `$repositoryRoot` で `-ExePath` param 自体が無い。
  対象を数え上げる死角（4 本目に黙る）は宣言して止める。
