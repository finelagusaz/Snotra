...
- **`cargo clippy` も `components:` へ明示する**: 却下（射程）。同じ理由で clippy も runner イメージへの暗黙依存だが（action は `--profile minimal`）、5 か月間顕在化しておらず #858 は fmt の issue である。1 語の追加で済むため別 issue で扱える。
