...
- **crate を `rel.startsWith(crate + "/")` で導出する**: 採らなかったが、**却下の理由は当初書いたものとは
  違う**。当初は「`<crate>/tests/*.rs` を拾って索引に載る筋合いの無いファイルへ reminder を出す」と
  書いていた——これは**実装より強い主張で、偽だった**。`G-module-index` の逆方向が名指すのは `cfg.src`
  配下のファイルだけなので `tests/` の `rel` は findings のメッセージに現れず、**前方一致にしても後段の
  `attributesTo` が 0 件へ落とす**。委譲レビューの変異注入がこれを緑のまま通し、実測で確かめ直した
  （実ツリーの `.rs` 101 件と、`tests/` 配下へ索引を持たない `.rs` を注入した対照で**出力の差分 0 件**。
  変わるのは `checkModuleIndex` を呼ぶ回数だけ・95 → 101）。**`cfg.src` を採る実際の理由は費用の
  最適化と、`attributesTo` を将来ゆるめたときの 2 段目である。この選択を守る検知器は無い。**
...
- **旧 WARN が唯一の合図だった経路が 1 つ消えた**（委譲レビューの逆向きの監査が拾った）。旧
  ...
  （`CLAUDE.md` を持つ member だけを縛る）にも掛からない。**露出は小さいが、今はどの層も見ていない。**
