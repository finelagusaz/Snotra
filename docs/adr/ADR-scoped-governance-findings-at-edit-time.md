...
- **crate を `rel.startsWith(crate + "/")` で導出する**: 却下。`G-module-index` の逆方向が見るのは
  `cfg.src`（`<crate>/src/`）配下だけなので、crate 名の前方一致では `<crate>/tests/*.rs` を拾い、
  **索引に載る筋合いの無いファイルへ reminder を出す**。実測（新規 `.rs` を含む直近 20 コミット）では
  索引を伴わなかった 2 件がどちらも `tests/` 配下だった。`MODULE_INDEX_CRATES` の `src` / `exts` で判定する。
