...
- **`pester-flake-repro.yml` はこの表に載らない**（#872/#936）。Pester を同一 runner 上で N 回繰り返して間欠失敗の標本を集める**測定用**の workflow であり、検証コマンドではない——**この workflow の緑は「検査が通った」を意味しない**（反復のうち何回落ちても緑で終わる。判定の SSOT は `scripts/repro-pester-flake.ps1` の冒頭 doc）。起動は手動 dispatch と、実験用の枝（`chore/pester-flake-**` / `exp/pester-flake-**`）への push のみ。
