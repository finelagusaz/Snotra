...
| 同じ走査元のうち **`.rs` と `.md` だけ**を編集し、**その中に**物理改行を跨いだコードスパンがある（#992） | 折れたコードスパン | 同上 |
...
- **`.rs` と `.md` 以外の編集では、上の表の判定がどれも走らない**（`post-edit.mjs` の `editFindingsReminder` が拡張子で早期 return する）。**母集団が広い判定ほどこの差が効く**——見出し参照とコードスパンの走査元はコメント記法を持つスクリプト（`.ps1` / `.psm1` / `.mjs` 等）を含むのに、そこを編集しても編集時には鳴らず、`governance:check` だけが後から赤にする。#992 の実在 6 箇所のうち **3 件がこの面に在った**。
