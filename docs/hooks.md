...
**上の表の発火条件はどれも「編集したファイルが `.rs` か `.md` である」を前提に読むこと。** `editFindingsReminder` が拡張子で早期 return するため、**それ以外の編集では表の判定が 1 つも走らない**——母集団が広い判定ほどこの差が効く（見出し参照とコードスパンの走査元はコメント記法を持つスクリプトを含むのに、そこを編集しても鳴らない）。**行ごとに書き分けない**——ゲートは 1 か所で全行を止めるので、書き分けると「この行だけ限定」という偽の非対称ができる。
