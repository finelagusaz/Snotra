...
- **`config.toml` の WARN 真陽性は事実上 `tauri.conf.json` のみ**（`config.toml` はランタイムのユーザー領域ファイルでリポジトリに実在しない）。その `src-tauri/tauri.conf.json` では WARN（人間向け・Windows 互換の注意喚起）に加えて `csp-test`（機械検査・CSP 契約）が併走する — 役割が違うため両方残している。
