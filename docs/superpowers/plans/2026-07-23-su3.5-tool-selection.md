...
- **書き戻し禁止（#638 決定 2）**: dedup は in-memory のみ。`apply_migrations()` の戻り値 `changed` に**寄与させない**（`load_from_dir_reporting` は `changed=true` で `save_to_dir` する実装・config.rs:826-829。寄与させるとユーザーの手編集行がファイルから消える）
...
Run: `cargo test -p snotra-core dedup_instant`
Expected: PASS（2 tests）。続けて `cargo test -p snotra-core` で既存全テストの無回帰を確認（特に `validate` / instant serde gate / `normalized_default` 系）

- [ ] **Step 5: コミット**

```bash
git add snotra-core/src/config.rs
...
- Produces（Task 3 が依存する正確な署名）:
  ...
  - `pub struct ToolFrame { pub restore_query: String, pub restore_results: Vec<SearchResult>, pub restore_selected: usize, pub target_path: String, pub target_is_folder: bool, pub tools: Vec<OpenerTool>, pub launch_query: String, pub saved_folder_filter: String }`
...
fn enter_tool_from_results_saves_and_replaces_rows() {
    ...
    assert_eq!(f.restore_query, "code");
}
...
/// launch_query は起動 API へ渡す元クエリで復元には使わない（SolidJS #538 の
...
(k) **view.rs の compile-green スタブ**（`ViewKind::Tool` 追加で網羅 match が壊れる 3 箇所。実挙動は Task 3 で確定）:
...
- [ ] **Step 5: TextEdit の無効化（§18.5 入力無効・target_path 表示）と Escape arm**

TextEdit ブロック（683 行付近）: `in_folder` の隣に `in_tool` を導出し、buf と add を変更:
...
```rust
        let mut buf = if in_tool {
            // §18.5: ツール選択中は対象パスを表示（SolidJS inputValue の targetPath parity）
            self.state
                .tool_frame()
        ...
        let response = ui.add_enabled(
            !in_tool, // §18.5 ツール選択中の入力は無効化（disabled は changed も発火しない）
...
auto-focus 行（738 行付近）へ in_tool 除外を追加（disabled ウィジェットへ focus 要求しない）:
...
Escape の match（590 行付近）へ arm 追加:
...
**Files:**
- 変更なし（検証のみ。乖離が出た場合のみ SPEC §18 に as-built 注記）
...
新規ファイル無し（索引更新不要）。Run: `npm run governance:check` → G1..G10 PASS
`src-tauri/CLAUDE.md` モジュール構成の egui_shell 行は責務散文が `//!` 正本のため変更不要（`search_state.rs` の `//!` に tool 軸を一行追記するのは Task 2 に含めてよい）

- [ ] **Step 4: コミット（残余があれば）と push**
