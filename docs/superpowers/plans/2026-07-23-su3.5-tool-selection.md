...
- **G1 不侵**: フラグ OFF（`SNOTRA_EGUI_MAIN` 未設定）の WebView2 経路・SolidJS・IPC の挙動を変えない（`launch.rs` の変更は `launch_with_tool_core` の可視性 `pub(crate)` 化のみ＝挙動中立）。**例外は #638 の dedup**——issue #638 と spec 決定 2 が明示する「両経路共通の core 修正」であり、手編集重複 config の候補リストが両経路で実行と一致する方向へ変わるのは意図（Task 1 review で確認済みの意思決定）
