...
### Task 4: 入口判定を「由来 1 問 → 決定表 1 つ → 停止の内訳 1 か所」に組み直す（最終レビュー後に挿入）

**挿入の経緯**: ブランチ全体の最終レビューが Important 3 件を出し、修正ウェーブ（`c7fd681`）で閉じた。その再レビューは Ready を出したが、残った Minor 4 件のうち 3 件が**同じ形**だった——「同じ事実が 2 か所に書かれ、片方が更新されていない」。これは本ブランチがスキル間で解いた二重所有が、**ファイルの中で再発**したものである。加えて `plan.md` が「このブランチでコミット済み」なら `/start-issue` が Step 6 まで走ったことになり、**同一性とレビュー到達が 1 つの事実で同時に立つ**——現行文はこれを見落として 2 軸を常に別々に確かめており、Minor P3 の全称（「証跡はこれだけである」）はその副産物だった。パッチ 4 件ではなく構造を直す。

**Files:**
- Modify: `.claude/skills/implement/SKILL.md`（`## Step 1 — 入口判定` 全体と `## 出力` 冒頭）
- Modify: `docs/superpowers/specs/2026-07-26-skill-workflow-boundary-design.md`（§3.1・§3.2 を実装に追随させる）

**Interfaces:**
- Consumes: `/start-issue`「5b. セルフレビュー（plan-review 固有の補完）」が `plan.md` 末尾に「セルフレビュー」節を書くこと、`/start-issue`「Step 6 — workspace をコミット & プッシュ」がその後に来ること
- Produces: 停止経路の名前 `1c-A` / `1c-B` / `1c-C`。`## 出力` はこれを指す

- [ ] **Step 1: `1a` を「由来を 1 回問う」に置き換える**

`.claude/skills/implement/SKILL.md` の `### 1a. workspace/plan.md の同一性確認` 節（見出しから `1b` の直前まで）を、次で置き換える:
...
**この 2 つは別の性質である**——同一性（今のタスクのものか）と、レビュー到達（`/plan-review` を通ったか）。ただし `/start-issue` は「5b. セルフレビュー（plan-review 固有の補完）」の後に「Step 6 — workspace をコミット & プッシュ」が来る順序なので、**コミット済みなら両方が同時に立つ**。
...
- [ ] **Step 2: `1b` を唯一の決定表にする**

`### 1b. 分岐` の見出しと直後の表を、次で置き換える（表の後ろにある「計画なしの経路でだけ行う調査」以降はそのまま残す）:
...
| 別タスクの残骸と分かった | **消さずに**何の残骸かを報告し、削除か退避かをユーザーに確認したうえで、「計画が無い」ものとして下の 2 行のどちらかへ進む |
| 由来の判断がつかない | **止める**（1c-C） |
| 計画が無く、計画を書かずに直せる | 下の調査を行ってから Step 2 へ |
| 計画が無く、計画が要ると判明した | **止める**（1c-A） |
```

- [ ] **Step 3: `1c` に停止の内訳を集約する**

`### 1c. 止まったときの引き渡し` 節の本文（見出しは残す）を、次で置き換える:
...
- **1c-B（計画はあるがレビューを経ていない）**: **新規 issue は起票しない。** 計画が未レビューである旨を報告し、**`/plan-review` を回し、その結果を `plan.md` 末尾の「セルフレビュー」節へ記録してから `/implement` を再実行する**よう促す（`/start-issue` は「Step 2 — main 最新化 & ブランチ作成」でブランチを作る構成のため、途中からの再開口を持たない）
- **1c-C（由来の判断がつかない）**: 何を確かめられなかったかを示して問う。答えを得るまで進まない
```

- [ ] **Step 4: `## 出力` を 1c を指す形にする**

`## 出力` 節の冒頭（現行の「**Step 1 で止まった場合**…」の段落と、その次の「Step 2 以降まで進んだ場合、以下を報告:」の行）を、次で置き換える:

```markdown
**Step 1 で止まった場合**（1c-A / 1c-B / 1c-C）は Step 2 以降に到達しないため、**入口判定の結果と、1c が定める渡し方に沿った報告だけ**を行う。下の 2〜4 は該当経路では生成物が存在しない。

Step 2 以降まで進んだ場合、以下を報告:
```

続く 4 項目の 1 番目を次に直す（同一性だけでなくレビュー到達も根拠に含めるため）:
...
Run: `node scripts/governance-check.mjs`
Expected: PASS。新たに書いた `` `/start-issue`「5b. セルフレビュー（plan-review 固有の補完）」 ``・`` `/start-issue`「Step 6 — workspace をコミット & プッシュ」 ``・`` `/start-issue`「Step 2 — main 最新化 & ブランチ作成」 ``・`` `/start-issue`「出力」 `` がすべて着地すること。落ちたら**参照を直す**（見出しを作りに行かない）。

- [ ] **Step 6: 設計書を実装に追随させる**

`docs/superpowers/specs/2026-07-26-skill-workflow-boundary-design.md` の §3.1（同一性確認）と §3.2（入口判定）の表を、上で確定した「由来 1 問 → 決定表 1 つ」の形へ書き換える。あわせて §3.1 の末尾に 1 行足す:
...
Run: `node scripts/governance-check.mjs && npm test`
Expected: `G1..G11 passed` と `230 passed`。常時ロード面は不変（触るのはスキル本文と spec のみで、どちらも課税面ではない）。

- [ ] **Step 8: コミット**

```bash
git add .claude/skills/implement/SKILL.md docs/superpowers/specs/2026-07-26-skill-workflow-boundary-design.md
```
コミットメッセージは一時ファイルへ Write して `git commit -F <tmpfile>`（HEREDOC を使わない）。件名:
```
refactor(implement): 入口判定を由来 1 問と決定表 1 つに畳む
```
本文に含めること: Minor 3 件が「同じ事実が 2 か所」という 1 つの病だったこと・コミット済みなら同一性とレビュー到達が 1 事実で立つこと・1c-A/B/C の命名で出力が内訳を持たなくなったこと・設計書を追随させたこと。
