...
- **`crateSources` と `moduleIndexSources` が食い違ったとき、計器の行が「消える」ことが手掛かりになる。**
  実ツリーに対してその行を assert するテストは無く、手掛かりが**不在の観測**になっている。
  閉じる案: 実ツリーで `duplicateDomains` がこの対を返すことを 1 件 assert する。**代償は、
  合否を持たない計器を実質の gate に変えてしまうこと**——`excludeTest` のような正当な設定で赤くなる。
  ゆえに「閉じない」を既定に置き、閉じるなら計器ではなく別の検知器を新設する側へ倒す。
