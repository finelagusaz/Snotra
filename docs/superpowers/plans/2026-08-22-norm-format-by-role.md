...
**まずその面が自分の書式を宣言していないか確かめる**（`grep -n '形式\|書き方\|各ルールは' <file> | head -5`）。宣言があれば、それが問い 3 の具体形になる——外から与えた書式の候補より、面の自己宣言のほうが強い根拠である（2026-08-22 の Task 4 で `snotra-settings/CLAUDE.md` が「太字 = 守る指示」と宣言しており、ラベル型のリードがその宣言に既に違反していた）。
...
候補を機械的に見つける:
```bash
grep -nE '^\s*- \*\*' <file> | grep -E '上の|末尾の|次の [0-9]|は [0-9]+ 件|は [0-9]+ つ'
```
