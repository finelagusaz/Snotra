...
**行動形化するのは「リードを読んだだけでは次の動作が決まらない」条項に限る。** リードが既に一意の動作を含意するもの——**禁止形**（「〜しない」「〜してはならない」）・**可能性の否定**（「〜できない」「省略できない」）・**許可形**（「〜しなくてよい」「〜してよい」）——は、語形が事実形に見えても残す。この線を引かないと、**読者の動作が既に一意な条項まで語形だけ入れ替える churn** になり、目的（「次に何をすればよいかが分かる」）に何も足さない（2026-08-22 の `docs/build-commands.md` の実行で確立した規則）。
