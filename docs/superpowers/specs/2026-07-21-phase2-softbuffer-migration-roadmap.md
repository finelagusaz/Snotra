...
- **SU6.5 完了** — flip 前ハードニング（PR 準備中・#643/#648/#652 close）。実装: #643（検索バー font_size 追従 + hint 色・バー 52px は WebView2 parity で据え置き）/ #648 残り 3 点（(A) start_launch wake / (C) InstallFailed dead payload 削除 / (F) mode fallback）/ #652（起動時 hotkey 失敗を egui でも通知——格納→show→wake の 3 段・変更失敗の非 wake と意図的非対称）/ 計測ハーネス measure-memory.ps1 の PrivWS 化 + `npm run measure:memory` / paint 計器 SNOTRA_EGUI_PAINT_TRACE。**実測でゲート合格**: G1 メモリ（egui visible 43.5 < WebView2 105.1 / hidden 0.4→0.9 << 12.0——ただし当初 egui-hidden が trim されず 43.4 のままだった gap を実測が発見し `hide_egui_main` に EmptyWorkingSet trim を配線して解消・Task 7）/ G3(a) hidden 再描画停止（60s trim 保持で実測確認・#628 は flip 非ブロッカー確定）/ G3(b) 入力レスポンス（raster_ms p95 4.68ms << 16.7ms・fill_mesh 最適化不要）/ G2 外観目視（scroll 追従を `scroll_to_me(None)` で WebView2 parity へ・Task 8）+ #643/#652/#648 スモーク全合格。flip 基準 5「WebView2 子孫 0」も egui ツリー件数 1 で実地確認。残 follow-up: 行高/バー高の font_size 連動（#646 設計パス）・updater 詳細 + 入力 text_color parity（#654）
- 残り: `SU7`（→「依存順・並行」）
