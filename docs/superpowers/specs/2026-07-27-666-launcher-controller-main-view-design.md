...
## 2. 決定: `update()` の文の実行順序を 1 行も動かさない
...
このとき **Escape / ↑↓ / →← の「読み」だけが現行の位置（34 段の 13・18・19・20）より前へ寄る**。安全である根拠は 2 つ:

- egui の入力はフレーム内で不変であり、読む順序は消費に影響しない（`view.rs` の Enter 後置ブロックが同じ根拠を明記している）
- **消費（`events.retain` による ↑↓ の除去）は TextEdit より前という制約だけを持つ**（確定事実 1）。前へ寄せても制約は保たれ、寄せた先と現行位置の間にある文（14〜17: focus / Escape / blur）は ↑↓ イベントを読まない
