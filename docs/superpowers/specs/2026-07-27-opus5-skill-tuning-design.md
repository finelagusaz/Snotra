...
## 敵対的読者レビューの結果（2 巡・実施済み）

停止条件どおり 2 巡で打ち切った。各巡で 2 クラス（手を抜く読者・規則を全部守る読者）を独立に起動し、成果物は呼び出し側が指定した絶対パスへ書かせた（4 体とも配送成立）。

- **1 巡目**: 体 A 10 件・体 B 9 件 + 数え上げ不一致 1 件 → 成立した 6 件を修正（`a0e99ba`）。最大の発見は**移送先が元の項目より狭かったこと**——「DRY は Phase 2c、パフォーマンスは Phase 3 が持つ」と書いたが、2c は「関数を起点に外向き」、Phase 3 は「性能に敏感な箇所に触れる場合」の条件付きで、その外側がどのフェーズにも属さなくなっていた
- **2 巡目**: 体 A 11 件・体 B 6 件 + 数え上げ不一致 4 件 → **1 巡目の修正自身が作った 4 件**を修正（`d2d2e05`）。(1) 1 巡目が残余を**閉じた列挙**で書いたため「目録に無いものは宙に浮く」と読め、(2) 2c の母集団要約が 2c 本文より狭かった。(3) `AGENTS.md` 側に残っていた同型の「カテゴリ A〜E」を直し、(4) **`/implement` Step 3 に「カテゴリ D はエージェントが実行できない・該当時は人間へ引き渡す」と、該当判定を拡張子でなくコードパスの意味で行う規定を足した**（(4) は将来の全実装に掛かる新しい義務であり、変更 1〜5 のいずれでもない後付けである）

**横断して見えたパターン（体 A の総括・本サイクル最大の知見）**:

1. **免除句は名指しした対象しか守らない。** 今回の 4 つの免除句はいずれも保護対象を具体的に列挙しており、逃げ道は**列挙の隣**へ移った（`safety-nets.md`「逃げ道は塞ぐたびに隣へ移る」の実例）
2. **義務の係り先が「自分が書く分類・自分が選ぶ形式」になっている箇所が 3 つある。** 書き手が分類を決める限り、義務は書き手が消せる
3. **記録テンプレートの有無が、実施の有無の唯一の観測点になっている。** テンプレートを持たない Phase / Step へ責務を移すと、その責務は沈黙する
...
- **2 巡目で成立したまま残った抜け道**（上限 2 巡は設計で先に決めた停止条件であり、残りを受容することは想定内である）:
  - *今回の較正が隣に作ったもの*: `research.md` の「未解決の疑問」に分類 4 と同型の保護句が無い / 「他**スキル**が規定する委譲」という免除語が `.claude/rules/safety-nets.md` の要求する敵対的読者サブエージェントを含まない / 「各項目 1〜3 行」が grep 結果そのものの記録を不利にし、Step 3 の件数照合が発火しにくくなる / `/retrospective` の免除句が Step 3 だけを名指し Step 4（タスク振り分け）を保護しない / `plan.md` の免除列挙 5 セクションの外に「セルフレビュー」節が落ちる / Step 2b の独立導出に 1〜3 行制約を渡せると読める
  - *今回の変更が作っていない既存の穴*: `/implement`「出力」に Step 4（4a check・4b code-reviewer）の居場所が無く、実施の有無が報告から消える / `/start-issue` 5b の回復経路が ①〜⑤ しか列挙せず、不着レイヤーの SPEC 同期・smoke 前提・設定キー 4 点セットが未検証で通る / `/plan-review` の `allowed-tools` に `Grep` が無く Step 3 の自前 grep が文言上実行不能（インライン起動では実害なし・実測済み） / `/implement`「出力」の「check, clippy, test」3 種がカテゴリ 6 分類を覆わない
- **3 巡目は回していない。** 2 巡目の修正がさらに隣を作っている可能性は残る
- **敵対的読者ループの停止条件は巡数にしか掛かっていない。** 1 件あたりの修正の分量に上限が無いため、各巡の塞ぎは文書を増やす方向にしか働かない——実際に本サイクルでは `code-reviewer` Phase 1 が 2 巡の塞ぎで元の 3.4 倍まで膨らみ、最終レビューの指摘で圧縮し直した。「塞ぎは 1 文まで。超えるなら受容する残余へ回す」といった予算を停止条件に加えるべきだが、`.claude/rules/safety-nets.md` は本サイクルの範囲外のため、次サイクルの教訓として送る
