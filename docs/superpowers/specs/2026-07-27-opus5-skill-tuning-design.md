...
- **2 巡目**: 体 A 11 件・体 B 6 件 + 数え上げ不一致 4 件 → **1 巡目の修正自身が作った 4 件**を修正（`d2d2e05`）。1 巡目の修正が残余を**閉じた列挙**で書いたため「目録に無いものは宙に浮く」と読め、かつ 2c の母集団要約が 2c 本文より狭かった。併せて `AGENTS.md` 側に残っていた同型の「カテゴリ A〜E」も直した
