...
### 決定 2: show 経路は位置をバー高で決め、実高は位置決定後の 2 回目の `set_size` で適用する

`show_egui_main` の `#[cfg(windows)]` ブロックは、`set_size(バー高)` → 位置決め → `set_size(実高)` → `show()` の順にする。**位置決定に使う 1 回目の `set_size` は不変（バー高固定のまま）**——バーの位置はユーザーが決めるものであり、status 行・toast 行の出没（クランプでも、可視中の伸びでも）で動かしてはならない（確定仕様・2026-08-04）。実高は毎フレーム側と同じ述語（`status_row_present`）・同じ積算式（`main_window_height`）で導出し、位置決定の後・`show()` の前に 2 回目の `set_size` として適用する。これにより最初のフレームが描く高さと畳んだ高さの食い違いが show の内部で解消し、show 後に窓が伸びる（#755）・縮んでから伸びる（#801）が消える。**reset-on-show への依存は 3 つのリテラル引数として呼び出し点に現れる**:
...
**位置クランプが読み戻すサイズは 1 回目の `set_size`（バー高）である**——2 回目の `set_size`（実高）はクランプの対象外で、位置決定より後にだけ効く。
...
| `src-tauri/CLAUDE.md`「実装パターン」 | 「show の操作順序制約」。位置決定に使う畳み込み先はバー高のまま不変で、位置決定後に実高への 2 回目の `set_size` が加わる |
| `view.rs` のデルタガード近傍のコメント | 「意図的な 2 導出」の説明 |

**`ADR-results-presentation-two-stage` は編集しない**（`ADR-adr-frozen-history`: ADR は凍結された歴史）。却下 6 の反転は新しい ADR（`docs/adr/ADR-show-path-derives-drawn-height.md`）に記録する。そこに書く否定の知識は本 spec の §2 と、**却下 6 が依存していた前提のうち反転した部分**である——旧 ADR は「毎フレームの導出をそのまま使えば結果件数で伸びた高さでクランプする」と読んで、show 経路と毎フレームの高さ導出の統合そのものを禁じた。実際に共有できるのは `main_window_height`（バー + status + toast のみで積算し、結果件数には伸びない）という**積算式**であり、これを show 経路の 2 回目の `set_size`（位置決定後・実高の適用）に使うことは安全である。**反転したのは「導出式を共有できない」という前提だけであり、「位置クランプの対象を実高にできる」という意味ではない**——位置クランプはバー高のまま据え置く（確定仕様・2026-08-04）。同じ ADR の却下 3 が残した教訓（禁止を恒久文書へ書く前に、その禁止が依存している前提を明示できるか確かめよ）の適用例である。
...
- **位置クランプの対象は変わらない（バー高のまま）。位置の意味論は不変である**——バーの位置は行の出没では動かない（show 時のクランプでも、可視中の伸びでも）。**#738 とは無関係**である——本 spec は show 経路が導出する高さの値を変えるだけで、位置クランプが対象にする高さそのものは変えない
