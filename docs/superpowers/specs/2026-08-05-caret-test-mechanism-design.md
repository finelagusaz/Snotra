...
### 実装で判明した規律: **故障注入は、本来の回帰より強い変異にしてはならない**

このサイクルで**同じ型の誤りを 3 回**踏んだ。いずれも「注入したのに落ちなかった」または「落ちたが理由が違った」形である。

| 回 | 変異 | なぜ測れていなかったか |
|---|---|---|
| 1 | `move_text_cursor_to_end` を後ろへ移す | focus 直後のキャレットが既に末尾で、その呼び出しが no-op だった（＝検査は並びを縛れていなかった） |
| 2 | 実機配管で focus 要求の条件を `false` にする | **5 フレームとも偽**になる。本来の回帰は「frame 1 だけ偽」であり、`Wait-SnotraTraceCondition` は一致の**最後**を返すので、本来の回帰なら検査は PASS していた |
| 3 | kittest で focus 要求を後ろへ移す | 判定フレームより前に `Harness` が既にフレームを走らせており、focus 要求が `!has_focus` ガードで**そもそも走らない**状態で測っていた |

**共通の機序**: 変異が本来の故障より強い／状態が本来と違うと、検査が縛れていなくても赤くなる。**注入が赤くなったことは、検査が当の回帰を捕まえる証拠にならない。** 変異は「実際に起きた回帰の姿」に合わせること——#938 の姿は `view.rs` の実測コメント（frame 1 が偽・frame 2 から真）が正本である。
