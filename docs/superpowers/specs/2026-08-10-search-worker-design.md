...
**(1) trace の書き込みがフレーム時間の計器を汚染する。** `update_us` は「フレームの所要」ではなく**「そのフレームで trace を何本吐いたか」**を強く反映する。計器が計器自身を測っている状態であり、絶対値での合否判定はこの形では成立しない。**実測値と一般則は `PERFORMANCE.md`「計測と受け入れ基準」が正本**（#1034 で横断的な正本へ吸収した）。

**(2) smoke は常に 1 件の索引を seed し、実運用点では走らせられない。** ゆえに「実運用点で smoke を走らせて赤→緑を見る」段取りは実行不可能であり、索引件数のゲートを置けば **H6 はどこでも永久に SKIP** になる——#930 が戒めた「発火しえない検出器」そのものになる。**smoke の索引規模が検知器の設計に課す制約は `docs/build-commands.md`「スモーク運用メモ」が正本**（#1034）。

**ゆえに受け入れ 2 は、実運用点（312k・`[[paths.scan]] path = 'C:\'`）での A/B 実測で示す。** 比べるのは打鍵直後のフレームの `update_us` であり、**A 側では検索の 20 ms がそこに含まれ、B 側では含まれない**。trace の汚染は A/B の両側へ等しく乗るので、実運用点では検索の 20 ms が汚染を上回って差として現れる。
