...
リフレッシュレートは `snotra-egui-runtime/src/monitor.rs` の `monitor_refresh_hz` が取得している値を使う。**固定の 16.7 ms を書き込まない**——高リフレッシュレート環境で緩すぎる判定になる。

### 3.4 H6 は索引の規模に依存する — 走る場所で発火しうるかを測る

**20 ms は 312,377 件の索引で出た数字である。** CI runner の索引は実運用点と規模が違うため、そこでは走査が速く、**A 側でも H6 が緑になりうる**。それは「直った」ではなく「測れていない」であり、#930 が「検出器を新設する前に、走る場所で発火しうるかを測る」と記録した型そのものである。

ゆえに H6 は**索引件数のゲートを持つ**: 件数が閾値を下回る環境では判定せず、**SKIP を明示して出す**（沈黙で通さない）。閾値と、CI・実機それぞれの実際の件数は PR 1 で実測して決める。**A 側ベースラインの採取は実運用点（312k・`[[paths.scan]] path = 'C:\'`）で行う。**
