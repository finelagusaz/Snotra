...
`index_tree.rs` と `query.rs` のコメントは木表現と派生文字列共有の根拠を説明する散文であり、名前を導出する実装ではない。同じ `query.rs` の `lower_file_name` は `file_name()` を呼ぶ実コードだが、小文字化と accent-folding を施した検索キーを作るもので、原文のまま `AppEntry.name` へ入る名前は導出しない。`folder.rs` のフォルダ内列挙は拡張子付きの名前を表示・フィルタリングへ使う別概念である。
...
issue #997 が同型の先例として挙げた `#437` は、この判断の反例ではない。`#437` が集約したのは `search.rs`（Wave 1）と `indexer.rs` にまたがる同一変換の写しであり、消費者が**別モジュール**にあったからこそ 1 つの関数を通す価値があった（由来は `query.rs::lower_file_name` の doc が持つ）。本件で残る導出は同一ファイル内にあり、各々が呼び出し側固有の除外処理に隣接する。両者を分ける線は §6 の再検討条件そのものである。
