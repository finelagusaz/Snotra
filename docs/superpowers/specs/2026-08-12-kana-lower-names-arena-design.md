...
**併合のずれには専用の検知器を置く。** 当初は「既存の A/B 突き合わせ（`compute_wave1` の
逐次と `kana_for_cached` の並列を比べる）が捕まえる」と書いたが、**それは成立しない**——
あちらが migemo=true で通す fixture は数件ゆえ塊が常に 1 つで、塊の順序を反転する変異を
当てても全テストが緑のまま通る（2026-08-12 にレビューで実測。対照として `from_chunks` が
空を返す変異は赤くなるので、経路に到達していないのではなく 2 つ目の塊を見ていない）。

ゆえに `KANA_CHUNK` を跨ぐ規模の fixture を 1 本置く
