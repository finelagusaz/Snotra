 ...
 * へのポインタ 1 行を safety-nets.md へ足したため、まず 8056 → 8188（+132 字）へ引き上げた。
 * ADR-0005 が警告する反射的な引き上げに当たらない根拠は、引き上げ幅がその 1 行の実測値
 * ちょうどであること——本文は 1 文字も増えていない。その後、骨格の正本を
 * `docs/superpowers/specs/` から `docs/` 直下へ移設しポインタ行のパス表記が 29 字縮んだため
 * 8188 → 8159 へ引き下げた。骨格そのものは面積対象外の文書に置き、rules へは到達経路だけを
 * 置いた（ADR-0011 却下 4）。
 */
