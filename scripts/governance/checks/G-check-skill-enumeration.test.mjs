...
describe("G-check-skill-enumeration checkCheckSkillEnumeration（3a の列挙 ↔ AGENTS.md 表・#778）", () => {
  // 守りたい対象 = 表へ check スキルを足した人が /implement 3a を直さず、
  // 新しいスキルが報告母集団から沈黙して落ちる形。
  const mk = (tableSkills, step3aSkills, files = []) =>
    snap(
      {
        "AGENTS.md": `## 条件別チェック（トリガー → 参照先）\n\n| t | ${tableSkills.join(" ")} |\n\n## 次節\n`,
        ".claude/skills/implement/SKILL.md": `## Step 4\n\n### 3a. 委譲の前に\n\n変更に応じて ${step3aSkills.join("・")} を実行する。\n\n### 3b. x\n`,
      },
      files,
    );
  ...
  it("赤: 表に在って 3a に無い（報告母集団から沈黙して落ちる）", () => {
    const f = checkCheckSkillEnumeration(mk(["`/dry-check`", "`/race-check`"], ["`/dry-check`"], SKILLS));
    expect(f.some((x) => x.message.includes("/race-check") && x.message.includes("3a の列挙に無い"))).toBe(true);
  });

  it("赤: 3a に在って表に無い（起動条件を持たない検査）", () => {
  ...
  it("判定対象外の不混入: `-check` で終わらないスキルは母集団に入らない", () => {
    // 表にだけ /plan-review が在っても、3a に無いことを咎めない
  ...
  it("赤: 見出しが変わって節を切り出せない（沈黙で通さない）", () => {
    const s = snap({ "AGENTS.md": "## 別の見出し\n", ".claude/skills/implement/SKILL.md": "### 3a. x\n`/dry-check`\n" });
