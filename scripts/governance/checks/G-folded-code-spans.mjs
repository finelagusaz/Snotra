...
//   - **母集団から外れる `.md`**。凍結された歴史（`docs/adr/` ・`docs/superpowers/`）を
//     `allHeadingRefDocs` が落とすため、**`.md` は大半が入らない**（`.rs` とコメント記法
//     スクリプトは全数入る）。**枚数も除外の一覧もここへ書かない**——`allHeadingRefDocs` が正本で、
//     腕が増えれば写しだけが腐る（`workspace/` はそれ以前に `makeSnapshot` が歩かない）
...
// **赤側の死角は今日どれも逃げ道を要さない**（スパンを割るか二重バッククォートで書ける）。
// ただし `.rs` の doctest でバッククォート自体を見せる例は、フェンス内なので二重化で逃げられない
// ——今日 0 件で、出たらこの一文ごと見直すこと。
