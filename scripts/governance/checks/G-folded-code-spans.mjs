...
//
// **死角は規範の免除ではない。** `docs/comment-guidelines.md`「日本語の折返し」の 2 形は
// 機構の射程と無関係に全ツリーへ掛かる——下に並べるのは「赤で気づける範囲の外」であって、
// 「折ってよい場所」ではない。そこを守るのは人である。
//
// **宣言する死角——ここが見ないもの（沈黙側）:**
//   - **行末のインラインコメント**（`let x = 5; // 説明`）。`linesOfComments` が行頭で判定するため、
//     **折れの両端が行末コメントに在ると 0 件で素通りする**（2026-08-22 実測）
//   - **母集団から外れる `.md`**。`allHeadingRefDocs` が `docs/adr/` と `docs/superpowers/` を
//     落とすため、**`.md` は大半が入らない**（`.rs` とコメント記法スクリプトは今日すべて入る）。
//     **枚数も除外の一覧もここへ書かない**——`allHeadingRefDocs` が正本で、腕が増えれば写しだけが
//     腐る（`workspace/` はそれ以前に `makeSnapshot` が歩かない）
//   - **`.md` のフェンス内**（`linesOutsideFences` が落とす）。フェンスは折返しの対象外である
//   - **多重バッククォートで開いたスパンの折れ**（`` `` `` で開いて次行の `` `` `` で閉じる形）。
//     追跡するのは単独のバッククォートだけで、2 連以上は `stripRuns` が落とす。**`checked` も 0 に
//     なるので、証跡の上でも「見た」と主張しない**。射程を広げる版を実データへ当てたら
//     **7 行すべてが誤検出**だった（フェンスの記号と、それを説明する散文）ので広げていない。
//     今日の実ツリーに実例は 0 件
//
...
//
// **述語は変えずに死角を宣言する側へ倒した**——母集団の外にある `.md`（この検査が一度も直して
// いないコーパス）へ当てて findings 0 件だったので、実データでの誤検出率は測れる限り 0 である。
// **赤側の死角には逃げ道が要る。今日は足りている**——スパンを割るか二重バッククォートで書ける。
// 逃げ道が無い形を 1 つ知っている: `.rs` の doctest でバッククォート自体を見せる例は、フェンス内
// ゆえ二重化できない。今日 0 件で、出たらこの一文ごと見直すこと。
 ...
 *
 * **この検査が追跡するのは単独のバッククォートで開いたスパンだけである**（下の死角宣言を参照）。
 * ゆえにここは「2 連以上を消す」だけでよく、**長さの一致は見ない**。長さを見る版を書いて実データへ
 * 当てたところ、多重スパンを追跡対象に広げると**実ツリーの 7 行すべてが誤検出になった**
 * （フェンスの開始/終了記号と、フェンス記号を説明する散文。2026-08-22 実測）ので、
 * **射程を広げず宣言する側へ倒した**。
 */
