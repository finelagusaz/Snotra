...
//
// **宣言する死角——ここが見ないもの:**
//   - **バッククォート自体が行を跨ぐ形**（対象の綴りが 2 行に割れる）。**#992 から `G-folded-code-spans`
//     がこの形を赤にする**——かつては規範が先に効いているだけだったが、規範だけでは足りず
//     6 箇所が実在していた（`docs/comment-guidelines.md`「日本語の折返し」）
...
// （**綴りをここへ列挙しない**——#1155 で `.mjs` が加わったとき、列挙した側だけが古びる）。
// ---------------------------------------------------------------------------
 ...
 *  **判定の中核であって装飾ではない**——恒等関数へ変異させると形 A が 17 件全滅した
