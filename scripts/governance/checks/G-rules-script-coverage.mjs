...
//! - `SCRIPT_EXT` を `.mjs` へ狭める → `governance:check` の**実行時**に鳴るのは錨である。錨の無い版は
//!   exit 0 で完全に沈黙し、錨のある版は exit 1（finding は 2 件とも錨）。**`npm test` では上の
//!   「母集団の下界」が別に赤くする**ので、「錨が唯一の検知点」ではない——層が違う
