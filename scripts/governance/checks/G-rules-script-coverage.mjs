...
//!   今日これを縛るのは `npm test` の側である（上に挙げた検知器がその経路を持つ）
//! - `WALK_EXCLUDE_PATHS` でディレクトリを落とす → **今日も赤い**。落ちたディレクトリが文書から
//!   逐語で名指されているため、**そこを走査する検査が鳴る**——どれが鳴るかは落とした先に依るので
//!   ここで検査名を挙げない。**ただし失われたものがある**——錨が担っていたのは検出ではなく
//!   **帰属**（原因を「母集団が縮んだ」と名指すこと）であり、そちらは撤去で消えた。
//!   **今日この赤を見た者は「文書が壊れた」と読む**
//!
...
//!
//! **この A/B から「走査の狭窄はどれかの層が捕まえる」と一般化してはならない**——捕まる理由も、
//! どの層が鳴るかも形ごとに違う。上の 2 つがその実例である（片方は `governance:check` で沈黙し、
//! 片方は赤くなるが原因を名指さない）。
