...
//
// **外部ツールの語彙は空白の規則が外す**: コマンドを書いた span（`gh pr view <PR> --json closingIssuesReferences` 等）は
// 空白を含むので、そもそも判定対象にならない。かつては行に
...
//   **かつてここには「測定の全セルで 0 件」と書いてあったが、それは穴について測った結果ではなかった**
//   ——#885 当時の述語は lowercase snake_case を見ておらず（#989 で足した）、上で偽陽性になる識別子は
//   すべてその字種である。0 は述語の構成が保証していたのであり、述語が広がった時点で潜在的に動いていた。
//   穴のせいで見逃された真の腐りは今日も 0 件のまま、塞ぐ側の代償だけが育っている
