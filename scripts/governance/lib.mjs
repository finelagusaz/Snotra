 ...
 *    - 深さ 2 以上の入れ子
 *    - **入れ子かつ物理改行で折れた形**。**`G-folded-heading-refs` も拾わない**——同検査の
 *      `OPEN_UNCLOSED` は入れ子の `「` で切れる（実測。入れ子を含まない同じ折れ方は拾う）。
 *      **どちらの検査の宣言にも無かった穴であり、ここが唯一の記載である**
 *    - ラベルが閉じない形
 ...
 *      外側を弾くので `checked` にも `findings` にも現れない。**旧実装では内側が独立に照合されていた
 *      形であり、この変更が作った唯一の観測性の後退である**（今日 0 件。ただし無効綴りの頭そのものは
 *      生きた母集団に 15 件在るので、内側に参照を書けばそこが沈黙する）
 *
