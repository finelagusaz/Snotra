 ...
 *  内側を独立に照合しており、外側が有効なら `findings` が 1 → 0、外側が無効なら `checked` ごと
 *  1 → 0 になる（実測）。**入れ子を含まない参照は影響を受けない**（対照を測って確認）。
