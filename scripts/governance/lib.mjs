 ...
 *
 *  **`g` フラグを持つので `matchAll` からだけ使う。** `matchAll` の複製は `lastIndex` を**引き継ぐ**が
 ...
 *  `findings` にも現れないので「照合していない」と「差分ゼロ」が区別できない。18 日間・独立 2 エピソードの
 *  沈黙が実在した（うち 1 件は製品コードの rustdoc）。決定と却下した代替案は
 *  ADR-nested-quote-heading-ref-labels が持つ。
 *
 *  **宣言する死角**（今日いずれも 0 件・**照合母集団**で実測。走査されない層まで広げると
 *  「ラベルが閉じない形」は 0 件ではない）:
 *    - 深さ 2 以上の入れ子
 *    - **入れ子かつ物理改行で折れた形のうち、`「` まで開いてラベル本文が次行へ流れる形**。
 *      `G-folded-heading-refs` の `OPEN_UNCLOSED` が入れ子の `「` で切れるので、そちらも拾わない。
 *      **行末が対象綴りで終わる折れ方の側は、入れ子の有無が判定に影響しない**——`TAIL_TARGET` は
 *      ラベルを見ないため（実測）。そちらが拾うかどうかは同検査が自分で宣言する死角の内側にある。
 *    - ラベルが閉じない形
 *    - **他の参照のラベルの内側に置かれた正準形参照**——外側が**有効な対象綴り**なら
 *      外側 1 件へ統合され、内側は独立に照合されない。外側が閉じなければ内側が独立に照合される（安全側）
 *    - **外側の対象綴りが無効なときは、内側の正当な参照ごと沈黙する**——`isRefTargetSpelling` が
 *      外側を弾くので `checked` にも `findings` にも現れない
 *
 *  **上の 2 項（内側を持つ形）はどちらも観測性の後退である。「1 つだけ」ではない**——旧実装は
 *  内側を独立に照合しており、外側が有効なら**内側の `findings` がすべて消え**、外側が無効なら
 *  `checked` ごと消える（実測）。**影響を決めるのは参照が入れ子を含むかではなく、置かれた行が
 *  入れ子の構造を作るかである**——入れ子の構造を持たない行の参照は影響を受けない（対照で確認）。
 *  露出は今日 0 件だが、無効綴りの頭を持つ一致は生きた母集団に現に在るので、
 *  「起こりえない」ではなく「まだ書かれていない」である。
 *
 *  **線が動いたことに注意**（#489）。「入れ子は照合されない」から「**1 段までは照合される**」へ変わり、
 *  深さ 2 を書いた者はより強く動くと期待する。ゆえに死角は数えず**宣言する**（`isRefTargetSpelling` と同じ形）。
 *
 *  **ここへ書く例が検出器を赤にしうる**——このファイルのコメントは照合母集団に入る（#1155 と同じ罠）。 */
