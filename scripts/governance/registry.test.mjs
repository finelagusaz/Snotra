...
// **射程の限界**: (1) この列自身を書き換えれば通る。機構が保証するのは「その 1 行が diff に現れる」
// ことであって、レビュアが見落とさないことではない。(2) **検知はここ（`npm test`）にしか無い**
// ——移行済みの検査を `"unmigrated"` へ戻しても `governance:check` は exit 0 のままで、evidence の
// 「ドメイン未移行 N 本」が 1 増えるだけである（2026-08-20 実測）。
//
