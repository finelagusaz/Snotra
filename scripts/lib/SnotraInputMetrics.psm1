...
.DESCRIPTION
  ...
  これは構造的な残余であって欠陥ではない（#1045）。

  **偶奇を決めるのはフォントだけではない。表示倍率も決める。** 同じ本体・同じ 8 フォントで、
  DPI 96（100%）では `InnerHeight` が 27（奇）で 4 種がずれ、DPI 120（125%）では 32（偶）で
  キャレットが 8 種とも偶数になり **8 種すべて `Skew` 0** になった（2026-08-16 実測）。
  ゆえに:

  - **この表の値は、実効 DPI が同じ測定どうしでしか突き合わせられない。** 倍率が違えば、
    egui を 1 行も変えていなくても全フォントの値が動く（`visual-input-metrics.ps1` が
    `Dpi` を列に持つのはこのためである）
  - **全種が対称に出たことは「#1045 の非対称が直った」を意味しない。** その倍率で出ない、
    というだけである。逆に、出る倍率では今も出る
#>
