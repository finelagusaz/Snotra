...
- **`event` と `ok` / `reason` の整合** — イベント名が意味を運ぶ設計
  ...
  **この検査が見ないもの: `outcome` そのものの誤り。** `event` と `ok` は `startup.rs` の
  ...
  現れうる（食い違う経路をその run が踏んだときに限る。`smoke.yml` が踏むのは成功経路だけである）。**その退行のうち、組の名前を `_` で明示的に捨てる形はここにしか届かない**（名前を
  捨て忘れた形は未使用変数として clippy の `-D warnings` が止める・#1026 で注入して実測。
  `finish` は一度きりで出力が trace だけなので、単体テストからは呼んでも観測できない）。
