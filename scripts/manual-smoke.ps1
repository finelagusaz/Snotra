...
後半（下端ぎりぎり）: **1 行 + padding ぶんのはみ出しは仕様**（SPEC.md §8「作業領域の残りが 1 行に満たない場合でも 1 行分（+ padding）は表示し、そのぶんのはみ出しは受容する」・#675）。
  → **それを超えて大きく沈むなら FAIL**。そのとき main 窓自身が作業領域の下端を超えていないかも見る。**バー（帯）そのものが下端を超えていたら FAIL**——#738 で「ポインタを離した時点でバー矩形を作業領域内へ戻す」を入れた（SPEC.md §8.2「表示中の作業領域への復帰」）。ただし status 行・toast 行が出て**伸びた分**のはみ出しは仕様である（§4.7・バーの位置は行の出没で動かさない）
"@
     trace = @() }
