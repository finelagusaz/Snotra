...
try {
    ...
    # 索引ロードと起動段の初期化が落ち着くまで待つ。**この 14 秒は背景再スキャン（#1001 で
    # 撤去済み）が走っていた頃の見積もりであり、今は余裕が過大である。** 縮めると段階 A が
    # 測る対象そのものが変わる（まだ動いている初期化を含む値になる）ので、**縮めるなら
    # 新しい値を実測してから**——過去の A/B とも比較できなくなる。
