...
**⚠️ この 2 つの env は #872 の機序再設計で読み手を失った。** `SNOTRA_PESTER_TRACE_DIR` の
写しと `SNOTRA_PESTER_FAILURE_GRACE_MS` の猶予は、どちらも `SnotraSmoke.Tests.ps1` の
キャレット統合検査が読んでいたが、その検査は `egui_input:focus_state` の観測 1 点へ縮み、
待ちが 1 つになって「遅着と喪失を分ける」問いごと消えた。**いま立てても何も起きない**
——この一式の撤去（下の .NOTES）までの間、記述だけが残っている。

**失敗時の猶予**（`-FailureGraceMs`）は、待ちが不成立で終わった検査に本体を即座に kill
させず、指定 ms だけ待ってから trace を読み直させる仕掛けだった。**「予算後の遅着」と
「恒久的な喪失」を分ける唯一の実験**であり（#936 受け入れ条件 1）、その問いには #938 が
答えを出した（喪失は実バグで、原因側を直した）。**上の ⚠️ のとおり、読み手はもう居ない。**
