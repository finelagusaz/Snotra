...
# 最小の有効 TOML。**config.toml を置くこと自体が要る**——ファイルが無いと first-run になり
# 既定値で起動して背景が `CLEAR_COLOR` になり、「色が届いていない」と誤読される。#824 で
# `Config` の全セクションに `#[serde(default)]` が付いたため、欠落セクションが parse を落として
# 破損復旧経路（`.bak` 退避 + 既定値起動）を踏むことはもう無い。ここで値を書く理由は
# `[visual]` の色（これが検証対象そのものである）、下で理由を説明する
# `auto_hide_on_focus_lost` と `[[paths.scan]]`、そして `-Interactive` から導く
# `show_on_startup` である。
