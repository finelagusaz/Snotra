...
- `HistoryBoostConfig`（`search.rs`）: 名称は「履歴ブースト設定」だが migemo フィールドも含まれている。
  検索コール全体の設定を渡す構造体として `SearchCallConfig` / `SearchOptions` へのリネームを検討すること。
  シグネチャ変更コストが低いタイミング（`search_with_history_boost` の大規模リファクタ時など）に対応する。
