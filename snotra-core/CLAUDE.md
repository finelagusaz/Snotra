...
- `search.rs` — 検索順位計算・履歴ブースト・incremental search キャッシュ・空クエリ時履歴候補（責務・スコア階層は `//!` と `SearchEngine` の struct doc）。以下は並列 Vec レイアウトの不変条件:
  ...
  - **重複する派生文字列は索引に持たず、鎖で共有する**（反復 4・5）: `lower_file_names[i]` → `lower_names[i]` → `entries[i].name` の順に、`assemble` が構築時に**測って**上流と一致するものを `None` へ潰す（実測 9.71 + 9.80 MiB / 526,316 ブロック）。`lower_names` の `None` は「`name` と同一」、`lower_file_names` の共有は `CompactEntry.file_name_is_lower_name`（空きパディングゆえ 32 B のまま）で表す——**`lower_file_names` の `None` には「file name 成分が無い」という先客がいる**ため、そちらだけ旗を要した。読み替えは `SearchEngine::entry_view` の 1 点で、同じ順に解決する
    ...
    - **記録側が潰したものを、cache-miss の枝がそのまま索引の表現に使う**（反復 11）。`save_cache_sorted_in` は書いた 4 本を `CachedMasks` として返し、`load_or_scan_with_stats` がそれを `LoadOrScanResult.cached_masks` へ載せる——かつては**計算して書いた直後に捨て、`new_from_tree` が全件を実体化してから同じものを作り直していた**（実測 構築段 allocs 2,801,461 → 85,391・peak 92.81 → 9.54 MiB・壁時計 539 → 24 ms）。**ゆえに `cache_changed` はどのコンストラクタが選ばれるかを決めない**——決めるのは `cached_masks` の有無と `CachedLower` の variant だけである。両経路が同じ表現へ着地することを **CI で守るのは `search/tests/build.rs` の合成 fixture 1 本だけである**（`save_side_collapse_and_assemble_measurement_agree_at_entry_view`）——実データ全件を突き合わせる相方は `#[ignore]` かつ scan パスが空なら自己スキップするので、**規模と実際のパスの形（深い鎖・根と非根の混在・非 ASCII・大文字の拡張子）は手元で明示的に走らせたときしか検証されない**。実データ側は**原文をファイルシステム走査から取る**（`index.bin` から取ると組み直し対組み直しの不動点になる）。cache-miss の直後に PATH エントリを併合する経路は `path_merge_after_cache_miss_agrees_with_deriving_over_the_extended_tree` が別に守る
