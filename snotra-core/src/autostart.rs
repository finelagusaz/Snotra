...
//!   `Explorer\StartupApproved\Run` へ無効マークを置く（2026-09-01 実測: 先頭バイトが
//!   `02` → `03` になり `Run` 値は残った）。[`is_enabled`] の意味論は「`Run` 値が存在する」で
//!   あって「実効的に有効」ではない。**追随させないのは読めないからではなく、無効化が
//!   タスクマネージャーで利用者が明示した意思であり、そこを設定アプリが読み替えるもの
//!   ではないからである**
