...
impl Engine {
    ...
    /// 設定の共有ハンドルを渡す（#1032）。
    ///
    /// **UI が毎フレーム行う live-read を、外側の `Mutex<Engine>` の外へ出すための口である。** 検索の worker は `search` の間じゅう外側の `Mutex` を握るため、UI が同じ錠越しに設定を読むと、そのフレームは worker の走査が終わるまで返らない（`read_window_width` の読み max の実測値は `PERFORMANCE.md`「設定の読みを engine lock の外へ出す」）。
    ///
    /// **実運用点での保持時間は 40〜95 ms である**（#1032 実測）。**この値の正本はここであり、他の箇所はここを指す**——`PERFORMANCE.md` に記録の在る `read_window_width` の読み max と違って、この値は #1032 の計装（撤去済み）でしか測られておらず、寄せる先が他に無い。
    ///
