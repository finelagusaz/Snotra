...
//! # 辿る規則は 1 つ
//!
//! **記憶域の並べ方は 2 通りあるが、辿る規則そのものを 2 回書いてはならない。** [`TreeNodes`] が
//! 「i 番の親・aux・名前をどう取るか」だけを抽象化し、[`walk_to_root`] と [`raw_path_into`] は
//! 両方の並べ方から同じ 1 つの実装が使われる（単型化されるので索引側の速さは落ちない）。
//! **並べ方が 2 通りある理由は [`IndexTree`] の doc が、表示名だけ 1 通りである理由は
//! [`NameArena`] の doc が持つ。**
//!
...
/// を占め、ロード段の確保の 76%・常駐ブロックの 76% がここだった（実測は `PERFORMANCE.md`「採用: 表示名を文字列アリーナで持つ」）。バイト数はほぼ同じでも、**ブロック数はアロケータ
/// 由来の税を決める**——反復 4・5 が観測した
/// `PERFORMANCE.md`「共有 1 反復あたり構築 +17 ms」の傾きは確保・
/// 解放の回数に比例していた。
///
/// **表示名だけは並べ方が 1 通りである**——連結バイト列 + オフセットの同じ物体を、ディスクも
/// 索引も持つ（`PathStore::adopt` は写さずに move する）。**ただしディスクを通らない消費者が
/// 1 つある**——`SearchEngine::kana_lower_names`（migemo 有効時のみ）は `index.bin` に保存せず
/// [`Serialize`] / [`Deserialize`] を通らないので、線上表現の制約もかからない。
/// **共有しているのは表現の核だけである。**
///
...
impl NameArena {
    ...
    /// `SearchEngine::new_with_cached_masks` は `to_kana` を全件へ当てるので、逐次で回すと
    /// 秒オーダーが起動に乗る（額は `PERFORMANCE.md`「採用: `kana_lower_names` も文字列アリーナで持つ」が正本。**ここへ写さない**——計器を回すたびに腐る）。
    /// 塊の中だけ並列に組み、ここで繋ぐ。
    ///
    ...
    /// （読み口を差し替える変異 2 つで判定した。表は `PERFORMANCE.md`「採用: 表示名を文字列アリーナで持つ」）。**緩和は削減と排他である**——名前を複製して持てば戻るが、
...
///
/// **並べ方が索引側と違うのは意図的である。** ここは並列 Vec（SoA）、索引側の `PathStore` は
/// 1 件 12 B の構造体の列（AoS）で持つ——索引は木を辿る段で `parent` と `aux` を同じ段で読むため
/// 同じキャッシュラインに載るほうがミスが少なく、ディスクの形は並列 Vec のほうが小さい
/// （`Vec<u32>` は postcard の varint で詰まる）。辿る規則を 1 つに保つ仕掛けは [`TreeNodes`]。
///
...
impl IndexTree {
    ...
    pub(crate) fn extend_with_roots(&mut self, entries: Vec<AppEntry>) {
        ...
        //
        // **範囲を伸ばそうと測り直しても空振りする**——実運用点では PATH エントリが必ず途中に
        // 入る（2026-08-13 実測）。却下の論証は `docs/adr/ADR-sorted-prefix-over-reflagging.md`。
    }
...
mod tests {
    ...
    /// - 深さが `CHAIN_CAP` 以上 → `chain` 配列の範囲外
    ///
    /// （素のバッククォートで書いてあるのは `#[cfg(test)]` の中だからである——リンク形にしても
    /// 着地は誰も確かめない。面ごとに何が確かめられるかは
    /// `docs/comment-guidelines.md`「名指しと正本の指名」）
    ///
