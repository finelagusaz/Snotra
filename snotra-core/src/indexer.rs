...
pub struct LoadOrScanStats {
    ...
    /// `index_load_unattributed_ms` として出す。**その引き算の両辺が同じ関数（`to_ms`）を
    /// 通るように、ここでは丸めずに渡す**（#1027）。丸めた値を入れると、外側と丸め方が
    /// 食い違ったときに差が黙って負へ振れる——**それを止める検査はテストには無く、
    /// `bench-startup.ps1` の `>= 0` だけである**（生成側を丸める変異で全検査が緑のまま
    /// 通ることを実測した）。
    ///
    /// **これは「この値がミリ秒へ落ちる場所は 1 か所だ」という主張ではない。**
    /// 表示のために `as_millis()` する消費者は他にもある（`main.rs` の
    /// `#[cfg(debug_assertions)]` の `eprintln!` と `tests/memory_footprint.rs` の 2 か所）。
