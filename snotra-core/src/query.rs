...
/// **`contains(['\\', '/', '¥'])` と 1 つに束ねないこと。** 複数 `char` のパターンは
/// 1 文字ずつ UTF-8 を復号して 3 通り比べるので、索引全件（実運用点で約 11 MiB）へ当てると
/// **構築が 2 → 26 ms へ伸びる**（実測）。単一 `char` の [`str::contains`] は memchr へ落ちるため、
/// **3 パスに分けるほうが速い**——束ねたくなる形が遅いほうである。
#[inline]
