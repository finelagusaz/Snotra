...
// アイコン剪定の 5 形（現行 / 正規化の篩 / 原文の篩 / FxHash / 篩+FxHash）を実 `index.bin` で並べて測る計器は、反復 12 で候補の採否が決まった時点で撤去した——**それがその計器の doc に書いた撤去条件である**。
//
// 数字・判定・再び測る値打ちが出る条件は、すべて `PERFORMANCE.md`「試みたが機能しない: アイコン剪定を篩へ通す」を正本とする（写しを置くと、条件が変わった日にこちらだけが古いまま残る）。
//
// **上の正準形の参照を折り返さないこと。** 折り返すと `governance:check` の照合対象から消え、参照先を改名しても緑のまま通る（2026-08-09 実測: 1 行へ直したら照合件数が 183 → 184 へ増えた）。**機序は書かない**——検査の述語を散文へ写すと、検査側が変わった日にここだけが偽になる（`.claude/rules/governance-docs.md`）。折返しが機械検査を壊すことの正本は `docs/comment-guidelines.md`「日本語の折返し」。
