...
//! # 何を決めるための計器か
//!
//! 索引の全走査は全ディレクトリを `read_dir` して子を列挙する（実測 約 22 秒・#1001。当時は
//! 背景再スキャンがこれをキャッシュヒットの起動ごとに踏んでいた）。
...
//!
//! **棚上げであって永久却下ではないので、このファイルは残す。** 自動の背景再スキャンは
//! #1001 で撤去され、走査は明示操作の契機だけになった（`ADR-rescan-explicit-only`）——
//! 残る全走査 1 回ぶんの代金を同じ器で測り直すためである。
//!
//! **撤去条件**: #1001 が閉じ、かつ明示の全走査の代金を測り直す必要が無いと判断した時点で、
