...
//!
//! **Phase A は実 `index.bin` を書き換えうる。** `load_or_scan_with_stats` は旧版を読んだら
//! その場で現行版へ昇格するので（契約は同関数の doc）、**旧版の常駐を測りたければ先に
//! 退避すること**。下の `on_disk_index_version()` はロードより前に版を読むため報告は正しいが、
//! 直せるのは「いま測った数字の解釈」だけで、消えた標本は戻らない。
//!
