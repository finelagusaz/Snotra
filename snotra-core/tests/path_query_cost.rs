...
/// `recent_history` の実コスト。**ウィンドウを開いた瞬間・クエリ消去時には走らない**（頻度と呼び出し元の正本は `SearchEngine::recent_history` の doc）。**この計測が測るのは「呼ばれたときの 1 回」である。**
///
...
fn measure_recent_history_cost() {
    ...
    println!(
        "  = ウィンドウを開くたび・クエリを消すたびに払う額（{n} 件ぶんの照合表を毎回組む現行の形）"
    );
}
