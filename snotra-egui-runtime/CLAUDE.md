...
- **視覚欠陥を追うときの軸: 自作 `fill_mesh` はカバレッジ AA を持たない**（ピクセル中心の二値判定・`raster.rs`）。glow/wgpu が sub-pixel で吸収していた分数差——フォント間のベースライン差・薄いストローク——が**整数 px へ丸められて顕在化する**。ゆえに「レンダラーを替えてから見えるようになった」類の症状は、まずこのクラスを疑う。#399（複数フォントを 1 ファミリに混ぜるとベースラインがずれる・`snotra-settings/CLAUDE.md`）が #579 で再発したときも、糸口はここだった。視覚欠陥は型検査・clippy・ユニットテストを素通りするため、**症状語を先に正確化してから**追う（第一印象に anchor すると再現条件の探索を丸ごと無駄にする）
