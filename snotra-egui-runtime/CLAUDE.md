...
- **`RuntimeFrame` の埋めないフィールドは既定値が黙って効く**（`RawInput` と同型）——`set_clear_color` を呼ばなかったフレームは `renderer.rs` の `CLEAR_COLOR`（`0x0028_2828`）へ落ちる。**呼び忘れの検知はビルドでも検査でもなく目視だけである**（`docs/build-commands.md`「`[visual]` の色を変える変更は、**非既定色で**目視する」）。この定数は `snotra-core` の `default_background_color()` と同値だが、**当 crate は同 crate に依存せず一致は規約にすぎない**（乖離時に落ちる検査は無い・受容する残余）
