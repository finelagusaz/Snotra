...
//! 作る**ので、この経路は実際に踏まれた——#872 の測定ハーネス（`scripts/repro-pester-flake.ps1`）
//! が反復ごとの env 復元でこれを作り、**2 反復目以降の全測定が計器つきで走っていた**（実測
//! 26/27 反復）。計器は 1 事象あたり 2 行の stderr を挟むため、率も故障の現れ方も変える。
//!
