...
mod tests {
    ...
    /// 「UI が拾えるキーの網羅」ではなく、**マッピングと手書き `expected` の食い違い**を弾くだけ
    /// である。逆向きに `Key::Backspace => Some("Backspace")` を足すと、落ちるのは集合一致の assert
    /// （`UI key mapping set changed`）である——**末尾の parse ループ（core が受理するか）も同じ
    /// テストが見ているが、その変異では働いていない**（2026-08-09 実測・#1008）。
    #[test]
