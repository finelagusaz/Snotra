...
pub(crate) enum TrKey {
    ...
    // スタートアップ登録（config.toml ではなくレジストリが正本・SPEC.md §7.7）
    HeadingStartup,
    CbLaunchAtLogon,
    StatusAutostartEnabled,
    StatusAutostartDisabled,
    ErrAutostartExeNotFound,
    ErrAutostartRegistry,
    HeadingLanguage,
    LabelLanguage,
    LanguageJa,
    LanguageEn,
...
fn ja(key: TrKey) -> &'static str {
    match key {
        ...
        TrKey::HeadingStartup => "スタートアップ",
        TrKey::CbLaunchAtLogon => "Windows のサインイン時に Snotra を起動する",
        TrKey::StatusAutostartEnabled => "スタートアップに登録しました",
        TrKey::StatusAutostartDisabled => "スタートアップから削除しました",
        TrKey::ErrAutostartExeNotFound => "snotra.exe が見つかりません",
        TrKey::ErrAutostartRegistry => "レジストリの操作に失敗しました（コード {code}）",
...
fn en(key: TrKey) -> &'static str {
    match key {
        ...
        TrKey::HeadingStartup => "Startup",
        TrKey::CbLaunchAtLogon => "Start Snotra when you sign in to Windows",
        TrKey::StatusAutostartEnabled => "Added to startup",
        TrKey::StatusAutostartDisabled => "Removed from startup",
        TrKey::ErrAutostartExeNotFound => "snotra.exe was not found",
        TrKey::ErrAutostartRegistry => "Registry operation failed (code {code})",
