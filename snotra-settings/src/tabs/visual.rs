...
mod tests {
    ...
    fn default_config_matches_obsidian_preset() {
        ...
        // **保証は狭い**（`..` を使わないこと・`app.rs` の `field_mutations` と同型）:
        // `PresetDef` にフィールドが増えるとここが E0027 になるが、`新フィールド: _,` の 1 行で
        // 通り、下の変異を足さないまま緑になる（同型の app.rs 側を 2026-08-09 に実測・#1008）。
        // 見ているのは「増えたフィールドを一度は目にしたか」だけで、変異の追加は強制しない。
        let PresetDef {
            preset: _,
            label: _,
            bg: _,
