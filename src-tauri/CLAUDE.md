...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `results_view.rs` / `results_window.rs` / `visual.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。`lifecycle.rs` は純粋核（`plan_hotkey` / `blur_should_hide`）、`search_state.rs` は検索状態の純粋核（`SearchState` / `interpret` / `QueryIntent`）、`layout.rs` は高さ算出 + debounce の純粋核（`Metrics` / `results_window_height` / `Debouncer`。旧 `compute_window_height` / `HeightParams` は #646 PR2 で撤去済み）、`icon_textures.rs` はアイコン・テクスチャ層の純粋核（PNG→ColorImage decode・抽出要否述語・可視集合 retain。worker spawn / load_texture の driver は `results_view.rs` が持つ・#646 PR2 で `view.rs` から移管——テクスチャは egui Context〔= 窓の renderer〕従属のため）、`notify.rs` は通知 primitive の純粋核（一時通知 NoticeSlot + updater toast 状態機械 UpdaterUi・#532 SU5）、`strings.rs` は UI 文言テーブル（言語は view.rs lang() の毎フレーム live-read）、`view.rs` は検索 view（TextEdit・キーボードナビ・起動・RowsSnapshot 発行・results 窓 driver・indexing 案内overlay）、`results_view.rs` は結果リスト窓の従属 view（`RowsSnapshot` を描画・行クリックは共有スロット経由で main へ逆流・アイコン worker・検索状態は main が所有・#646 PR2）、`results_window.rs` は results 窓の所有型（生 Win32 の show/hide/topmost と可視フラグを 1 つの型が同時に持つ・#671 PR A′）、`visual.rs` はテーマ（config `[visual]` + `show_icons`）の 1 フレーム分の読み取り値と純粋な導出（`VisualSnapshot` / `RowTheme`・#673）、`mod.rs` は窓生成（main/results 両窓）・show/hide（両窓同期）・位置永続・reset-on-show・hide/config-wake listener（責務は各 `//!`）
  ...
  - **テーマの読みは 1 フレーム 1 回（#673 spec 決定 4）**: `read_visual` が返す `VisualSnapshot` を各 view の `update()` で 1 回だけ取り、以降はそれを読む。**同じ値を後段で config から読み直さない**——間に `config_watcher` の適用が挟まると、同じフレームの中で新旧が混ざる（新 `font_size` を旧行高で描く等）。**snapshot を `self.` へ保持してもならない**（毎フレーム live-read が config 変更の反映経路そのもの・#576 / #646 決定 2）。導出式の正本は `layout::Metrics::from_config` と `layout::path_size` で、guard 内からそれを呼ぶ。**色のパーサは 2 つあり統合しない**——描画色は `egui::Color32::from_hex`（`#RGB` 等も受理）、tao のネイティブ背景ブラシは `config_watcher::parse_hex_color`（`#RRGGBB` 厳格）。一本化すると白フラッシュ回避経路の挙動が変わる
