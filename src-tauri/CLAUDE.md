...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `launcher_controller.rs` / `results_view.rs` / `results_window.rs` / `visual.rs` / `window_coordinator.rs` / `font_stack.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。以下は各ファイルの分担:
  - `font_stack.rs` はフォント解決と `set_fonts` 登録（消費者が `view.rs` と `results_view.rs` の両方にあるため独立モジュール・#666 段 3 タスク 1。責務詳細は `//!`）
  - `launcher_controller.rs` は検索セッション層（show を跨ぐ状態・結果・選択・起動・履歴・期限）を所有し、`view.rs` から遷移メソッドとして呼ばれる（依存は一方向・#666 段 3 タスク 2。責務詳細は `//!`）
  ...
  - `icon_textures.rs` はアイコン・テクスチャ層の純粋核（PNG→ColorImage decode・抽出要否述語・可視集合 retain。worker spawn / load_texture の driver は `results_view.rs` が持つ・#646 PR2 で `view.rs` から移管——テクスチャは egui Context〔= 窓の renderer〕従属のため）
  - `notify.rs` は通知 primitive の純粋核（一時通知 NoticeSlot + updater toast 状態機械 UpdaterUi・#532 SU5）
  - `strings.rs` は UI 文言テーブル（言語は launcher_controller.rs lang() の毎フレーム live-read）
  - `view.rs` は main 窓の 1 フレーム（入力の読み 2 段・描画・OS 窓への適用。検索セッションの状態と遷移は `launcher_controller.rs`・results 窓の driver は #749 で `window_coordinator.rs` へ移設。責務詳細は `//!`）
  - `results_view.rs` は結果リスト窓の従属 view（`RowsSnapshot` を描画・行クリックは共有スロット経由で main へ逆流・アイコン worker・検索状態は main が所有・#646 PR2）
  - `results_window.rs` は results 窓の所有型（生 Win32 の show/hide/topmost と可視フラグ、直近適用サイズの memo を 1 つの型が同時に持つ・#671 PR A′ / #749）
  - `visual.rs` はテーマ（config `[visual]` の色・font・padding + `show_icons`。`window_gap` は除く）の 1 フレーム分の読み取り値と純粋な導出（`VisualSnapshot` / `RowTheme`・#673）
  ...
  - `mod.rs` は窓生成（main/results 両窓）・共有状態・config の 1 フレーム読み・listener 登録・起動時 updater check（責務は各 `//!`）
