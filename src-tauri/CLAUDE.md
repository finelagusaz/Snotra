...
**`&mut self` で状態を進めてから処置を返す関数は `#[must_use]` を持つ**——落とすと副作用（hide 要求・repaint 予約・検索の発火・ビュー復元）が黙って消え、状態は既に進んでいるので呼び直しでは戻せない。付け方・射程・対象外は次のとおり:

- **配置は「型を本 crate が所有するなら型段（`enum` 宣言）、`bool` / `Option<T>` のような primitive・外来型を返すならメソッド段」。** 型段（`EscapeOutcome` / `BlurAction`）はその型を返す将来の関数も自由関数も覆う（`blur_grace_action` が実例）。`Option` は型に `#[must_use]` を持たない（`Result` は持つ）ので、`Option` を返すものは必ずメソッド段になる
- **参照（`&T`）・不透明型（`impl Trait`）・ジェネリック型に包まれた／消された返り値を返す関数を足すとき、および既存の関数の返り値型をそう変えるときは、型段に加えてメソッド段も付けること。** 型段がメソッド段より射程の狭い一点がそこにある——型段はそういう返り値では発火せず、覆われる例外は `Box<T>` / tuple / 配列の 3 つだけである（#934 で probe crate を立てて実測。rustc が再帰するのはそこだけで、`Option<T>` / `Vec<T>` は std 型でも発火しない。`Result<T, _>` が発火するのは `Result` 自身が `#[must_use]` だからで中身のおかげではなく、診断文も `unused std::result::Result` と中身を名指さない——`Box` の `unused boxed A` / tuple の `unused A in tuple element 0` と対照）。メソッド段は返り値の形によらず発火する。後者（返り値型の変更）が見落としやすい——メソッド段なら属性は関数に付いて追随するが、型段はシグネチャを編集した瞬間にガードだけが黙って消える。現時点でそういう形は 0 件
- **メッセージ（`= "…"`）で失うものを名指すのも規約である**（#840 の `on_escape_pressed` も #934 で追随させた）
- **対象外が 2 種ある。** (1) トークンを返すもの（`enter_folder` / `navigate_folder` の `u64`）——production で token を消費するのは `spawn_folder_load` だけであり、落とせば spawn の呼び忘れとして同じ数行の中で目に見える（型が塞いでいるのは「token 無しに load を呼ぶこと」だけで、「load を呼び忘れること」ではない）。付けてもテストが `let _ =` を要求して逃げ道を増やすだけになる。(2) 状態を進めずに導くもの（自由関数 `plan_hotkey` / `overlay_kind` / `present_results` / `blur_should_hide` / `interpret` と、`&self` メソッド——`interp` は `QueryIntent` の導出、`accept_folder_result` は token の照合述語）——名前の上では処置を返すが、同じ入力で呼び直せる＝失われるものが無い
...
`read_visual` が返す `VisualSnapshot` を各 view の `update()` で 1 回だけ取り、以降はそれを読む。snapshot の範囲・色の扱い・style への適用は次のとおり:

- **snapshot は `[visual]` 全体ではない。** `window_gap` は `position_results_below_main` が別に読む（`Moved` リスナーとも共用しフレームに閉じないため・意図的な除外）
- **同じ値を後段で config から読み直さず、snapshot を `self.` へ保持もしない。** 間に `config_watcher` の適用が挟まると、同じフレームの中で新旧が混ざる（新 `font_size` を旧行高で描く等）。保持しないのは、毎フレーム live-read が config 変更の反映経路そのものだからである（#576 / #646 決定 2）。導出式の正本は `layout::Metrics::from_config` と `layout::path_size` で、guard 内からそれを呼ぶ
- **色のパーサは `egui::Color32::from_hex` の 1 本であり、tao のネイティブ背景ブラシへは `visual::native_brush_color` が `Color32` から変換する**（#680 の 1 を解消・spec `docs/superpowers/specs/2026-07-28-config-background-color-design.md` 決定 4）。`from_hex` は `#RGB` 等も受理する。ブラシ側の alpha は 255 に固定する（softbuffer の clear color が `0x00RRGGBB` で alpha を持てず、下地と定常が食い違うため。両者は同じ `Color32` から導くので必ず一致する）。`#RRGGBBAA` の alpha は「無視」されない——`from_hex` が RGB を alpha で premultiply する（正本は `visual.rs` の `background_color_premultiplies_alpha_rather_than_ignoring_it`——実際に測っている唯一の場所）。2 本立てだった頃は `#FFF` が描画色だけ通り下地は既定色へ落ちていた
...
検索 worker は `engine.search` の間じゅう `Mutex<Engine>` を握る（実運用点での保持時間は `Engine::config_handle` の doc）ため、その錠越しに設定を読むとフレームが走査の完了まで返らない（`read_window_width` の読み max と予算超過フレーム数の実測値は `PERFORMANCE.md`「設定の読みを engine lock の外へ出す」）。読みの口・中身の制約・機構の射程は次のとおり:

- **口は 2 つ、read guard を取る地点は 1 つである**——`&AppState` を持つなら `AppState::read_config`、`&AppHandle` しか無いなら `egui_shell::read_config`（後者は前者への委譲であり、自分で見るのは `AppState` 不在の面倒だけである）。この 2 つ以外に `AppState.config` へ届く口は、`state.rs` の外には無い（#1129）——フィールドが private ゆえ、外に `state.config.read()` と書くと `cargo check` が E0616 で落ちる（回帰の形を注入して 2026-08-18 に実測）。構築も同じく閉じた——`AppState` を建てる道は `AppState::new` だけで、外で構造体リテラルを書くと E0451 になる。おかげで「engine と同じ `Arc` を渡す」（#1032）を守る地点は 3 か所から 1 か所へ縮んだ（内訳は `AppState::new` の doc）。閉じたのは外から届く綴りである——`state.rs` の中（`#[cfg(test)] mod tests` を含む）では今も綴れる（「crate 内のどこでも」から縮んだのであって消えてはいない）。射程は `AppState::read_config` の doc
- **`read` へ渡すのは純 CPU だけにする——錠も I/O も取らない。** 読みの間じゅう `update_config` の書き込みは進めないので、中で不定時間ブロックしうるものはすべて禁じる。engine 以外の `Mutex` も同じである。とくに `engine.lock()` は、遅くなるのではなく両者が永久に待つ（製品に `engine Mutex → config の write` の順序が実在するため。機序は `AppState::read_config` の doc が正本）。ファイル I/O も同じ理由で禁じる（実例と実測値は `commands/launch.rs` の `resolve_opener` の doc・#524。同ファイルの 2 関数が `is_dir` を読みの前に置いているのはこの理由である）。射程は読みだけである——書き込みは `update_config` の 1 本で、そちらは engine lock の内側に残す（理由は `snotra-core/CLAUDE.md`「engine.rs のロック最小化パターン」）
- **この規則に条件は付かない**（#1123 で最後の 1 つを消した）——どのスレッドで走る読みかも、何のための読みかも問わない。ゆえに呼び出し元を辿ってスレッドを判定する責務も、非自明な経路（`on_event_loop` / tao の window-event リスナー / `app.listen`）を覚える必要も無い。機構はコンパイラである（#1123 で lint から移した）——`Engine::config` は `#[cfg(test)]` で閉じてありこの crate からは綴れない——閉じたのは外から届く綴りであって、`snotra-core` の内側は自分の `config` フィールドを直に読む（回帰の形を注入すると `cargo check` が E0599 で落ちることを 2026-08-18 に実測。`clippy.toml` の禁止と `#[expect]` による分類の記録は同時に撤去した）
- **規範は機構より広い。「engine 錠越しの config の読みは書けなくなった」と書いてはならない。** 塞がるのは `Engine::config` と `AppState.config` という綴りであり、`engine.lock()` 越しに `config_handle().read()` を取り直す形は今も通る（#1123 / #1129 の両サイクルで実測）。`Engine` の他メソッド（`search` / `recent_history` / `begin_index_drain` 等）を錠越しにイベントループから呼ぶ形も同じだけ待つ——これは仮定ではない: `launcher_controller` の `/history` 枝（`recent_history`）と `record_folder_expansion` が現にその形であり、`on_enter` の同期 `search` は #1004 が明示的に正当化している。guard の中に I/O を書く形も構造では止まらない——クロージャ形が保証するのは guard を外へ持ち出せないことだけである（受容する残余。`AppState::read_config` の doc が正本）
- この規則が条件を持っていた頃の前史——かつて置いていた装置と、その切り方の案 A〜G をすべて却下した理由——は `ADR-config-read-exception-discriminator`、装置そのものを置かないと決めた判断と却下した代替は `ADR-config-read-without-exception`、残った直読みへ検知器を置かず可視性で塞いだ判断（と、そのとき却下した grep 検査・全フィールド private 化）は `ADR-appstate-config-visibility`
