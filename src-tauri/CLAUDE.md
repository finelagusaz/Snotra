...
- **ある窓の show / hide / topmost のいずれか 1 つが tao を迂回したら、残り 2 つも必ず迂回側へ寄せる。混在は許されない。** `apply_diff` はフラグ差分がゼロなら早期 return し、`VISIBLE` を持たない窓には `SW_HIDE` を副作用で撃つ。片方だけ raw にすると「`hide()` が何もしない」「`set_always_on_top` で窓が消える」が同時に生まれる（#646 PR2）。窓ごとの層は次で固定する:
  ...
  - **述語のゲートは「読んだ時刻」しか守らない。** ゲートの読みと raw `ShowWindow` の間には Win32 呼び出しが挟まり、`hide_egui_main` は**別スレッド**（hotkey listener は Win32 メッセージループスレッド上で走る——本ファイル「Win32 メッセージ配送の注意」）からその隔たりへ割り込める。ゆえに `results 可視 ⇒ main 可視` は 3 点で守る: ①事前ゲート（`present_results`）②**事後検査**（`layout::must_retract_results`——撃った後に `main_visible` を読み直し、失われていれば撤回する）③**hide の権威性**（`layout::HideReason::MainGone` の hide は可視フラグを無視して raw 操作を撃つ）。**③が要る理由は「フラグ=false・窓=可視」の食い違いである**——その状態で main が消えると、フラグを見る hide は黙って no-op し、main が hidden の間は `update()` が走らない以上、拾い直すフレームが来ない
