...
各ルールは「**リード（`###` 見出しか太字）= 守る指示**、後続 = 理由・経緯」の形式で読む。迷ったらリードに従えば安全。
...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `launcher_controller.rs` / `results_view.rs` / `results_window.rs` / `visual.rs` / `window_coordinator.rs` / `font_stack.rs` / `search_dispatch.rs` / `search_worker.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。以下はファイル別の索引（`//!` に収まらない横断不変条件は本節の `###` 各項）:
  ...
  - `launcher_controller.rs` — 検索セッション層（show を跨ぐ状態・結果・選択・起動・履歴・期限）の所有者（責務は `//!`）。型・構築・読み口（`&self`）だけを持ち、遷移は責務ごとの子モジュールが `impl` を分けて持つ（子は private・`view.rs` へ届く名前は `pub(in crate::egui_shell)` と本ファイルの re-export が決める）
    - `launcher_controller/activation.rs` — 起動の入口（Enter / クリック / Shift+Enter）と dispatch・in-flight 回収・slash の即実行（責務は `//!`）。**起動の入口 3 本をこのファイルへ集めてあるのは、ソーステキスト検査の母集団を割らないためでもある**（理由は `launcher_controller.rs` の `//!`）
...
**`&mut self` で状態を進めてから処置を返す関数は `#[must_use]` を持つ**——落とすと副作用（hide 要求・repaint 予約・検索の発火・ビュー復元）が黙って消え、状態は既に進んでいるので呼び直しでは戻せない。**配置は「型を本 crate が所有するなら型段（`enum` 宣言）、`bool` / `Option<T>` のような primitive・外来型を返すならメソッド段」**——型段（`EscapeOutcome` / `BlurAction`）はその型を返す将来の関数も自由関数も覆う（`blur_grace_action` が実例）。**`Option` は型に `#[must_use]` を持たない**（`Result` は持つ）ので、`Option` を返すものは必ずメソッド段になる。**型段がメソッド段より射程の狭い一点がある**——**参照（`&T`）・不透明型（`impl Trait`）・ジェネリック型に包まれた／消された**返り値では発火しない——**覆われる例外は `Box<T>` / tuple / 配列の 3 つだけである**（#934 で probe crate を立てて実測。rustc が再帰するのはそこだけで、`Option<T>` / `Vec<T>` は std 型でも発火しない。`Result<T, _>` が発火するのは `Result` 自身が `#[must_use]` だからで中身のおかげではなく、診断文も `unused std::result::Result` と中身を名指さない——`Box` の `unused boxed A` / tuple の `unused A in tuple element 0` と対照）。メソッド段は返り値の形によらず発火するので、**そういう形を返す関数を足すとき、および既存の関数の返り値型をそう変えるときは、型段に加えてメソッド段も付けること**——後者が見落としやすい（メソッド段なら属性は関数に付いて追随するが、型段はシグネチャを編集した瞬間にガードだけが黙って消える）。現時点でそういう形は 0 件。**メッセージ（`= "…"`）で失うものを名指すのも規約である**（#840 の `on_escape_pressed` も #934 で追随させた）。**対象外が 2 種ある。** (1) **トークンを返すもの**（`enter_folder` / `navigate_folder` の `u64`）——production で token を消費するのは `spawn_folder_load` だけであり、落とせば spawn の呼び忘れとして同じ数行の中で目に見える（**型が塞いでいるのは「token 無しに load を呼ぶこと」だけで、「load を呼び忘れること」ではない**）。付けてもテストが `let _ =` を要求して逃げ道を増やすだけになる。(2) **状態を進めずに導くもの**（自由関数 `plan_hotkey` / `overlay_kind` / `present_results` / `blur_should_hide` / `interpret` と、`&self` メソッド——`interp` は `QueryIntent` の導出、`accept_folder_result` は token の照合述語）——名前の上では処置を返すが、**同じ入力で呼び直せる**＝失われるものが無い。**「落とせなくなった」と書いてはならない**（受容残余は 4 件——上の「型段の射程」・末尾の「`double_must_use` を当てにしない」と、次の 2 つ）: `let _ =` は今も通り——`lifecycle.rs` のテストに実使用があり、**rustc 自身が修正提案として案内する**（`clippy::let_underscore_must_use` は pedantic で未有効。有効化するとその正当なセットアップを誤爆させる）。また捕まえるのは「一度も見なかった」だけで、**`if ….is_some() { }` のような「見たが捨てた」形は通る**（`UpdaterUi::try_begin_install` はそこで `Update` を失い install 不能で固着する）。**型段へ移すとき古いメソッド段を消し忘れる誤りに `clippy::double_must_use` が当たることはあるが、当てにしてはならない**——捕まえるのは**メッセージ無しの残存だけ**である（診断文自身が `with no message` と名乗る。#934 で発火したのは #932 が入れた属性が素だったためで、**上の「メッセージで名指す」規約どおりに書かれた残存は沈黙する**。probe crate で実測）。**deny 化しているのは `ci.yml` と `.claude/hooks/post-edit.mjs` が渡す `-D warnings` であって設定ファイルではない**——この lint は clippy::style ＝ warn 既定で、`[workspace.lints.clippy]` に在るのは `disallowed_methods` だけ（#950 が同じ罠を root `Cargo.toml` のコメントで名指している）。ゆえに**移設のときは古い方を消したことを目で確かめること**。
...
`request_repaint_after(d)` は**フレームの到来を約束しない**（機構は `snotra-egui-runtime/CLAUDE.md`「不変条件」）。1 回きりの予約に賭けると hide や再検索が次の無関係な入力まで宙吊りになる。**armed 期限は 4 つ**（検索 debounce・一時通知・起動タイムアウト・blur 猶予）で全数がこの形。**再要求してよいのは「時間経過で解消する不成立」だけである**——フォーカス・設定値・他プロセスの生死のように時計と無関係な条件で再要求すると `request_repaint_after(ZERO)` の永久スピンになる。それらの変化は**変えた側が wake する責務を負う**（上の規範）

### テーマ色・font・行高の読みは 1 フレーム 1 回（#673 spec 決定 4）

`read_visual` が返す `VisualSnapshot` を各 view の `update()` で 1 回だけ取り、以降はそれを読む。**`[visual]` 全体ではない**——`window_gap` は `position_results_below_main` が別に読む（`Moved` リスナーとも共用しフレームに閉じないため・意図的な除外）。**同じ値を後段で config から読み直さない**——間に `config_watcher` の適用が挟まると、同じフレームの中で新旧が混ざる（新 `font_size` を旧行高で描く等）。**snapshot を `self.` へ保持してもならない**（毎フレーム live-read が config 変更の反映経路そのもの・#576 / #646 決定 2）。導出式の正本は `layout::Metrics::from_config` と `layout::path_size` で、guard 内からそれを呼ぶ。**色のパーサは 1 本である**（#680 の 1 を解消・spec `docs/superpowers/specs/2026-07-28-config-background-color-design.md` 決定 4）——`egui::Color32::from_hex`（`#RGB` 等も受理）だけを使い、tao のネイティブ背景ブラシへは `visual::native_brush_color` が `Color32` から変換する。**ブラシ側の alpha は 255 に固定する**（softbuffer の clear color が `0x00RRGGBB` で alpha を持てず、下地と定常が食い違うため。両者は同じ `Color32` から導くので必ず一致する）。**`#RRGGBBAA` の alpha は「無視」されない**——`from_hex` が RGB を alpha で premultiply する（正本は `visual.rs` の `background_color_premultiplies_alpha_rather_than_ignoring_it`——実際に測っている唯一の場所）。2 本立てだった頃は `#FFF` が描画色だけ通り下地は既定色へ落ちていた。**背景色だけは style を経由しない**——`RuntimeFrame::set_clear_color` が `run_ui` → paint の順序に乗るため同じフレームに届く。**style を経由する 3 値（`extreme_bg_color` / `selection.bg_fill` / `weak_text_color`）も同じフレームに届く**（#751。経路は別のままで、到達フレームの非対称だけが消えた）——ただしそれは **`ui.visuals_mut()` で適用しているからである**。**`ctx.set_visuals` をはじめ、`Context` 経由で global style を書いてはならない**: egui 0.35 の root `Ui` は pass 冒頭で `ctx.global_style()` を `Arc` snapshot するため、そこへの書き込みは次の pass からしか効かず、色だけを変えた config 適用フレーム（＝次フレームが来る保証の無い状況と一致する）で入力欄だけが旧色で残る。**この禁止は `src-tauri/clippy.toml` の `disallowed-methods` が機構で守る**（#900）——ただし**守るのは列挙した 7 メソッドの直呼びだけ**であり、`ctx.options_mut` / `ctx.memory_mut` から `Options` の `dark_style` / `light_style` へ直接書く経路は通る（列挙と除外理由の正本は同ファイル。**規範は機構より広い**）。**帰結が 2 つある。**（1）**適用位置が correctness の条件である**——**visuals を読む最初の操作**（ウィジェットの描画・子 Ui の生成）より前に置くこと。**その位置は `search_input_ui` の入口 1 か所に固定してある**（#949 で `update()` から移設）——3 値の唯一の消費者である `TextEdit` を描く関数であり、**同じ pass の子 `Ui` を実コードのまま観測する検査がこの順序を縛る**（正本は同関数の doc。#751 が「検知手段が無い」と記録した受容残余はここで閉じた）。**ただし検査が見るのはその関数の中だけである**——`update()` 側でその呼び出しより前に visuals を読むウィジェットを足す退行は捕まらない（**残る受容残余**。現時点でその区間に visuals を読む描画は無く、`ui.interact` は `create_widget` を呼ぶが visuals を読まないので上にあってよい）。（2）**global style はもう 3 値を持たない**——main 窓へ新しく egui コンテナ（`Area` / `Window` / `CentralPanel` / popup / tooltip）を足すなら、**その Ui へ自分で visuals を渡すこと**。それらは `ctx.global_style()` から Ui を作るため、既定色で描かれる。**#949 以降はこれがより広く当てはまる**——3 値が届くのは `search_input_ui` の入口以降だけである。却下した代替案（runtime のフック・`request_repaint` の対症療法・両方書く・検知器を置く）は `ADR-visuals-application-target`、その**「検知器を置く」の見送り**を覆した経緯は `ADR-visuals-order-detector-at-choke-point`

### config の読みは `read_config` を通す。`engine.lock()` を経てはならない（#1032 / #1123）

検索 worker は `engine.search` の間じゅう `Mutex<Engine>` を握る（実運用点での保持時間は `Engine::config_handle` の doc）ため、その錠越しに設定を読むとフレームが走査の完了まで返らない（`read_window_width` の読み max と予算超過フレーム数の実測値は `PERFORMANCE.md`「設定の読みを engine lock の外へ出す」）。**口は 2 つ、read guard を取る地点は 1 つである**——`&AppState` を持つなら `AppState::read_config`、`&AppHandle` しか無いなら `egui_shell::read_config`（後者は前者への委譲であり、自分で見るのは `AppState` 不在の面倒だけである）。**この 2 つ以外に `AppState.config` へ届く口は、`state.rs` の外には無い**（#1129）——フィールドが private ゆえ、外に `state.config.read()` と書くと `cargo check` が **E0616** で落ちる（回帰の形を注入して 2026-08-18 に実測）。**構築も同じく閉じた**——`AppState` を建てる道は `AppState::new` だけで、外で構造体リテラルを書くと **E0451** になる。おかげで「engine と同じ `Arc` を渡す」（#1032）を守る地点は 3 か所から**1 か所へ縮んだ**（内訳は `AppState::new` の doc）。**閉じたのは外から届く綴りである**——`state.rs` の中（`#[cfg(test)] mod tests` を含む）では今も綴れる（**「crate 内のどこでも」から縮んだのであって消えてはいない**）。射程は `AppState::read_config` の doc。**`read` へ渡すのは純 CPU だけにする——錠も I/O も取らない**（読みの間じゅう `update_config` の書き込みは進めないので、中で不定時間ブロックしうるものはすべて禁じる。engine 以外の `Mutex` も同じである）。**とくに `engine.lock()` は、遅くなるのではなく両者が永久に待つ**（製品に `engine Mutex → config の write` の順序が実在するため。機序は `AppState::read_config` の doc が正本）。**ファイル I/O も同じ理由で禁じる**（実例と実測値は `commands/launch.rs` の `resolve_opener` の doc・#524。同ファイルの 2 関数が `is_dir` を読みの**前**に置いているのはこの理由である）。**射程は読みだけである**——書き込みは `update_config` の 1 本で、そちらは engine lock の内側に残す（理由は `snotra-core/CLAUDE.md`「engine.rs のロック最小化パターン」）。**この規則に条件は付かない**（#1123 で最後の 1 つを消した）——どのスレッドで走る読みかも、何のための読みかも問わない。ゆえに呼び出し元を辿ってスレッドを判定する責務も、非自明な経路（`on_event_loop` / tao の window-event リスナー / `app.listen`）を覚える必要も無い。**機構はコンパイラである**（#1123 で lint から移した）——`Engine::config` は `#[cfg(test)]` で閉じてあり**この crate からは綴れない**——閉じたのは外から届く綴りであって、`snotra-core` の内側は自分の `config` フィールドを直に読む（回帰の形を注入すると `cargo check` が **E0599** で落ちることを 2026-08-18 に実測。`clippy.toml` の禁止と `#[expect]` による分類の記録は同時に撤去した）。**規範は機構より広い**——塞がるのは `Engine::config` と `AppState.config` という綴りであり、`engine.lock()` 越しに `config_handle().read()` を取り直す形は**今も通る**（#1123 / #1129 の両サイクルで実測。ゆえに「engine 錠越しの config の読みは書けなくなった」とは言えない）。`Engine` の他メソッド（`search` / `recent_history` / `begin_index_drain` 等）を錠越しにイベントループから呼ぶ形も同じだけ待つ——**これは仮定ではない**: `launcher_controller` の `/history` 枝（`recent_history`）と `record_folder_expansion` が現にその形であり、`on_enter` の同期 `search` は #1004 が明示的に正当化している。**guard の中に I/O を書く形も構造では止まらない**——クロージャ形が保証するのは guard を外へ持ち出せないことだけである（**受容する残余**。`AppState::read_config` の doc が正本）。この規則が条件を持っていた頃の前史——かつて置いていた装置と、その切り方の案 A〜G をすべて却下した理由——は `ADR-config-read-exception-discriminator`、装置そのものを置かないと決めた判断と却下した代替は `ADR-config-read-without-exception`、**残った直読みへ検知器を置かず可視性で塞いだ判断**（と、そのとき却下した grep 検査・全フィールド private 化）は `ADR-appstate-config-visibility`
...
- **show 側に逆操作は不要 — untrim を書かない**: trim されたページは show 時に OS が透過的に re-fault する。明示 untrim API は存在しない。trim が hide 前後どちらで走っても無害（再 fault するだけ）
