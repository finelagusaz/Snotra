...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `results_view.rs` / `results_window.rs` / `visual.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。`lifecycle.rs` は純粋核（`plan_hotkey` / `blur_should_hide`）、`search_state.rs` は検索状態の純粋核（`SearchState` / `interpret` / `QueryIntent`）、`layout.rs` は高さ算出 + results 可視性の導出 + 幾何 + debounce の純粋核（`Metrics` / `results_window_height` / `present_results` / `results_top_y` / `available_below` / `Debouncer`。旧 `compute_window_height` / `HeightParams` は #646 PR2 で撤去済み・旧 `results_should_show` は #752 で `present_results` へ吸収）、`icon_textures.rs` はアイコン・テクスチャ層の純粋核（PNG→ColorImage decode・抽出要否述語・可視集合 retain。worker spawn / load_texture の driver は `results_view.rs` が持つ・#646 PR2 で `view.rs` から移管——テクスチャは egui Context〔= 窓の renderer〕従属のため）、`notify.rs` は通知 primitive の純粋核（一時通知 NoticeSlot + updater toast 状態機械 UpdaterUi・#532 SU5）、`strings.rs` は UI 文言テーブル（言語は view.rs lang() の毎フレーム live-read）、`view.rs` は検索 view（TextEdit・キーボードナビ・起動・RowsSnapshot 発行・results 窓 driver・indexing 案内overlay）、`results_view.rs` は結果リスト窓の従属 view（`RowsSnapshot` を描画・行クリックは共有スロット経由で main へ逆流・アイコン worker・検索状態は main が所有・#646 PR2）、`results_window.rs` は results 窓の所有型（生 Win32 の show/hide/topmost と可視フラグを 1 つの型が同時に持つ・#671 PR A′）、`visual.rs` はテーマ（config `[visual]` の色・font・padding + `show_icons`。`window_gap` は除く）の 1 フレーム分の読み取り値と純粋な導出（`VisualSnapshot` / `RowTheme`・#673）、`mod.rs` は窓生成（main/results 両窓）・show/hide（両窓同期）・位置永続・reset-on-show・hide/config-wake listener（責務は各 `//!`）
