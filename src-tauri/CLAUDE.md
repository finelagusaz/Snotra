...
- `main.rs` — エントリポイント・Tauri セットアップ・イベントリスナー登録（責務は `//!`）。**背景再スキャンの spawn と結果適用（`apply_rescanned_index`）は Task 4（#1001）で撤去済み**——索引の更新は明示操作（`start_index_build` の kick）だけを経路とする
