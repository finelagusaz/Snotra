...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `launcher_controller.rs` / `results_view.rs` / `results_window.rs` / `visual.rs` / `window_coordinator.rs` / `font_stack.rs` / `search_dispatch.rs` / `search_worker.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。以下はファイル別の索引と、`//!` に収まらない横断不変条件:
  ...
  - **config の live-read は `egui_shell::read_config` を通す。`engine.lock()` を経てはならない**（#1032）: 検索 worker は `engine.search` の間じゅう `Mutex<Engine>` を握る（実運用点で 40〜95 ms）ため、UI がその錠越しに設定を読むとフレームが走査の完了まで返らない——`read_window_width` 単独で 43,939 µs を実測し、60fps の予算を超えたフレームが 11 本あった（A/B は `PERFORMANCE.md`「設定の読みを engine lock の外へ出す」）。**`read_config` の中で lock を取る操作を書かないこと**（read guard を握ったまま `engine.lock()` を要求すると待ちが両方に乗る）。**射程は読みだけである**——書き込みは `update_config` の 1 本で、そちらは engine lock の内側に残す（理由は `snotra-core/CLAUDE.md`「engine.rs のロック最小化パターン」）。**例外は「その読みの完了をユーザーが待たない場所」である**（#1076 で「`commands/` の操作時の読み」から書き直した）——icon worker・folder worker・tray スレッド（`commands/launch.rs` の `resolve_opener` 系）は engine lock のままでよい。**弁別子はディレクトリでも頻度でもなく、待っている人が居るかである**（規範が挙げる害がそれだからである）: egui フレームの中の読みは**フレームの復帰**を止め、show 経路（`window_coordinator.rs`）の読みは**窓が出るの**を止める。**どちらかは呼び出し元を辿って判定すること**——同じ関数が両方から呼ばれるようになれば分類は変わる。`commands/` に在っても `get_instant_commands` は `run_search_with` の Instant 枝から**毎打鍵 egui フレームの中で**走るため `read_config` を通す（場所で切っていた頃はここが穴だった）。**「フレームの外だから例外」と読んではならない**——hide 経路・hotkey listener・`config-applied` listener はどれもフレームの外だが例外ではない（listener は emit 元スレッドで同期実行される〔本ファイル「Win32 メッセージ配送の注意」〕ため、そこから show / hide が走ればユーザーはその完了を待っている）。**`config_watcher` が適用の前に読む旧 config は射程外である**——あれは読みではなく適用手続きの一部で、`update_config` と同じ錠の内側で取ることに意味がある。**機構は無い**——`engine.lock().config()` は今もコンパイルが通るので、UI 層へ新しい読みを足すときは規範として守る。**この条項が `commands/` も覆うようになったぶん、その残余は以前より広い面に掛かる**
