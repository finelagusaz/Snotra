...
- `egui_shell/`: ディレクトリモジュール（`mod.rs` + `lifecycle.rs` / `search_state.rs` / `layout.rs` / `icon_textures.rs` / `notify.rs` / `strings.rs` / `view.rs` / `results_view.rs` / `results_window.rs` / `visual.rs` / `window_coordinator.rs` / `font_stack.rs`）。製品メインウィンドウ（egui/softbuffer）の外殻 + 検索体験（#532 SU2〜SU7・flip 済みで唯一の UI 経路）。`font_stack.rs` はフォント解決と `set_fonts` 登録（消費者が `view.rs` と `results_view.rs` の両方にあるため独立モジュール・#666 段 3 タスク 1。責務詳細は `//!`）。`lifecycle.rs` は純粋核（`plan_hotkey` / `blur_should_hide`）、`search_state.rs` は検索状態の純粋核（`SearchState` / `interpret` / `QueryIntent`）、`layout.rs` は高さ算出 + results 可視性の導出 + 幾何 + debounce の純粋核（`Metrics` / `results_window_height` / `present_results` / `results_top_y` / `available_below` / `size_delta_exceeds` / `Debouncer`。旧 `compute_window_height` / `HeightParams` は #646 PR2 で撤去済み・旧 `results_should_show` は #752 で `present_results` へ吸収）、`icon_textures.rs` はアイコン・テクスチャ層の純粋核（PNG→ColorImage decode・抽出要否述語・可視集合 retain。worker spawn / load_texture の driver は `results_view.rs` が持つ・#646 PR2 で `view.rs` から移管——テクスチャは egui Context〔= 窓の renderer〕従属のため）、`notify.rs` は通知 primitive の純粋核（一時通知 NoticeSlot + updater toast 状態機械 UpdaterUi・#532 SU5）、`strings.rs` は UI 文言テーブル（言語は view.rs lang() の毎フレーム live-read）、`view.rs` は検索 view（TextEdit・キーボードナビ・起動・RowsSnapshot 発行・indexing 案内overlay。results 窓の driver は #749 で `window_coordinator.rs` へ移設）、`results_view.rs` は結果リスト窓の従属 view（`RowsSnapshot` を描画・行クリックは共有スロット経由で main へ逆流・アイコン worker・検索状態は main が所有・#646 PR2）、`results_window.rs` は results 窓の所有型（生 Win32 の show/hide/topmost と可視フラグ、直近適用サイズの memo を 1 つの型が同時に持つ・#671 PR A′ / #749）、`visual.rs` はテーマ（config `[visual]` の色・font・padding + `show_icons`。`window_gap` は除く）の 1 フレーム分の読み取り値と純粋な導出（`VisualSnapshot` / `RowTheme`・#673）、`window_coordinator.rs` は窓を駆動する責務（main の show/hide（両窓同期）・位置永続と復元・results の毎フレーム driver・wake primitive。**z-order は含まない**——`commands/window.rs` と `ResultsWindow` が持つ。**main のサイズは 2 か所に分かれる**——show 経路の bar_height collapse はここ、毎フレームの動的高さは `view.rs`（ADR-0007 の意図的な 2 導出）・#749）、`mod.rs` は窓生成（main/results 両窓）・共有状態・config の 1 フレーム読み・listener 登録・起動時 updater check（責務は各 `//!`）
  ...
  - **テーマ色・font・行高の読みは 1 フレーム 1 回（#673 spec 決定 4）**: `read_visual` が返す `VisualSnapshot` を各 view の `update()` で 1 回だけ取り、以降はそれを読む。**`[visual]` 全体ではない**——`window_gap` は `position_results_below_main` が別に読む（`Moved` リスナーとも共用しフレームに閉じないため・意図的な除外）。**同じ値を後段で config から読み直さない**——間に `config_watcher` の適用が挟まると、同じフレームの中で新旧が混ざる（新 `font_size` を旧行高で描く等）。**snapshot を `self.` へ保持してもならない**（毎フレーム live-read が config 変更の反映経路そのもの・#576 / #646 決定 2）。導出式の正本は `layout::Metrics::from_config` と `layout::path_size` で、guard 内からそれを呼ぶ。**色のパーサは 2 つあり統合しない**——描画色は `egui::Color32::from_hex`（`#RGB` 等も受理）、tao のネイティブ背景ブラシは `config_watcher::parse_hex_color`（`#RRGGBB` 厳格）。一本化すると白フラッシュ回避経路の挙動が変わる
