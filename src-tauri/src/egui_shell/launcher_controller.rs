...
mod tests {
    ...
    ///   起動の入口の外なので赤にする理由が無い
    ///
    /// **受容する残余（緑へ倒れる形が 1 つある）**: 複数行文字列やブロックコメントの中に
    /// **字下げ 4 で `fn ` から始まる行**が在ると、それが偽のヘッダとして帰属先を横取りし、
    /// 同じメソッドの後続の禁止語はその偽ヘッダへ帰属して**緑になる**（2026-08-17 に対照つき
    /// で実測——偽ヘッダ有りで緑・無しで赤）。**#1112 の穴より狭い**: 旧設計は文字列中の
    /// `    }` 1 行で切れたのに対し、こちらは起動の入口のいずれかの中で、禁止語より前に、
    /// ヘッダの形をした行が文字列／コメントへ入ることを要する（実測時点でこのファイルに
    /// 偽ヘッダは 1 件も無く、認識されたヘッダはすべて実定義だった）。**文字列の状態追跡は
    /// 入れない**——検査の道具立てが検査対象より複雑になる。
