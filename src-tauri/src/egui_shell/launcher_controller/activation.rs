...
//!
//! **起動の入口をこのファイルの外へ出してはならない**（この制約の正本はここである）。
//! `activation/tests.rs` のソーステキスト検査が `include_str!("../activation.rs")` で母集団を
//! 取るため、入口が別の子モジュールへ移ると母集団が割れる。**どの入口が対象かの正本は散文では
//! なく `tests.rs` の `entry_points` 配列である**——ここで数え上げると、足したときにこの行だけが
//! 腐る。
//!
//! **守られ方は方向で違う。移動は赤になり、追加は沈黙する。** 既存の入口を移せば検査冒頭の
//! ヘッダ assert が落ちる（赤を消すために `include_str!` のパスだけ書き換えると射程が黙って
//! 狭まるので、そのときは母集団ごと動かすこと）。一方、**別の子モジュールに 4 本目の入口を
//! 新設する形は、どの検査も赤にしない**——`entry_points` は明示の配列で、母集団の側から
//! 導かれていないためである。**これは受容する死角であり、塞いだとは書けない。**
//!
