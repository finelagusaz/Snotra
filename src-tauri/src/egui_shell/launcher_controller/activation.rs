...
//!
//! **起動の入口は 3 本ある**——`on_enter`（Enter）・`activate_or_execute`（Enter とクリックの
//! 単一 dispatch）・`shift_activate`（Shift+Enter）。3 本が同じファイルに在るのは責務が同じだから
//! だけではない: `activation/tests.rs` のソーステキスト検査が `include_str!` でこのファイルを
//! 読むため、**入口を別の子モジュールへ移すと検査の母集団が割れる**（移すなら検査も一緒に）。
//!
