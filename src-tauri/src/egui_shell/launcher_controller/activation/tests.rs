...
/// 禁止語より前に**最後に認識されたヘッダ**が起動の入口であること。**「直前のファイルの最後の
...
///
/// **「母集団」はこのファイルで 2 水準で使う。狭まりの見られ方が水準ごとに違う。**
/// [`method_body`] が切り出す**メソッド本体**の水準では、狭まりそのものを見る検査が在る
/// （canary の assert と、それを固定する [`method_body_rejects_a_population_without_the_canary`]）。
/// [`sources`] が返す**ファイル集合**の水準では、狭まりを見るのはアンカー保持ファイルが
/// 落ちる場合だけである（`docs/development-principles.md`「検証の層と、層と層の隙間」）。
