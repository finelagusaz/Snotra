...
/// **分割はこの抜けを狭めても広げてもいない**——`instant_prefix` はいま `search_flow.rs` に、
/// `resolve_tools` は同じファイルに在るが、どちらの形も緑のまま通るのは変わらない
/// （帰属はヘッダの列挙であって、呼び出しグラフを辿らないため）。
///
...
/// **帰属の濾過が要らなくなったわけではない**——2026-08-27 に母集団を測ると `read_config(` は
/// 2 行在り（`execute_instant_selected` と `resolve_tools`。どちらも起動の入口ではない）、
/// それを通しているのは今も帰属である。**残る 2 つの禁止語は母集団に 0 件で、帰属の濾過は
/// 現時点で空回りしている**（`self.indexing()` は `run_search_with` と一緒に `search_flow.rs`
/// へ移り、`read_visible_rows(` は分割前から 0 件）——**それでも検知器は空虚ではない**。
/// 塞いでいるのは「入口が読み直す形が**足された**とき」であり、その発火は変異注入で実測した
/// （`on_enter` へ `self.indexing()` を 1 行挿すと赤になる）。
///
