...
//! それが緑でも何も測っていない**。見出し参照（`docs/…`「…」の形）だけは
//! `npm run governance:check` が見る。
//!
...
/// 書き換えても、**それだけでは全緑のままである**（実測。赤になるのは、加えて隣接ファイルの
/// 最初のヘッダより前に禁止語が現れたときである）。塞ぐ道具立ては検査対象より複雑になるので
/// 置かない（[`owners_of`] の残余節が同じ判断を記録している）。
///
...
/// 変わる等）を赤にするのは [`sole_file_with`] の側である
