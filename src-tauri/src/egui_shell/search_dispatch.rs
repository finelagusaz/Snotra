...
///
/// **armed が下りる経路はメソッドだけではない。** [`crate::egui_shell::launcher_controller::LauncherController::consume_reset_pending`] は show のたびに [`crate::egui_shell::layout::Debouncer`] を丸ごと作り直すため、`poll` も `cancel` も通らずに armed が落ちる（[`crate::egui_shell::layout::Debouncer::new`] は `armed: false` で作る）。**この述語を型の内側へ移すなら、reset 経路で落とす責務も一緒に移ること**——いま取り残しが起きないのは `Debouncer` ごと捨てているからであり、フィールドだけを移した瞬間にその救いは消える。対で動く `last_input_at`（同じ `impl` のフィールド）は `consume_reset_pending` が触らないので show を跨いで古いまま残っており、**今それが無害なのは `armed` が false だからである**——`armed` を移して reset を忘れると、show 直後の最初のフレームで `poll` が「隠れていた時間」を経過として読み、その場で trailing を撃つ。**同型の残余（show を跨ぐ状態を足して `consume_reset_pending` の一覧へ入れ忘れる形。検知手段が無いことまで）と、その解き方の先例は [`crate::egui_shell::lifecycle::BlurGrace::reset`] の doc が正本である**（#745。`*self = Self::default()` を `consume_reset_pending` から呼ぶ形で、フィールド代入ではなくメソッドで丸ごと畳む）。
///
