...
//!
//! **`SearchState::rows_generation` とは別の量である**——世代は「行が差し替わったか」、ここの seq は「どの要求か」を指す。統合しない。**両方を同じ場所で進めるのは [`crate::egui_shell::search_state::SearchState`] の private な行差し替え点である**（#1039。この型はその内側に住む private フィールドであり、外から名前で届かない）。
//!
...
impl SearchDispatch {
    ...
    /// in-flight を失効させる。
    ///
    /// 「同期で行を差し替える出所は必ずここを通す」は spec §4.5
    /// （`docs/superpowers/specs/2026-08-10-search-worker-design.md`）が置いた**規約**だったが、
    /// #1039 で [`crate::egui_shell::search_state::SearchState`] の**行の差し替えと同じ private
    /// メソッドの内側**へ入れたことで機構になった——`results` は private で、そこへ代入する経路が
    /// その 1 メソッドしか無いため、**差し替えを書いて失効を書き忘れる形が表現できない**。
    ///
    /// **機構が保証するのはその向きだけである。** このメソッドは `egui_shell` の内側から見えるので、
    /// 行を差し替えずにここだけ呼ぶ新しい経路は書ける（`rg "\.invalidate\(\)"` の production ヒットは
    /// [`crate::egui_shell::search_state::SearchState`] の行差し替え点 1 件だけで、本ファイルの
    /// `#[cfg(test)]` に 2 件。**行を差し替えずに呼ぶ経路はまだ無い**）。それを足すなら、`pending` の
    /// 生成 1 に対し破棄 1 が全終端にあるという不変条件を自分で確かめること。
    ...
    ///
    /// **0 が sentinel として曖昧でないのは、[`SearchDispatch::issue`] が `next_seq += 1` を先に行い
    /// seq が 1 始まりだからである。** 消費者はこれに依存する: `pending_seq() == seq` を「その seq が
    /// まだ in-flight である」と読む形（`LauncherController::drain_search` が捨てた理由を切り分ける
    /// のに使う）は、0 と実在の seq が衝突しないことで初めて全域で正しい。
...
mod tests {
    ...
    fn stale_result_is_dropped_after_synchronous_replacement() {
        ...
        // クエリを空にした → 行を差し替える側（#1039 以降は `SearchState::put_rows`）が撃つ
        d.invalidate();
        // worker の結果が遅れて届く
        assert!(
