...
/// クランプが毎フレーム発火する**——マウスドラッグでは除外される経路が、ここでは除外されない。
/// 対照実験（クランプ行を落としたローカルビルド）で分離した:
///
/// | | 移動中に到達する bottom | 外に留まれるか |
/// |---|---|---|
/// | クランプ有効 | 作業領域の下端**ちょうど** | 留まれない（`Enter` の**前に**引き戻される） |
/// | クランプ無効 | カーソルが画面下端で止まる位置 | 留まる |
///
/// 止まる値が作業領域の下端ちょうどであることが、止めているのがカーソルの限界ではなく
/// `WorkArea::clamp` であることを示す。**多モニターでの封鎖そのものは未測定である**——
/// 測ったのは機序であり、封鎖は上の作業例からの演繹にとどまる。
///
