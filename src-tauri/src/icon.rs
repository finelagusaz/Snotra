...
impl IconCache {
    ...
    /// 現在のキャッシュではない**——そのずれが安全である理由は [`Self::remove_paths`]
    /// の doc が正本とする。
    ...
    ///
    /// # 受容する残余（3 つ・どれも構造では塞いでいない）
    ///
    /// **1. 剪定を生き延びたキーを有界な時間で片づける経路は無い。** 判定の窓の間に
    /// 挿入されたキーは `dead_paths` に居ないので残り、`enforce_cap` は FIFO ゆえ索引を
    /// 見ず、`save_if_dirty` はむしろ `icons.bin` へ書く。片づくのは次の
    /// `drain_index`——config が動かなければ来ない。**これは lock を握っていた頃からの
    /// 性質である**（当時も、剪定中に lock 待ちした挿入は `retain` の**後**に着地して
    /// 判定を受けなかった。窓の長さも同じ）——包含が厳密に成り立つのは誰も観測できない
    /// 一瞬だけだった。害は cap（既定 1,000）件に有界で、索引に無いパスは表示されない
    /// ゆえ読まれない。
    ///
    /// **2. 窓の間にキャッシュが世代交代することがある。** `invalidate_icon_cache` が
    /// `None` と `icons.bin` 削除を撃つと、次の表示が `IconCache::load` で**別の世代**を
    /// 建てる——呼び出し側の `as_mut()` は `None` を弾くだけで、世代の違いは見ない。
    /// **消えるものは正しいが、時期が早い**: `dead_paths` の要素は (a) 古い世代に在り
    /// (b) 呼び出し側がこれから差し替える索引に無いパスなので、消してよいものである。
    /// ただし新しい世代がその同じパスを**旧索引の表示行のために抽出し直した直後**なら、
    /// その 1 件は空になって再抽出が走る（行そのものが差し替えで消えるまでの間）。
    /// **「無害」ではなく「一時的な再抽出で済む」が正しい**。
    ///
    /// **3. 終了時の flush と競合しうる。** `main::flush_persistent_state` が窓の間に
    /// lock を取ると剪定前の `icons.bin` を書いて `dirty` を落とし、その後の除去は
    /// `exit(0)` に追われて保存されない。**旧実装でも起きたが、負ける窓が「lock 争奪の
    /// 一瞬」から「走査の 36 ms 全体」へ広がった。** 帰結は残余 1 と同じ（死んだ
    /// エントリが 1 セッション分残る）。
