...
pub fn start_index_build(app: &AppHandle) -> bool {
    ...
    // `RescanOutcome::Changed`（main.rs）が担っていた。今回ここにも足したため、
    // **現在は 2 経路（ここと `RescanOutcome::Changed`）から `invalidate_icon_cache` が
    // 呼ばれる**——背景再スキャンの撤去は Task 4（#1001）の予定で、それが終わればここが
    // 唯一の担い手になる。**判定を置かない**——ユーザー（あるいは config 変更）が再構築を
...
#[cfg(test)]
mod tests {
    /// **アイコンキャッシュの無効化の担い手は現在 2 つある: ここと背景再スキャンの
    /// `RescanOutcome::Changed`（main.rs）である。** 背景再スキャンの撤去は Task 4
    /// （#1001）の予定で、それが終わればここが唯一の担い手になる。#996 が再構築時の
    /// 索引照合剪定を撤去して以来、**メモリ内 `IconCacheState` と `icons.bin` の両方を
    /// 無効化する**（`invalidate_icon_cache`）直接契機はここと背景再スキャンに限られる
    /// ——表示無効化トグル（`config_watcher`）はメモリ内のみを破棄する別経路であり、
    /// この 2 つには数えない。**ここが落ちると、索引再構築を直接の契機とする無効化を失う**
    /// ——背景再スキャンはエントリ集合が変わったときしか撃たないため、集合が変わらない
    /// 再構築（設定変更・手動 `/s` 等）ではアイコンが古いまま FIFO 上限まで残る。
    ...
    fn start_index_build_invalidates_the_icon_cache() {
        ...
        assert!(
            ...
            "start_index_build がアイコンキャッシュを無効化していない——背景再スキャンの\
             `RescanOutcome::Changed` に頼るだけでは、集合が変わらない再構築（設定変更等）で\
             アイコンが古いまま残る"
        );
    }
}
