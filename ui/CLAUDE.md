...
- `ownedTimer.ts`: 所有ワンショットタイマー primitive（`createOwnedTimer(ms)`）。`setTimeout`/`clearTimeout` ハンドルを内部所有し `arm`（前回破棄 → ms 後に fn を 1 回）/`cancel`（冪等・teardown 兼務）/`isPending` を提供。**timer resource のみを所有し debounce policy（leading 等）は持たない**。`arm` は fn を `setTimeout` の中でしか呼ばず同期発火しないため再入が安全（契約不要）。SolidJS/api 非依存の純粋ファクトリ。`stores/search.ts` の `refreshTimer`（検索 50ms・leading は `!isPending()` から導出）と `stores/instantCommand.ts` の `fetchTimer`（instant 30ms・leading なし）の 2 インスタンスが利用（#536）
...
- **`await` 後に保存状態を復元する場合は staleness チェックを入れる**: `await` 前に保存した状態を失敗時に復元するパターンでは、`latestRun` の `isStale()`（lane タスク内）または `searchLane.current()` の比較（例: `executeInstantCommandSelected` の `current() === preGen + 1`）で「`await` 中に状態が変わっていないか」を検証してから復元する。無条件復元は新しい状態を上書きするレースコンディションを生む。加えて、`await` 中にユーザー入力で状態が変わること自体を防ぐガード（`handleInput` の `launching()` チェック等）を根本対策として併用する
