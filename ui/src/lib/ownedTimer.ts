...
export interface OwnedTimer {
  /** 保留中タイマーを破棄し、`ms` 後に `fn` を 1 回だけ呼ぶタイマーを張り直す。
   *  `fn` は必ず `setTimeout` の中で呼ばれる（同期発火しない）。`fn` の throw は setTimeout の
   *  callback 境界で失われ `arm` の呼び出し元へは伝播しない — 呼び出し側は throw しない `fn`
   *  （例: `() => void runRefresh()`。`runRefresh` は内部で `.catch` 済み）を渡す責務を持つ。 */
  arm(fn: () => void): void;
