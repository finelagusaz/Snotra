...
impl EpiIntegration {
    ...
    pub fn new(
        ...
        Self {
            ...
            is_first_frame: true,
            initial_window_visible: native_options.viewport.visible.unwrap_or(true),
        }
    }
    ...
    pub fn post_rendering(&mut self, window: &winit::window::Window) {
        ...
        if std::mem::take(&mut self.is_first_frame) {
            ...
            if self.initial_window_visible {
                window.set_visible(true);
            } else {
                // Keep the app alive for background event handling while avoiding startup flash.
