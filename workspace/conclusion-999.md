...
- **R-2（2 の扱い）**: 再現しないものを追い続けない。**次に遭遇したときの測り方は
  `docs/adr/ADR-injected-arrow-key-physical-identity.md` の「帰結」に残した**（2 系統の env の立て方・
  注入側の計器が呼び出し側 env に握られること・`6>>` での拾い方・`take` の階差の読み方）。
  再現ハーネス `workspace/repro-999.ps1` 自体はサイクル末に `workspace/` ごと消えるため、
  **手順は消えない場所へ移してある**
