...
## 再測の結果（Phase 2′・2026-07-26）

**1 回目の再測は無効**（私の手順ミス）——カーソル修正の目視確認のためホバーを依頼したため、`scroll_area` のフェードアニメーションが混入し、hide を 2 回またぐ 62 秒の run になった。アイドルの実数は取れなかった。

そこで**プロトコルに依存しない判別**を作った: **「点滅の待ち（400-600ms）の直後に 30ms 未満のフレームが何枚続くか」**。セッション長・操作量に依存せず、スピンの有無だけを測る。

| ログ | 点滅待ちの回数 | 直後のスピン平均 | スピン 0 の割合 |
|---|---|---|---|
| BEFORE（`predicted_dt=1/60`） | 135 | 5.61 フレーム | **0%** |
| 1 回目の再測（条件汚染あり） | 30 | 0.57 | 93% |
| **クリーン計測（`628-idle-clean.log`）** | **122** | **0.00** | **100%** |

### クリーン計測の最終値

| 項目 | BEFORE | AFTER |
|---|---|---|
| 定常アイドルの main | 11.9 fps | **2.0 fps**（5 秒あたり 10 フレームが 50 秒以上一定） |
| 定常アイドルの results | 19.8 fps | **2.0 fps** |
| paint 平均 | 1.41ms | 0.96ms |
| **CPU（1 コア比）** | **5.1%** | **0.59%**（122.5 秒・打鍵区間込み） |
| 点滅遷移ごとのスピン | 平均 5.61 フレーム | **0（122 回すべて）** |
| hidden 後のフレーム | 0 | **0**（最後の `egui_hide:done` 以降 0 行） |

2.0 fps は **egui のキャレット点滅の下限そのもの**（on 0.5s / off 0.5s = 遷移 2 回/秒）であり、これ以上は点滅を止めない限り下がらない。目視での確認（カーソル点滅の消失・blur→hide の動作）もユーザーが実機で取得済み。
