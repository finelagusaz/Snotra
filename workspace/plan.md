...
`c:\users` / `c:\users\` も実運用点では p50 16.0〜16.5 ms で 1 フレームに達している。

**この issue は「余裕を広げる」ではなく「常に落ちているものを直す」issue である。**
判定の基準（受け入れ条件 4 の「max < 16,700 µs」）はそのまま使えるが、**p50 も現状で
超えている**ので、記録と判定の文言を Phase 4 で直す。
