...
- [ ] 「4.5 最大列挙数」の「0 件のときは窓を出さない…最大表示件数が 0 のときも出さない」bullet に、
...
- [ ] 「8.6 状態遷移図」の遷移ルール要約の行（`NormalMode -> ToolSelectionMode` の `Shift+Enter` が
...
- [ ] `npm run governance:check` が緑（カテゴリ F）
