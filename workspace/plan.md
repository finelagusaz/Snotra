...
## 0. 合意事項（2026-07-25 に合意済み）

> **合意ゲートは通過した**（ユーザー確認・2026-07-25）:
> - **決定 1 = 案 A**（worker / channel / drain / listener + 窓をまたぐ共有状態 + async）
> - **決定 4 = 足す**（`.claude/rules/src-tauri.md` に 1 行・参照の形で）
> - **Phase 2 のフォールトインジェクションでサブエージェント起動を許可**（読者 2 クラス × 上限 2 巡・最大 4 体）
> - 決定 2（適用範囲に `snotra-settings` を含む）・決定 3（機構カテゴリで書きシンボルは例示ラベル付き 1 件）は推奨どおり据え置き
>
> 以降、Phase 1 から着手してよい（`/implement`）。
...
- ~~決定 1〜4 は合意待ち~~ → **合意済み**（§0）。変更ファイルは **5 件**に確定（SKILL.md / `AGENTS.md` / ルート `CLAUDE.md` / `start-issue/SKILL.md` / `.claude/rules/src-tauri.md`）
- **`AGENTS.md` とルート `CLAUDE.md` は行数を増やさない置換に限る**（常時ロード 216/216・余裕ゼロ）
- **`ResultsShared.clicked` が世代を運ばない件（S10）は実コードの潜在欠陥かもしれない**——本 issue のスコープ外だが、スキルの試金石として使うと同時にユーザーへ観測事実として報告済み
