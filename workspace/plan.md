...
- **`SPEC.md` は更新しない。** 判定は AGENTS.md の 2 参照で行う: (a) `SPEC.md` §4.2.1
  「インクリメンタル検索の不変条件」は**再利用の可否を 4 条件で持つが、候補収集の記述は持たない**
  （**「検索の意味論と結果の並びしか持たない」は偽である**——code-review が実測で訂正）、
  (b) 本変更はその 4 条件のどれも変えず、**結果を 1 件も変えない**内部最適化である
  - **ただし §4.2.1 の列挙そのものは不完全だった**（`has_path_sep` / 前回クエリ非空 /
    前回候補集合が非空の 3 つが欠落。**本変更以前からの欠落であり、この差分が入れた退行ではない**）。
    **2026-08-13 のユーザー指示で条件 5〜7 として補った**——`can_reuse` の連言 7 つと一対一になる。
    **番号は追加のみとする**——`.superpowers/sdd/plan/spec-inventory-identifiers.md` が
    「§4.2.1 条件4」と序数で指しており、間に挿入すると沈黙で参照が壊れる
