...
実 `index.bin` v7 312,108 件 + PATH マージ 100 件 = 312,208 件 /
実 `history.bin` の **global 17 件**（非空。`query` / `folder_expansion` は未計測）/
...
- **ブリッジ（2 つの計器が同一構成で一致することの実測）と、issue 本文の値と重ならない件の裁定**:
  **正本は `PERFORMANCE.md`「実運用点のパスクエリのフレームコストと、マッチ後の分解（#1067）」に
  置いた。ここには写しを残さない**——`plan.md` はサイクル終了処理で撤去されるので、
  ここに置くと恒久文書が主張する「実測してある」の唯一の証拠が次のサイクルで消える（レビュー M-6）
...
表の最終行「旧計測点」であり、そこでは旗が真のまま——`cmp_paths` は呼ばれるが
`a.cmp(&b)` で即返るので支配項のコストが発生しない（呼ばれなくなるのではない）。
...
### Phase 7 — 判定に従う実装（B: 整列している範囲を捨てない）

- [x] 旗（bool）を範囲（sorted_prefix_len: usize）へ替える。cmp_paths は両辺が範囲内のときだけ index 比較
- [x] index.bin の形式は不変（保存時に導き、読み戻しで len か 0 へ戻す）——版バンプ無し
- [x] extend_with_roots は範囲に触れない（末尾へ足すだけなので範囲は壊れない）
- [x] 全件 A/B（PATH マージ込み・非空虚検査つき）で結果が集合・順序とも不変であることを実測
- [x] 実測: c:\ の p50 が 23,126〜24,339 → 15,404（1 フレームの内側へ）。対照 3 本は動かない
- [x] PERFORMANCE.md / snotra-core/CLAUDE.md / path_store の //! を更新
