...
- [x] **（実装中に判明・計画外）fixture を 4 件目まで足す**——死角 4（他の参照のラベルの内側に
      置かれた正準形参照）は挙動が変わる形の 1 つ（OLD は内側を拾い、NEW は外側 1 件へ統合する）なので、
...
### Phase 3 — 委譲レビューへの fix-forward（計画外・c4cb4433 の後）

- [x] **R-1（High）— 未宣言の沈黙経路を宣言し fixture で固定した。** 外側の対象綴りが**無効**なとき、
      内側の正当な参照ごと沈黙する（`isRefTargetSpelling` が外側を弾き `checked` にも数えない）。
      **旧実装では内側が独立に照合されていた形＝観測性の後退**（**「唯一」は Round 2 で偽と判明**）。
      自分で再測: 露出は今日 0 件だが、無効綴りの頭を持つ一致は生きた母集団に現に在る。
      塞がずに受容した理由は ADR の当該項が持つ（塞ぐには 2 消費者が共有する新しい導出が要り、
      それ自身が「片方だけ直す形」を作り直す）
- [x] **R-2（High）— JSDoc が実装より強かった。** 死角 2 の括弧書き
      「折れ一般は `G-folded-heading-refs` が別に赤にする」は全称として偽。自分で再測:
      入れ子あり形 B は `OPEN_UNCLOSED` が入れ子の `「` で切れて **G-folded も拾わない**
      （入れ子なしの同じ折れ方は拾う）。**どちらの検査の宣言にも無かった穴**なので、
      `lib.mjs` の doc をその唯一の記載にした
- [x] **R-3（Medium）— 死角 4 の不在断定に射程が無かった。** 外側が閉じない角では内側が
      独立に照合される（安全側）。両方向を書き分けた
- [x] **R-5（Low）— probe の NOTES が 1 件ずれ。** 本コミットが足した fixture が超集合スキャンに
      写る。`refScanLines` が `.mjs` から供給するのはコメント行だけなので母集団外であり、
      **doc の「今日 0 件」と probe の値は両方正しい**。混同しないよう注記した
- [x] **R-6（Low）— 性能の数値が再現しなかった。** 「93.6 → 66.5 ms」は warmup 不揃いと見られる。
      ADR の主張を「速い」から「遅くならない」へ弱め、`research.md` に再測定値を残した

### Phase 4 — Round 2 レビューへの fix-forward（計画外・57d60306 の後）

**3 件のうち 2 件は Phase 3 の修正自身が作った偽の主張である**（`universal-claim-fix-regenerates-itself`
の 3 連鎖。「偽の全称を直した文がまた全称で偽になる」）。**数え上げをやめ、下限の主張へ倒して止めた。**

- [x] **R2-2（Critical 相当）— 「唯一の観測性の後退」が偽。** 自分で対照つきに再測:
      死角 5（外側が無効）は OLD `{checked:1,findings:1}` → NEW `{0,0}`、
      **死角 4（外側が有効で着地）も OLD `{1,1}` → NEW `{1,0}`**。対照（入れ子なし）は両者 `{1,1}` で不変。
      **後退は 2 つある。** 写し 3 か所（`lib.mjs` / ADR / fixture コメント）をすべて直した
- [x] **R2-1（High）— 「15 件」が誤った指標だった。** 自分の測定が `REF_HEAD` + `「`（**閉じ不問**）を
      数えていた——**閉じない頭は `HEADING_REF` に一致しないので、原理的に内側を飲み込めない**。
      本物の一致で測ると無効綴り **13 件**（有効 365 件が `governance:check` と一致するので構成は正しい）。
      **数を書くのをやめた**（足すたびに腐る・正本は分岐そのもの）
- [x] **R2-3（Medium）— 直した文がまた全称で偽。** 「`G-folded-heading-refs` も拾わない」は
      **形 A（行末が対象綴り）で偽**。自分で再測: 形 A は入れ子の有無によらず `TAIL_TARGET`=true で赤、
      沈黙するのは**形 B かつ入れ子**だけ。射程を形 B へ絞った
- [x] **R-4（Low・Round 1 から未決着）— 見送りを明示する。** ADR の `G-near` 残余の「不可視」は
      不正確（実際は別物として checked=1）。**`G-near` は 1 バイトも変えていないので回帰ではなく**、
      本件の射程外として残す。文言だけ「不可視」から実態へ寄せる
**PR 本文を書く者への申し送り（作業項目ではない——コミット以降は計画が所有しない）**:
`c4cb4433` と `57d60306` の commit message に「唯一の挙動変化」「唯一の観測性の後退」が残っている。
**squash マージで main の commit message になるので、PR 本文へ写さないこと**（#1056 の形）。
正しい主張は「観測性の後退は 2 つあり、どちらも入れ子を含む形に限る」である。

### Phase 5 — Round 3（枠組みを変えた反証専用の巡）への fix-forward

**出口の判定**: **実装の挙動を誤って書いた主張は 0 件**——死角 5 項と ADR の帰結・却下理由・
受容する残余 14 項が、反証手順を書き下ろす枠にすべて耐えた。残った 3 件はいずれも
**1 文の中の 1 語**で、書き換えれば真になり実装を触る必要が無い。

- [x] **F-C2 — 既存の doc 行が偽だった**（本件が書いた行ではないが、同じ JSDoc 内で誤りを残せない）。
      「`matchAll` は内部で複製するため `lastIndex` を持ち越さない」を自分で再測: **複製は
      `lastIndex` を引き継ぐ**（`lastIndex=12` を注入すると先頭の一致を落とす）。持ち越さないのは
      「元へ書き戻さない」方だけ。**結論（`matchAll` だけを使う）は正しく根拠が誤り**で、
      読者は「`matchAll` は免疫がある」と読んで当の失敗へ歩く。実害は今日 0（消費者 2 本とも
      `matchAll` のみ・grep で確認）。根拠を実測どおりに書き直した
- [x] **F-C7 — スコープの名札が誤り。** 「今日いずれも 0 件・**全追跡ファイル**で実測」は偽で、
      走査されない層まで広げると「ラベルが閉じない形」が 0 件ではない。**照合母集団**へ直した。
      **Round 2 の R2-1 と同じ型**（数に誤ったスコープ／定義が付く）が同じ括弧の中で 2 度続いた
- [x] **F-C9 — 主語が誤り。** 「入れ子を**含まない参照**は影響を受けない」は偽——それ自身が
      入れ子ゼロの参照でも、無効綴りの外側のラベル内に置かれれば沈黙する。影響を決めるのは
      **置かれた行が入れ子の構造を作るか**。主語を「参照」から「行」へ替えた
- [x] **⚠️ A9 — 数え上げを消した。** 「`findings` が 1 → 0」は内側 2 件のとき 2 → 0 になる。
      「内側の `findings` がすべて消える」へ書き替えた
- [x] **⚠️ A12 — 見積もりを実測へ。** 「数百件規模の改名」を「決定時点の実測で 249 件 / 65 ファイル
      （アンカー総数 3282）」にした
