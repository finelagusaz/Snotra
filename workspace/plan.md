...
- **`.psm1` に書いた正準形の参照も機械照合されない**（`code-review-1034.txt` の W3）。
  G-heading-refs のソース側母集団は `.md` と `.rs` だけで `.psm1` を含まない。着地は手で確認した
...
- [x] `PERFORMANCE.md`「計測と受け入れ基準」へ正本 bullet を新設する。**置く位置は
...
- [x] `PERFORMANCE.md` A 側の但し書き（`:484-488`）を、数値と機序を落として
      `PERFORMANCE.md`「計測と受け入れ基準」への参照へ置き換える（表を読むのに要る
      「差分として読む」は残す）
- [x] `PERFORMANCE.md` #1032 節（`:550`）から「（1 本約 10 ms）」を落とす
- [x] `src-tauri/src/trace.rs` の `//!` へポインタ 1 行を足す（**英語**——既存ブロックが英語で、
      `docs/comment-guidelines.md`「言語（日英）」が同一ブロック内の混在を禁じる。**数値は書かない**）

### Phase 2 — 制約 2（smoke の索引は 1 件）

- [x] `docs/build-commands.md`「スモーク運用メモ」へ帰結 bullet を新設する。含める要素:
...
- [x] `docs/development-principles.md`「構造的設計原則と強制の階梯」の故障注入 bullet へ
...
- [x] `.claude/rules/safety-nets.md`「効いていることは、フォールトインジェクションで一度は実測する」
      の該当 bullet へ、逆向きも同じ節が本文を持つ旨を 1 文で足す（写しは置かない）
- [x] `scripts/lib/SnotraTraceInvariants.psm1` の H7 の arm へ再現手順を追記する。含める要素:
...
- [x] `docs/superpowers/specs/2026-08-10-search-worker-design.md` §3.3 の (1)(2) から数値を落とし、
...
- [x] `src-tauri/src/egui_shell/layout.rs:406` の引用が指す内容が §3.3 に残っていることを、
      **現物を読んで**確かめる（序数参照ゆえ `governance:check` は照合しない）
- [x] `npm run governance:check` を実行して緑を確認する
- [x] `cargo doc --workspace --no-deps --document-private-items` を実行して緑を確認する
- [x] 写しの残数を grep で数え、生きた文書に残るのが正本 1 か所だけであることを確かめる
- [x] 触った段落を**現物で**読み直し、隣接する主張が偽になっていないか確認する

## 実装時の逸脱（3 件）

**(1) 受け入れ条件 1 の「trace の 1 行が約 10 ms」は残し、根拠を 2 本立てにしたうえで
未調停の反例を併記した**（レビュー 2 巡の往復の結果）。経緯を残す:

- 1 巡目の H1 は「測ったのは間隔であって費用ではない」と指摘し、私は主張を間隔へ弱めた
- **2 巡目の R2-H2 がその訂正を過剰と判定し、私が一次資料で裏を取って撤回した**——
  `git show 23cba3c^:src-tauri/src/trace.rs` に `write_started.elapsed()` を `TRACE_ELAPSED_US`
  へ積む実装が在り、**#1032 は `eprintln!` の書き込み時間そのものを直接計時していた**。
  `PERFORMANCE.md` の「50〜96%」はその控除量である。1 巡目が第 2 の柱に引いた `:518` は
  別の量（区間の残差）についての注記で、あれは誤読だった
- ゆえに費用の主張は**別々の 2 通りの測り方**（#1004 の間隔・#1032 の直接計時）に支えられている。
  一方で起動段の ON/OFF A/B は「差は雑音」と記録しており**未調停である**。勝者を決めず、
  反例を子 bullet として明示した（`AGENTS.md`「検証の作法（全タスク共通）」の「全称表現は
  前提条件とセットで書く」）
- 3 巡目の指摘で、第 1 文へ射程「**フレームループ中の**」を付けた（反例が子 bullet に在るのに
  第 1 文だけ無限定だった）

**(1b) 反例のリンク先にあった偽の数え上げを直した**（3 巡目 R3-M1・**射程の拡張**）。
`PERFORMANCE.md`「計器は系を乱していない」は理由を「起動段の trace は 1 行だけだから」と
書いていたが、**起動窓には `hotkey:listener_enter` / `hotkey:registered` / `startup:ready` が出る**
（grep 実測）。**私の子 bullet がその段落を初めて論拠の在り処として名指した**ため、
`.claude/rules/governance-docs.md`「ガバナンス文書の参照と命名のルール」の書く約束 (3)
（触った節の隣の主張が今も真か見る）の対象になった。理由を「未解明」へ直し、数え上げを持たない
形（「1 行ではない」）に置き換えたうえで、費用の正本を相互参照した。

**(2) 教訓 3 の `Debouncer` の記述から量化子「debounce 幅以上に」を落とした**（同 H2）。
`layout.rs` の `on_input` は `!armed` のとき leading を返し、`poll` の trailing 発火が `armed` を
降ろすので、**trailing 直後の打鍵はその場で leading を撃つ**——間隔の下限は保証されていない
（`PERFORMANCE.md` が「このフレームに leading と trailing の 2 回の検索が乗る」と実測している）。
「バースト中の再発火を抑える」へ弱め、`SnotraTraceInvariants.psm1` 側の同趣旨の一文も揃えた。

**(3) 数値の扱い（当初から）**

計画は教訓 3 の本文へ「`Debouncer`（50 ms・leading）」「実運用点の検索が 40〜70 ms で拮抗」と
数値を入れる指示だったが、**数値を書かなかった**（最終形は逸脱 (2) のとおり「`Debouncer`（leading）が
バースト中の再発火を抑えるのに対し実運用点の検索時間が debounce 幅と拮抗する」）。理由は 2 つ:

- `src-tauri/CLAUDE.md` が実運用点の検索を **40〜95 ms** と記しており、40〜70 と書くと生きた層に
  食い違う 2 つの範囲が並ぶ（`AGENTS.md`「検証の作法（全タスク共通）」の写しの禁）
- 50 ms は `launcher_controller.rs` の `Debouncer::new` が正本であり、規範文書へ写すと片方だけが動く

教訓の主張（構造的に発火しないことがある／人工的な遅延が要る）は数値なしで成立する。
