...
### Phase 6 — レビュー第 2・3 巡（同じ枠組みを修正差分へ再実行）

**`AGENTS.md`「レビュー指摘へ修正（fix-forward）を当てた」に従い、指摘を出した枠組みへ
新しい sha を渡して再実行した。** 結果、**修正のために書いた文が 2 巡続けて偽だった。**

- [x] **H-3（第 2 巡・最重要）: 「`bench-startup.ps1` の `>= 0` はこの前提に対しても唯一の網
      である」が偽。** レビュアが**その文が名指したその変異**（生成側を四捨五入）を注入して
      bench を回すと **exit 0・負値 0 件**。機序は `floor(外側) − floor(丸めた内側)` で、
      丸めが足せるのは高々 1 ms、実測した構成の余裕は 3〜4 ms——**摂動が余裕より小さいので
      符号が反転しない**。切り捨てで丸める形は `floor ∘ floor = floor` で出力が完全に不変。
      **網は 1 本ではなく 0 本だった。** → 本数の主張を全廃し、回した検査を名指す形へ
- [x] **H-4: 「もう書けない」が「書けるが検知される」だった**（変異 (a) はコンパイルを通る）。
      あわせて、当該テストが見るのは 50.9/50.5 の 1 組だけである射程も明記
- [x] **M-3「残るのは」→「少なくとも」／ M-4「検査が 1 つも落ちない」→ 回した検査の名指し／
      M-5「消費者 2 か所」（実測は 3 か所 / 2 ファイル）→ 列挙をやめて `git grep` を指す／
      L-3「この 2 行」→「両辺」**
- [x] **前巡で私が書いた「向きを守っているのはテスト 1 本だけ」も偽だった**——レビュアの
      変異 (g) が `bench-startup.ps1` も反転を捕まえること（-4/-3/-3）を実測。両方を書く形へ
- [x] **第 3 巡: 争点として返した 3 文はすべて真と実測された。**
      とくに「この要求を守る検査は見つかっていない」は、レビュアが自ら ⚠-6 で申告していた穴
      （`npm test` / Pester / `governance:check` / `smoke:egui` を当てていない）を**私が争点として
      返し、11 検査すべてに変異を当てて全件 exit 0** を確認して塞がれた
- [x] **M-6（逆向き監査で発見）: 統合の過程で「なぜテストでは捕まえられないのか」の機序が
      消え、送り先に着地していなかった**（`git grep` で生きた `.rs` に 0 件・主エージェントが再実測）。
      `startup.rs` は「理由は `LoadOrScanStats::total` の doc が正本」と送っているのに、
      **送り先がその理由を持っていない**状態だった。→ 正本へ 1 文を復帰。
      これは「次の担当者が `Timeline` を直に組む検知器をもう 1 本足して『守った』と考える経路」を
      事前に否定する情報である
- [x] **L-5: 「3〜4 ms」に機体の但し書きが無かった** → 「**機体依存**」を明記
      （[[two-dev-machines-unlabeled-in-perf-doc]] と同型）
- [x] 収束判定はレビュア自身に委ねた（「止めてよい」と回答・所見の在り処が
      実装の外側 → 網の実効性 → 消えた説明と毎巡浅くなり、新規の主張に 1 件も当たらなくなった）

### この計画の反省（`/retrospective` へ送る材料）

**「構造で消した」と書くときに、何が消えていないかを測らずに書いた。** 2 度とも
「修正のために書いた文」が次の嘘になっており、これは `MEMORY.md` の
[[universal-claim-fix-regenerates-itself]]（#1091 で 3 連鎖）と同じ形である。
**この計画は「doc に書く主張の射程（全称にしない）」という節を自分で持っていて、なお踏んだ。**
効いた唯一の手当ては**変異注入**で、推論では 2 回とも文がもっともらしく見えていた。
