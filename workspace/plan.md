...
裁定: 2026-08-18 にユーザーが「採択・`#[expect]` 形」を選択（§人間レビューに逐語）
...
8. `clippy.toml` の死経路 3（fingerprint）に、2026-08-18 の観測との食い違いが日付つきで残っている
...
**注入の作法**（**実装をコミットしてから注入する**——2026-08-18 に、未コミットの実装が載るファイルへ注入し、撤去に `git checkout -- <path>` を使って `#[expect]` 4 件を全損した。HEAD へ戻るので注入だけでなく実装ごと消える。リポジトリの記録にある #934 と同じ型である）: `clippy.toml` だけを触った回は `.rs` を touch するか `cargo clean -p snotra` を挟む（同ファイルの死経路 3）。**2026-08-18 の観測では touch なしでも診断が出たが、機序を測っていないので手順は従来どおり残す**（上位集合を満たすので安全側）。**稼働中のガードを弱める向きの変異（禁止行の削除・書き損じ・コメントアウト）は複製に当てる**——`clippy.toml` を scratchpad へ複製して変異させ `CLIPPY_CONF_DIR` を向ける（`.claude/rules/safety-nets.md`「複製に変異を当てる」）。作業ツリーへ足す向きの変異（新しい違反・`#[expect]` の一時除去）は作業ツリーで行い、`git checkout -- <個別パス>` で戻して `git status --short` で残っていないことを確認する。
...
- [x] **裁定: 4 件の注釈を抑制と見るか記録と見るか** — 2026-08-18 にユーザーが「採択・`#[expect]` 形」を選択。決め手は M4（注釈が禁止の実効性のカナリアになる）と、`style_ui` 前例との差が件数ではなく**呼び出し点ごとの判定義務の有無**であること。
...
- [x] `clippy.toml` の死経路 3 へ 2026-08-18 の観測を日付つきで追記（**機序は主張しない**）
...
- [x] 実装差分を確定させる（カテゴリ A + E + F が全緑・レビュー 2 巡の指摘をすべて反映済み）

## 注入の実測（Phase 4・2026-08-18 / rustc 1.97.1 / clippy 0.1.97）
...
## code-reviewer（4b・ラウンド 2＝ fix-forward の再実行）

同じエージェントを `SendMessage` で継続（新規起動ではない）。Critical / High **0 件**。Medium 3 / Low 4 / ⚠️ 6。**指摘は 1 件もラウンド 1 の territory を再審理しておらず、すべて修正差分の内側にある**（`review-frameworks-not-rounds` の「件数でなく所見の在り処で判断する」に合致）。

| # | 指摘 | 対応 |
|---|---|---|
| M1 | 新しく書いた「**改善の向きにだけ**嘘をつく」を、同じ節の 12 行上が反証している（黙る経路はもう 1 つ、`-D warnings` が落ちる劣化の向きがある） | 採用。「にも」へ改め、もう 1 つの経路を名指した。**偽の全称を直した文がまた全称で偽になる形**（#1091 の 3 連鎖）の実例 |
| M2 | 「綴りへの参照…**だけ**」は n=1 の実測から境界を**広げる**向きに書き換えた形。修正前の「直呼びだけ」は狭すぎたが安全側だった | 採用。「メソッド呼びの形**だけではない**」の下限主張へ倒し、「捕まらない形が他に無いことは測っていない」を併記（`clippy.toml` と `src-tauri/CLAUDE.md` の両方） |
| M3 | `plan.md` の「未検証（PR 本文へ送る）」欄がエイリオス測定後も据え置きで、同ファイル内の記述と矛盾。**PR 本文は写しの数え上げの母集団外**（#1056）なので誰にも訂正されない | 採用。当該行を測定済みへ書き換え、「PR 本文へ送らない」と明記 |
| Q2 | M1 の条件はまだ不完全——**注釈が覆う範囲に同じ lint の別の違反が無いこと**にも依る（`#[expect]` は射程内の診断 1 件で充足するため）。いまそれを保っているのは文段への配置だが、その規則に書かれた理由は別物 | 採用。条件を追記し、文段配置が構造的にそれを保っていることを繋いだ |
| L1 | 実測日が 1 日ずれている | 採用。**git で一次確認**（自分の 3 コミットはすべて 2026-08-18・08-17 は直前の main の日付）。`clippy.toml` 3 か所・`.mjs`・`CLAUDE.md`・`workspace/` を訂正 |
| L2 | 引用「上の「歴史上無い」」が書き換え後の本文に無い文字列を指している | 採用。git で確認した順序（#951 = 2026-08-06 の禁止 → #1043 = 2026-08-11 の実例）をそのまま書いた |
| L3 | 群 1 と群 3 で同じ lint の射程の書き方が非対称 | M2 の下限主張化で解消（**群 1 側には触らない**——あちらは安全側かつ #900 の射程） |
| L4 | 折返しの再整形漏れ | 採用（M2 の書き換えで畳んだ） |
| ⚠️1〜6 | マクロ展開内は未測定／`#[allow(unfulfilled…)]` 迂回は据え置き／条件の対が 3 か所にある／build-commands の現在形／トレイ分類は射程外／`plan.md` の最終項目 | いずれも据え置きを支持。最後の 1 件はこの節の直後に閉じる |
...
- ~~エイリアス経由の呼びが捕まるか~~ — **測定済み（2026-08-18）**。`let f = Engine::config;` のパス式も捕まる（注入して当該行が error）。射程の記述は `clippy.toml` 群 3 と `src-tauri/CLAUDE.md` を「メソッド呼びの形だけではない」の下限主張へ更新済み。**PR 本文へ送らない。**
...
- [x] 承認済み — 2026-08-18 / 問い: "`workspace/plan.md` を読んで、**注釈を追加する**か**明示的に承認**してください。" / 回答: "OK"
