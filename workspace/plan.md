...
### Phase 7 — レビュー ラウンド 4（絞り込み）: **High 0 / Medium 0 / Low 1**・**収束**

窓を外した差分は、レビュアが名指しした再実行条件に該当したので、そこだけ絞って再実行した。

- [x] 退行なし——プローブ 44 入力がラウンド 3 と**不一致 0**。sticky の `lastIndex` を跨いだ
      副作用も無し（毎回代入してから `exec`、失敗時は仕様上 0 に戻る）
- [x] ハッシュの上限が消えたことを接頭辞 2 種 × 9 水準（〜1000）で確認。**窓版では 23 以上が緑**
- [x] 縛りは弱まっていない（変異 3 種で実測。新カナリアは窓版の回帰で 23/40/255 だけが落ちる）
- [x] **Low**: `blankRustNonCode` の doc が、間に入れた const 宣言によって**関数から外れていた**。
      const とその doc を前へ移動（挙動不変ゆえプローブの再実行は不要——レビュアの判定に同意）。
      **どの機械検査も見ない欠陥である**——`governance:check` は見出し参照の着地しか見ず、
      hook は `.mjs` の doc の掛かり先を見ない
