...
### Phase 7 — 委譲レビュー 3 巡目への対応

3 巡目（`workspace/verify-1179-round3.txt`・アンカー `3bff387b`）は High-2 / Low-2 の解消を独自に実測し、
**検知器の宣言が実態より広い**ことを 1 件（High-3）と、誤検出 3 形を出した。

- [x] **High-3**: `$repoRoot` の鎖は閉じたが `$ExePath` の鎖は開いていた。`param()` の既定を
      直書き絶対パスへ戻すと `if (-not $ExePath)` が偽になり**導出が一度も走らない**——
      「どう書かれているか」を見る述語は**走らない形**に原理的に届かない（変異 I）。
      **しかも D/G より悪い**: D/G は cwd が別コピーのときにしか出ないが、**I は cwd = worktree
      （`npm run` と同じ最も有利な条件）でも出る**。それでも Pester 131・vitest 901・smoke が全緑だった
      → **入口を閉じた**（`param()` の既定が空であることを見る `Should` を追加）＋**宣言を実態へ合わせた**
      （「鎖として閉じている」という閉包の主張をやめ、「守るのは既定の枝が導出を通ること」と
      **肯定形で**書き、`$ExePath` の後続上書き（変異 J）を**受容する残余として名指した**）
- [x] **Low-1（誤検出 3 形）**: コメント 1 行の追加で赤くなり文言も事実に反する形（N）、
      コロン記法 `-RepositoryRoot:$repoRoot`（L）、引数順の入れ替え（M）
      → コメント行を母集団から除き、コロン記法も受け、**件数の縛りをやめて全呼び出しへ課す**形へ。
      「ちょうど 1 件」は誤検出を生むだけで 2 件になったとき何が壊れるかを言えない（⚠️-D も同時に解消）
- [x] **⚠️-E**: `-ProfileDir` の 2 枝で `GetFullPath` の版が割れていた（既定だけ 1 引数版）。
      1 引数版は `Environment.CurrentDirectory` で解決し PowerShell のカレント位置とは別物
      → 両枝とも 2 引数版へ揃え、基準だけを違える（明示値は cwd・既定は repoRoot）
- [x] **検知器を両側から測る**——守るべき退行 5 形と正当な変更 4 形すべてで期待と一致（下表）
- [x] Phase 7 の差分で `test:powershell` 131 / `npm test` 901 / `governance:check` が緑。
      既定の `-ProfileDir` が Medium-1 の修正を保つことを `LastWriteTime`（19:53:07 → 20:24:42）で実測

#### 検知器の両側測定（複製ツリー・Pester の配線ごと・正版は緑）

| 足 | 期待 | 実測 | 内容 |
|---|---|---|---|
| D | 赤 | 赤 | 導出を cwd 起点へ |
| G | 赤 | 赤 | 使用点だけ cwd 起点へ |
| M-add | 赤 | 赤 | 正しい導出の後ろに再導出を足す（後勝ち） |
| **I** | 赤 | **赤** | **`param()` の既定を直書きへ戻す（#1179 の逐語的復活）** |
| K | 赤 | 赤 | 導出行をバッククォートで折る（**宣言済みの形の要求**） |
| **J** | 緑 | **緑** | **導出の後で `$ExePath` を上書き（宣言済みの残余と一致）** |
| L | 緑 | 緑 | コロン記法（正当） |
| M-swap | 緑 | 緑 | 引数順の入れ替え（正当） |
| N | 緑 | 緑 | 関数名に触れるコメントを 1 行足す（正当） |

**J が緑であることは失敗ではなく、宣言が実態と一致していることの実測である。**

### Phase 8 — 委譲レビュー 4 巡目（最終巡）への対応

4 巡目は**ブロックなし**で返った（閉じる条件を brief に前もって書いた）。新しい迂回 2 件（P・Q）と
誤検出 2 件を出したが、委譲先自身が「述語をもう 1 段足すのではなく宣言を下限主張へ弱めよ」と推奨した。

- [x] **P（`param()` の既定を行継続で折って次行へ直書き）は閉じた**——述語を足さずに、既存の判定を
      「`=` の右が空になる」から「**空リテラルが在る**」へ精密化しただけである。P は `npm run` と同じ
      最も有利な条件でも症状が出る足で、閉じる価値があった
- [x] **Q（`Set-Variable` で差し替え）は閉じない**——ソースの形を見る述語の**原理的な天井**であり、
      `Should` を足すたびに「その形以外」が残る。委譲先の推奨どおり、**宣言を下限主張へ弱めた**
      （「覆うのは各点の代表的な書き方だけで、同義の別構文は射程外」）
- [x] **宣言を伸ばさず縮めた**（⚠️-H）。宣言は 0 → 13 → 20 → 27 行と巡ごとに伸びており、
      `AGENTS.md`「保証を宣言する文の隣に、保証されていない一覧を置かない」に照らすと**長いほど
      「全部守られている」と読まれる面積が広がる**。閉包の主張をやめ、17 行へ縮めた
- [x] **誤検出 2 件（行末コメント・`$null` 既定）を解消**——空リテラルの判定へ両方を含めた
- [x] **⚠️-G**: 件数検査の `-Because` が実際の働きより強かった → 「呼び出しが 1 つも無くなっていないこと」へ
- [x] **検知器を 11 本で両側から測る**（守るべき退行 6 / 正当な変更 5・NG 0 件）
- [x] `test:powershell` 131 / `npm test` 901 / `governance:check` が緑

| 足 | 期待 | 実測 | | 足 | 期待 | 実測 |
|---|---|---|---|---|---|---|
| D 導出を cwd 起点へ | 赤 | 赤 | | **Q** `Set-Variable` で差し替え | **緑** | **緑** |
| G 使用点だけ cwd 起点へ | 赤 | 赤 | | **J** 導出後に上書き | **緑** | **緑** |
| M-add 再導出を足す | 赤 | 赤 | | c1 二重引用符 | 緑 | 緑 |
| **I** 既定を直書きへ | 赤 | 赤 | | c4 行末コメント | 緑 | 緑 |
| **P** 既定を折って次行へ | **赤** | **赤** | | c5 既定を `$null` | 緑 | 緑 |
| K 導出行を折る | 赤 | 赤 | | | | |

**Q と J が緑であることは失敗ではなく、宣言が実態と一致していることの実測である。**

- [x] **否定の知識を ADR へ回収**（`workspace/` は squash マージで main に残らないため、
      ここで拾わなければ却下理由が永久に失われる）→ `docs/adr/ADR-harness-executable-path-derivation.md`。
      収めたのは (1) `--target-dir` の却下（`CARGO_TARGET_DIR` 追随の約束を壊す）、
      (2) 明示相対 `-ExePath` を re-root しない判断、(3) 述語を 5 呼び出し元へ広げない判断、
      (4) 同義の別構文まで閉じない判断。**採用案の再説明は書かない**（コードと doc が持つ）
