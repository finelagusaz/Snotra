...
| 純追記の扱い | **外す** | 発火 41〜46% → 24〜25%。消えるのは小さな発火ばかり（件数は 26% しか減らない） |
...
- [ ] `dependents.mjs` に逆引き索引（`(対象パス, 正規化ラベル) → 参照箇所[]`）を作る。参照の抽出は `refScanLines` と `G-heading-refs.mjs` の `HEADING_REF` を再利用し、**正規表現を 2 本目として持たない**
- [ ] 節の行範囲を返す関数を置く（未確定 1 の結論に従う）
- [ ] 変更行 → 該当する節 → 依存箇所 を返す関数を置く
- [ ] `dependents.test.mjs`: 節を書き換えたら依存が出る／別の節なら出ない／依存 0 なら空／前方一致で複数節に当たる場合の扱い
- [ ] `npm test` が緑

### Phase 2 — hook から呼ぶ（subprocess）

- [ ] `dependents.mjs` に CLI 入口を置く（`node scripts/governance/dependents.mjs <rel>` で、その編集に対する報告を stdout へ）。**`git diff HEAD -U0` の実行もこの中に置く**——hook 側に判定を持たせない
- [ ] `post-edit.mjs` から `root` 基準で spawn する（`runCheck` と同じ経路。**静的 import を足さない**——`try { main() } catch` の外で落ちる経路を作らないため）
- [ ] スクリプトが**無いツリー**（この機構より前に凍結された worktree 等）では、hook を落とさず静かに何もしない。ただし**その静けさが沈黙 = 合格を壊さない**ことをテストで固定する
- [ ] `post-edit.test.mjs`: 発火する／純追記では発火しない／`.rs` では発火しない／依存 0 では発火しない／**スクリプト不在でも hook が JSON エンベロープを返す**
- [ ] `npm test` が緑

### Phase 3 — 文書

- [ ] `docs/hooks.md` の散文へ追加し、**「この reminder の不在は『依存が無い』を意味しない」**を明記（発火一覧の表には行を足さない）
- [ ] 偽になる散文 4 か所を直す（ルート `CLAUDE.md` L29・`docs/build-commands.md` L32/L167・`.claude/rules/governance-docs.md` L23）
- [ ] `docs/adr/ADR-dependents-reminder-at-edit-time.md` を書く（却下した案と実測値）
- [ ] **同じ概念で grep し直し、直し漏れが 0 件であることを確かめる**
