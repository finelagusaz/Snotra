...
- [x] **3 巡目の指摘（Low 2 件）を反映する**——枠組みを変えて依頼した（所見の列挙ではなく
      「各主張を偽にする手順を書き、実行して偽にできたものだけを挙げる」形。**自分の修正差分が
      対象だと同じ枠組みは収束しない**という実測記録があるため）
  - **3-1（`docs/architecture.md` の `**` が奇数個で以降の強調が反転する）** → **自分で先に発見し
    `3a0eee9` で修正済み**だった。レビュアも独立に同じ欠陥へ到達した。**どの機械検査も見ない**
    （`governance:check` は 19 検査すべて green のまま素通りした）
  - **3-2（`instant_prefix` の doc の「#1032 の射程の外」が規範の文言と食い違う・⚠️）** → 反映。
    **レビュア自身の 1 巡目の言い回しが逐語で doc へ入っていた**という形で、
    ルート `CLAUDE.md`「所見が正しくても、そこに添えられた機序の説明は独立に誤りうる」の実例である。
    `src-tauri/CLAUDE.md` の一次文言を自分で読んで裁定した——「射程は読みだけである」は
    **読みと書き込みの別**であって頻度の別ではなく、認められた例外は `commands/` の操作時の読みだけ。
    `instant_prefix` は `egui_shell`＝UI 層ゆえ**規範の対象内・未移行**が正しい。「エッジ駆動だから
    射程の外」と読めると、`engine.lock()` 越しの config 読みを新設してよい先例に使えてしまう
- [x] **4 巡目（枠組み変更版）の指摘を反映する**——「各主張を偽にする手順を書き、実行して偽にできたものだけを挙げる」形。
      **7 主張のうち 2 件が偽になった**（5 件は手順を実行して偽にできず）
  - **主張 1 が偽（Medium）** → 反映。`armed ∧ pending == 0` が「解けるのは trailing 発火・`cancel`・
    `invalidate` の 3 つ」という私の列挙は**閉じていなかった**。`consume_reset_pending`
    （`launcher_controller.rs:986`）が `Debouncer` を**丸ごと作り直す**ため、`poll` も `cancel` も
    通らずに armed が落ちる（`Debouncer::new` は `armed: false`・`layout.rs:434`。自分で
    `rg "search_debounce\s*=|Debouncer::new"` して確認）。**#1039 への含意が重い**——`armed` を
    `SearchState` の内側へ移すとき reset 経路で落とす責務も移さないと、hide を跨いで未反映が残る。
    いま取り残しが起きないのは `Debouncer` ごと捨てているからにすぎない。doc へ追記した
  - **主張 4 が偽（Low）** → 3 巡目の 3-2 と同一で `7d080a6` で反映済み。**ただしレビュアの修正例に
    あった「毎フレームの読み 2 件（read_window_width / max_results）」の数は採らなかった**——
    数え上げは足すたびに腐る（`AGENTS.md`）
  - **主張 3（⚠️・「終端」より「上端」）** → 反映。他の `invalidate` 経路はこれより早く閉じる
  - **主張 6（⚠️・「この 2 つ」という数）** → 反映。「これら」へ替えて数を落とした
  - **枠組みの効果**: 主張 1 は**レビュア自身が 3 巡目まで 1 度も踏み込まなかった経路**から出た。
    所見を数える形では 3 巡とも `on_input` / `poll` / `invalidate` の周辺しか見ておらず、
    「**その変数を変える方法をコードの側から全列挙する**」（＝メソッド名ではなくフィールドへの
    代入を grep する）を手順として課して初めて出た
- [x] **5 巡目（最終）の指摘を反映する**——**4 主張のうち 1 件が偽**（3 件は偽にできず）
  - **「#1032 が実際に移したのは毎フレームの読みで、こちらはエッジ駆動ゆえ優先されなかった」が偽** →
    反映。`git show 8182cb5 -- src-tauri/src/egui_shell/` を自分で実行し、**show 経路の
    `general.ime_off_on_show`（フレーム外）とフォント登録の `visual.font_family`（起動時 1 回）も
    同じコミットで移っている**ことを確認した。ゆえに「エッジ駆動だから残った」は成り立たない。
    **理由を書くのをやめ、確かめられる事実（移設に入らなかった・理由はコミットから読めない）だけにした**
  - **⚠️ 1 語** → 反映。「認められた例外は**読みの中では** `commands/` の操作時の読みだけ」と限定を足し、
    切り出して引用されても崩れない形にした。例外の**理由**（別目的で同じ錠を既に取る）に当たらないことも明示
  - **この 1 件の出所**: レビュアが 4 巡目に「修正例」として渡した文言（「毎フレームの読み 2 件」）である。
    私は `7d080a6` で**規範の文言だけは一次証拠に当てた**が、**同じ文言の別の節（#1036 のコミット範囲）を
    検算対象に入れていなかった**。ルート `CLAUDE.md`「採るのは所見であって機序ではない」の射程は
    **渡された文言のすべての節**に及ぶ——節ごとに出典が違えば、検算も節ごとに要る
