...
- [ ] `snotra-core/tests/path_query_cost.rs` に `measure_path_query_frame_cost_at_operating_point`
...
- [ ] `sorted_by_path` は `PathStore` の private ゆえ crate 外から読めない。**読めるものへ
      置き換える**か、`#[doc(hidden)] pub` な既存の口が無いか調べて決める（未確定 1）
- [ ] 関数 doc に次を書く: 既存ハーネスと**判定を混ぜない**理由 / 実 `%APPDATA%` を直接読むこと /
      `load_or_scan_with_stats` が旧版を書き換えうること（現行は v7 ゆえ起きない）
- [ ] **この計器が単独で捕まえる経路を doc に数え上げる**（`docs/development-principles.md`
...
- [ ] **この計器が見ないものも doc に書く**（同「検証の層と、層と層の隙間」）:
...
**以下は測定でしか解消しない。** #1059 の plan.md（`2758340`・未確定 1〜5 が同じ形）と同型であり、
Phase 2・3 の実行中に `- [x]` へ倒して測定値・判断・却下理由を本文へ反映する。

- [ ] **11.4 ms の内訳** — Phase 3 の ablation で測る。支配項を名指しできる状態にする。
      過剰決定の検算 (i)(ii) が合わなければ、`research.md` §2.6 の第 4 の候補（`TopK::merge` /
      `into_sorted_vec`）を切り出す変異を追加する
- [ ] **実運用点の実額と 2 要因の寄与** — Phase 2 で測る。`sorted_by_path = false` が
      tie-break をどれだけ重くするか、`substring` が name 側をどれだけ軽くするか。
      **符号が予測できないので両方向を記録する**
- [ ] **PATH マージ後の `c:\` のマッチ件数** — Phase 1 の出力で確かめる。「全件」でなくなるなら
      その行の性格が変わるので、記録の文言を直す
- [ ] **判定: 削る手段が在るか** — Phase 3 の分解を見て決める。**在れば**手段（候補は
      「`extend_with_roots` が旗を下ろすのをやめる——マージ後に測り直す、または整列を保って
      併合する」「上の第 3 案: index → boost の疎な表」）を選び、**この計画を書き換えて**
      実装フェーズを足す。**無ければ** `PERFORMANCE.md` + ADR へ却下を記録する。
      どちらへ転んでも Phase 4 の記録は実行する
- [x] **Phase 5（spike 撤去）を本 PR に含めてよいか** → **含める。** 2026-08-13 の承認は
      Phase 5 を含む計画に対して与えられた（承認の問いで名指しして提示した）。
      撤去条件「本 issue の判定を記録した PR がマージされたら消す」は PR #1066（`2758340`）の
      マージで発火済みである
...
- [x] 承認済み — 2026-08-13 / 問い: "**`workspace/plan.md` へ注釈を追加する**、または **計画を明示的に承認する** のどちらかをお願いします / 承認の範囲は **「計器を足して測ること」** に限らせてください。判定後の issue クローズや実装は外向き・不可逆ですので、実測をお見せしてから改めて指示をいただきます / 未確定が 1 件残っています — **Phase 5（#1059 spike 足場の撤去）を本 PR に含めてよいか** / お含みおきください — この計画は #1059 と同型で、**測定でしか解消しない未確定を残したまま workspace をコミット**します" / 回答: "OK"
  ...
  - **Phase 5 は承認に含まれると解した**（問いで名指しして提示し、異議が無かったため）。
    違っていれば Phase 5 を落とす
