...
- [x] **レビュー ラウンド 3**（枠組みを変えた——**開かれた散文レビューをやめ、偽にできる問い 3 つ**に絞り
      「この結論を偽にする手順」を書かせた。理由はメモリ `review-frameworks-not-rounds`——自分の修正差分に
      同じ枠組みを当て続けると収束しない）— **Medium 2 / Low 1 / ⚠️ Low 1。問い 3（ラウンド 2 の修正が
      新しく偽にしたものはあるか）は所見なし**。全件を一次証拠で裁定し採用:
  - **Medium 1（この差分が持ち込んだ唯一の欠陥）**: Low 4 の修正で**書き直した文を挿入し元の文を消して
    いなかった**——`config_watcher` の射程外の文が同一条項に 2 本。`grep -c` は 1（同じ行ゆえ）だが
    **`grep -o | wc -l` が 2** を返す。削除し、**同じ道具で 1 になることを実測**（本サイクル 3 度目の同型）
  - **Medium 2（列挙が覆い切っていない）**: 「イベントループスレッドで走るのは…`on_event_loop` が
    **インラインへ倒れる** listener の本体」は外延が足りない。一次証拠（`snotra-egui-runtime/src/proof.rs`）で
    確認——**`on_event_loop` はインラインでも post でもクロージャをイベントループスレッドで走らせる**
    （インラインか post かは**いつ**であって**どこで**ではない）。ゆえに**自分が先例として挙げた hotkey
    （platform スレッドからの emit ＝ post 側）が自分の規則から漏れていた**。加えて tao の
    window-event リスナー（`mod.rs:373-378` の `Moved` → `position_results_below_main` が `window_gap` を
    `read_config` で読む）も漏れる。**列挙をやめ、非自明な 2 つを明示して正本（`proof.rs`）を指す形へ**
  - **Low 3**: 「1 フレーム 1 lock」の言い換えが 4 か所（`view.rs:560,636` / `results_view.rs:537` /
    `launcher_controller.rs:787`）取り残されていた → すべて「読み」へ。**厳密には偽ではないが、
    本サイクル最初のコミットで兄弟 1 件だけを言い換えていた**
  - **⚠️ Low 4**: `main.rs` の「`read_config` が**錠そのもの**を避ける」（既存文）→「engine の錠を避ける」
  - 残る一致は `docs/adr/` と `docs/superpowers/plans/` のみで、**どちらも凍結された歴史ゆえ対象外**
