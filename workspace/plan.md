...
| `.claude/agents/code-reviewer.md` | 変更（Phase 2・**D4 承認済み**） | `## Phase 1: 実装検証` へ観点 1 行 |

計 12 ファイル（新規 1・変更 11）。`scripts/governance-check.mjs` / `scripts/governance-check.test.mjs` / `docs/build-commands.md` は **D2 見送りにより対象外**。
...
`docs/comment-guidelines.md:21` の「**数えず列挙せず、正本をリンクで指す**」を次のとおり置き換える（**腐るのは数であって名指しではない**という区別を入れる。**段数と序数は恒久文書へ書かない**——初稿が段数を書いたまま段を足し、code-reviewer の H1 がそれを捕まえた。**以下この計画に残る「N 段目」は plan が消えるまでの便宜であり、恒久文書へ持ち出さない**（序数は段を 1 本挿入すると黙って別の段を指し、合わないことすら見えない・R2-L4））。
...
- [x] 上記へ書き換える。**模範例 `Engine::new_from_cache` の記述は消さない**（「版の番号を書かない」と宣言しつつ正本を名指しする形は、精緻化後の条項の模範例としてそのまま生きる）

### 1b. 新節「日本語の折返し」

- [x] `## rustdoc の様式` の後（`## TSDoc の様式` の前）へ新節 `## 日本語の折返し` を置く。**見出し名はこの字面のとおりにする**（Phase 2 の rule がこの名前で着地する）。内容:
...
- [x] `## 言語（日英）` へ次を足す。**repo が自足する形で書く**（人間の寄稿者と、global 設定を持たないエージェントの両方に効かせるため）
...
**射程規則を置く根拠（実測）**: 「窓」は `.rs` コメントに 290 行あるだけでなく、**リポジトリ自身の規範文書に定着している**——**出現数**で `src-tauri/CLAUDE.md` 43 対 `ウィンドウ` 15、`snotra-egui-runtime/CLAUDE.md` 13 対 2、`docs/architecture.md` 8 対 8（2026-08-08 実測。`grep -o | wc -l`。**行数で数えると 27 対 13 / 8 対 2 になる**——結論は変わらないが、数え方を書かない数値は再検証できない）。カタカナを要求する条項を書くと**その瞬間に自分の `CLAUDE.md` が違反になる**（`AGENTS.md`「検証の作法」の「実装より強い主張になった瞬間に嘘になる」）。

**「畳み込み」は `snotra-core/CLAUDE.md` が規範として使っている**（「畳み込み比較を別実装で書き起こしてはならない」）ので定着語＝対象外。この判断は維持する。

**訂正（code-reviewer の H2・2026-08-08）**: 当初ここに「**「被覆」は規範文書に 1 件も無い**（`.md` のヒットは `.superpowers/` の履歴資料のみ）」と書いたが**これは偽である**。`SPEC.md:94` と `SPEC.md:647` に計 5 か所あり（`grep -c "被覆" SPEC.md` = 2 行 / `grep -o` = 5 か所）、`SPEC.md` は `governanceDocs`（`scripts/governance-check.mjs:1342`）の母集団に入る規範文書である。**誤りの機序は `grep -rln … | head` で出力を切り詰め、その切り詰めた一覧から全称否定を書いたことである**（`AGENTS.md`「検証の作法」の「切り詰めた出力から導いた断定」そのもの）。**帰結**: 「被覆」と「カバー」はどちらも定着語であり、射程規則の定着テストは解を返さない。ゆえに 1c へ tie-breaker（「訳語が複数定着している語義では『迷ったら訳さない側へ倒す』が決める」）を足した。**Phase 3c の `font_stack.rs` の修正は妥当なまま**——同ファイル 15 行目が既に「カバー」を使っており、条項が定着語について強制する唯一の項目（同一ファイル内で混在させない）を満たす方向である。**受容する残余**: `SPEC.md:647` は as-built の記述として「被覆判定」のまま残り、実装側 `//!` は「カバー」になった（共通の錨は `font_covers_cjk`）。
...
- [x] Phase 1 の完了時点で `npm run governance:check` を実行する（節を足したので `G-heading-refs` の既存参照元が壊れていないこと・`G-stale-identifiers` が条項本文で赤くならないこと）。**`cargo doc` はこの Phase では不要**（`.rs` を触っていない）

**実装中に判明して追加した条項（1a への追記）**

- [x] **リンク形が効くのは `///` と `//!` だけである**ことを 1a の 2 段目へ書き足す。Phase 3c で `indexer.rs:811`（`//` のインラインコメント）へ `` [`LoadOrScanResult::cached_masks`] `` と書いた直後に気づいた——**rustdoc は `//` を読まないので着地は誰も確かめず、括弧が「検査されている」という誤った安心だけを与える**。条項に書かないと、同じ誤解が「リンクにしたから安全」の形で再生産される（当該コメントは素のバッククォートへ戻した）

---

## Phase 2 — 配送の穴を塞ぐ

- [x] `.claude/rules/comments.md` を新規作成する。**内容は下記のとおり**（要約を置かず正本を指すだけ。既存 router 3 枚と同型。**664 字と実測済み**）
...
- [x] `npm run governance:check` を実行し、`G-rules-globs`（4 本すべてが実在ファイルにマッチ）・`G-heading-refs`（5 本の参照が着地）・`G-area-budget`（rules 面 ≤ 12000 字）が green であることを確認する。**面積は 10470 → 11134 / 12000（余裕 866 字）と実測済み**。超えたら本文を削る（上限を上げない）
- [x] **配送の実測（完了・2026-08-08）**——4 glob すべてが `DELIVERED`、いずれも既存 router と**同時配送**（`BOTH`）で隠していない。成果物は `workspace/rules-delivery-verification.md` と同 `-2.md`。実測の内訳: `snotra-core/src/config.rs` → `comments.md` + `snotra-core.md`、`snotra-egui-runtime/src/repaint.rs` → `comments.md`（**今まで rule が 1 枚も無かった crate へ到達**）、`src-tauri/src/icon.rs` → `comments.md` + `src-tauri.md`、`snotra-settings/src/style.rs` → `comments.md` + `snotra-settings.md`。**重複排除はファイル単位で 1 セッション 1 回**と一次証拠で確認され、ゆえに 1 context で測れる glob は 1 本だけ——4 本を閉じるのに新鮮な context を 4 つ要した（最初の計測手順はこの制約を見落として判定を混同しかけ、サブエージェントが指摘して測り直した）。**`snotra-settings` の 1 本だけは孫エージェントの報告であり主エージェントの一次証拠ではない**
- [x] ~~当初の 2 枚読み手順~~——**同一セッションでは 2 本目が重複排除に隠れて測れない**ため上の 4 context 方式へ差し替えた。切り分けの規則（両方届かないなら harness がセッション途中の新ファイルを見ていない）は結果として不要になった（`comments.md` は 4 context すべてで実際に配送された）:
...
- [x] `.claude/agents/code-reviewer.md` の `## Phase 1: 実装検証` へ観点を 1 行足す（**D4 承認済み**）。**内容は「コメント規約（`docs/comment-guidelines.md`）に照らす」ではなく、条項のうち機械が見ないものを名指しする**——`docs/comment-guidelines.md`「第一原則」の**書かないもの**（コードが決めている構造の事実）に当たる記述が新規コメントに無いか。**要約を書かず正本を指す**（`.claude/agents/` は `G-area-budget` の母集団外だが、サブエージェント起動ごとに必ず読まれる面である）
...
- [x] `snotra-egui-runtime/src/proof.rs:43-44` — `current_thread().id() == context.main_thread_id`
- [x] `src-tauri/src/egui_shell/results_window.rs:216-217` — `expected ResultsScale, found MainScale`
- [x] `src-tauri/src/egui_shell/strings.rs:48-49` — `t("search.placeholder.folder", { dir: fs.currentDir })`
- [x] `src-tauri/src/icon.rs:754-755` — `cargo test -p snotra --release icon_pipeline_cost_probe -- --ignored --nocapture`
- [x] `snotra-settings/src/style.rs:102-103` — `ScrollArea::vertical().auto_shrink([false,false]).scroll_source(drag:false)`

**やり方**: そのコードスパンが 1 行に収まるよう改行位置を動かす。**文言は変えない**（変えると「何を直したか」が読めなくなる）。段落全体を 1 行へ寄せるのは、その段落に触るときだけでよい（1b の適用範囲どおり）。

- [x] 修正後、`node <scratchpad>/measure-split-span.mjs` を再実行して **0 件**を確認する

### 3b. 散文の数え上げ 3 件（D3 の合意が前提）

- [x] `snotra-core/src/search/build.rs:83` — 「通る経路は 2 つ（`new_from_tree` と、`new_with_cached_masks` の v3 フォールバック腕）」。**数を落とし、名指しを intra-doc link 形へ**（`` [`SearchEngine::new_from_tree`] `` 等）。直後の「1 本に寄せてあるのは〜」の理由文は残す
- [x] `snotra-core/src/search.rs:331` — 「**呼び出し元は 2 つで、どちらも明示の操作である**」→ 数を落とし「**呼び出し元はどちらも明示の操作である**」。`/r` とトレイの名指しは**残す**（直後の「頻度を書くなら呼び出し元を名指しする」がその理由の正本）
- [x] `snotra-egui-runtime/src/proof.rs:24` — 「構築点は 2 つだけである: …」→ 数を落とす。「**3 つ目**を足すときは」→「**足すときは**」（数を消したので序数も外す）

### 3c. 独立導出が見つけた 2 件（本レビューで追加・根拠は主エージェントが再照合済み）

- [x] `snotra-core/src/indexer.rs:811` — `// マスクを計算してキャッシュに含める。起動時に SearchEngine::new_with_cached_masks() がマスク再計算をスキップできるようにする。`。**呼び出し元＋到達可能性の写しであり、しかも反復 11 の事実より狭い**（`derive_columns` は保存経路と cache-miss 経路の両方が通る。正本は `snotra-core/CLAUDE.md` の「記録側が潰したものを、cache-miss の枝がそのまま索引の表現に使う」と `LoadOrScanResult::cached_masks` の doc）。**消費者を数え直すのではなく、正本を指す形へ書き換える**（1a の条項どおり）
- [x] **実装中に判明**: `src-tauri/src/egui_shell/font_stack.rs:7` の「フォント登録は **3 枝**（#532 SU4 の 2 枝に #689 が 1 枝を足した）」も**同じ段落に在った数え上げ**である（1a が禁じる型。語彙依存の regex が「N 枝」を拾えず取りこぼしていた）。**数だけ落として列挙と issue 参照は残した**——「フォント登録の枝は次のとおり（#532 SU4 の形に #689 が 1 枝を足した）」。触った段落なので 1b に従い 1 行へ寄せた
- [x] `src-tauri/src/egui_shell/font_stack.rs:8-9` — 「CJK 非被覆なら」「解決し**被覆するなら**」の**「被覆」2 件を「カバー」へ**。font coverage の語義。**根拠は訂正済み**（当初「規範文書に 1 件も定着していない」と書いたが `SPEC.md` に 5 か所あり偽だった。正しい根拠は 1c の tie-breaker＝どちらも定着しているので「訳さない側へ倒す」＋同ファイル 15 行目が既に「カバー」ゆえ混在解消の方向。詳細は 1c の訂正欄）。**issue の「単語をより自然に」に対する唯一の実適用**である

- [x] **実装中に判明（逆向きの監査）**: 数を落とす操作が**不変条件を 2 件弱めていた**。(1) `proof.rs:24` の「構築点は 2 つ**だけ**である」の「だけ」は網羅性の主張で、`EventLoopProof` の private フィールド + `PhantomData` による健全性を支えている——数と一緒に落としたので「構築点は**ここに挙げるものに限る**」へ言い直した（1a の 4 段目が許す一意性の宣言の形）。(2) `search.rs:331` の「**どちらも**」は暗に 2 を含み、3 つ目の呼び出し元が現れると嘘になる——「**いずれも**」へ変え、数に依存しない全称の要求にした。**数を消す作業は、数が運んでいた網羅性・一意性の主張まで一緒に消しうる**（消した行の不変条件を名指しして再確立地点を探す監査で見つけた）
- [x] `cargo doc --workspace --no-deps --document-private-items` を実行する（**doc コメントを触ったので必須。PostToolUse hook は沈黙する**）
...
## code-review 結果（Step 4b・成果物 `workspace/code-review-732.md`）

Critical 0 / High 2 / Medium 1 / Low 3 ＋ ⚠️ 7 件。**High 2 件と Medium 1 件はいずれも根拠を主エージェントが再照合して真と確認し、修正した**。

- **H1（真）**: 条項本文が「次の **3 段**」と書きながら 4 段あった。**自分が新設した「数を書かない」条項を、その条項本文が破っていた**。→「次のとおり」へ直し、**この列に数を書かない**理由を条項へ書き足した（`plan.md` 側の「3 段へ置き換える」も同時に訂正）
- **H2（真・主エージェントの失敗）**: 「『被覆』は規範文書に 1 件も無い」は**偽**。`SPEC.md:94` と `:647` に計 5 か所あり、`SPEC.md` は `governanceDocs` の母集団に入る。**機序は `grep -rln … | head` で切り詰めた出力から全称否定を書いたこと**。→ 1c へ tie-breaker（複数定着なら「訳さない側へ倒す」）を追加、訂正と残余（`SPEC.md:647` は as-built ゆえ据え置き）を記録
- **M2（真・同一パターンの全コードパス検索の成果）**: `snotra-core/tests/path_query_cost.rs:231` が「空クエリ（窓を開いた瞬間・クエリ消去時）に走る」と書いており、**`search.rs:333` が「2 度招いた」と記録した誤読の 3 例目が同じ crate に生きていた**。実装で接地（呼び出しは `/r` とトレイの 2 経路のみ・空クエリは `launcher_controller.rs:765` で早期 return）して真と確認し、頻度の主張を外して正本へ寄せた
- **L1（真）**: `code-reviewer.md:28` の `同「日本語の折返し」` は、この差分が自分で禁じた略記で**どちらの検査も見ない**。→ 正準形へ展開（見出し参照 174 → 175 件）
- **L3（採用）**: `font_stack.rs` の列挙に「なぜ列挙するか」が無く、1a の 3 段目で分類できない。→「index 0 の不変条件が枝ごとに同じであることを 1 目で見せるため」を追記
- **L4（採用）**: `plan.md` 内部の「3 段」/「4 段目」不整合 → H1 と同時に解消
- **⚠️1（採用）**: 「#689 が 1 枝を足した」が 1a 違反か履歴か決まらない → 条項へ「**過去の変化量は対象外**」を追記
- **⚠️2（採用）**: 「窓」の実測値に数え方が無く再現しない（出現数 43 対 15／行数 27 対 13）→ 数え方を明記
- **⚠️7（受容する残余）**: 配送実測 4 本のうち `snotra-settings` の 1 本だけ主エージェントの一次証拠が無い（孫エージェントの報告）。重複排除ゆえ現セッションからは再測できない

**レビュー後の再検証（`AGENTS.md`「条件別チェック（トリガー → 参照先）」の fix-forward 行）**: `cargo fmt` / `clippy` / `cargo doc` / `cargo test -p snotra-core` / `-p snotra` / `npm run governance:check` をすべて再実行し exit 0。

### 2 巡目（同じエージェントを `SendMessage` で継続・成果物 `workspace/code-review-732-round2.md`）

**Critical 0 / High 0。** 1 巡目の 6 所見と ⚠️ 2 件はすべて閉じたと再測で確認され（見出し参照 174 → 175 件・`同「` の残り 0 件・行またぎスパン 0 件・5 スパン全件 grep 可）、新規は Medium 2 / Low 4。**全件当てた**:

- **R2-M2（真・1 巡目の修正が作った穴）**: `path_query_cost.rs:231` の修正が**呼び出し元を再列挙**しており、正本を「いずれも」で数に依存しない全称へ寄せた直後にその性質を下流で打ち消していた。しかも `git log -S` で当該行の出自は `da73bcb`（#962）で、**その時点で既に空クエリの早期 return が在った＝主張は生まれた時から偽**（腐ったのではない）。→ 否定の主張・正本ポインタ・計測の意味づけだけ残し、列挙と機序の再説明を落とした
- **R2-M1（真）**: tie-breaker の残余の根拠「as-built だから書き換えない」が repo の規範から導けず、同じ bullet の適用範囲と将来衝突する（誰かが `SPEC.md:647` を別の理由で触った瞬間 2 文が逆を指す）。→「**今回の変更で触っていないので上の適用範囲どおり射程外**」へ寄せ、repo の規範から導ける形にした
- **R2-L1 + R2-L2（真）**: H1 を直した括弧が、**字面では同じ型を再発**（数を書くなと命じる文が「3 段」「4 段」を含む）。かつ「段」と「列」の語の混在。かつ「歴史メモの様式」が要求する issue 番号と日付を欠く。→ 1 手で 3 つ閉じた（「この箇条の段数を書かない——初稿は段数を書いてしまい…・#732 / 2026-08-08」）
- **R2-L3（真・レビュアー自身が持ち込んだ誤り）**: 「1 目で見せる」は語形の誤りで、正しくは「一目」（慣用句）。1 巡目の修正例が逐語で採用された形。→「一目」へ
- **R2-L4（部分・射程限定）**: 「段数は書かない」と宣言しながら序数参照 5 件が残る。**恒久文書には 0 件**で `workspace/` 限定ゆえ Low。→ 宣言を「段数と序数は恒久文書へ書かない」＋「plan に残る序数は消えるまでの便宜であり持ち出さない」へ

**レビュアーの自己訂正も記録に値する**: 1 巡目の観点 3（参照の着地）は見出しだけを母集団に取った proxy 測定であり、実際の `collectAnchors` は番号付きリスト項目と太字リード項目も集める。`collectAnchors` を import して測り直すと結論は変わらず、1 巡目が `NO LANDING` と誤判定した 3 本も実際には着地していた。**`AGENTS.md`「検証の作法」の「代理ではなく対象そのもので測る」を、レビュアー自身が破って自分で見つけた形である。**

**3 巡目は実施しない**（レビュアーの判定に同意）——当てた 5 件はいずれも文言 1 行で、検算する枠組み（`governance:check` と `cargo doc`）は既に走っている。2 巡目後の再検証もすべて exit 0。
