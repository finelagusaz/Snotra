...
#### code-reviewer の指摘と対応（4b・1 巡目）

全文は `workspace/code-review-1106.txt`。Critical 0 件。**逆向きの監査 3 点はいずれも「孤立した不変条件なし」**
（`git log -S` で #749 の判断の写しが 0 件であること、改名した検査の assert が新名へそのまま在ることを実測）。
意図的分離として渡した 5 件は 5/5 とも根拠が成立した。

| 所見 | 対応 |
|---|---|
| **H1**: `results_area_collapsed` を既存 doc ブロックの末尾へ挿入したため、`results_window_height` の doc（「`0.0` は hide の契約値……ここが正本である」を含む）が述語側へ移り、本体が doc ゼロになっていた。4 つの参照が宙に浮いた | **修正**（一次証拠で確認済み）。述語を関数の後ろへ移し、doc を元へ戻した。**どの機械検査も見ない種類の誤り**である——`cargo doc` は intra-doc link でないので緑、fmt/clippy/test も無関係 |
| **M1**: `docs/architecture.md` の該当行がゲート 1 つ・凍結値 1 つの位相のままだった。計画が「不要」とした理由（「窓の駆動の構造は変わらない」）が**対象を外していた**（当該行が語るのは起動ゲートの位相） | **修正**。2 ゲートが独立であること・凍結値が 2 つあることへ書き換えた |
| **M2**: 新規コメントが「配り先の列挙」を書いており、`view.rs` では**同じファイルの 5 行上が「配り先は数えない」と明示している**その直下だった | **修正**（`view.rs`）。`window_coordinator.rs` 側は `FrameIndexing` の doc と同型の役割宣言ゆえ維持 |
| **L2**: `ResultsInputs::max_results` の doc が読み点の制約を語らない（隣の 2 フィールドと非対称） | **修正**。制約と、その理由が隣の 2 つと違うことを追記 |
| **W1**: 述語の doc が「同値はテストが測る」と書くが、「production の第 2 引数は `Metrics::row_height` だけ」は**どのテストも測っていない**（規範） | **修正**。測られているもの（床・代表行高での同値）と規範を doc で書き分けた |
| **W3**: 検知器が `read_config(` を禁止語にしているため、将来の無関係な config 読みも落ちる | **メッセージのみ修正**。検査自体は維持——起動の入口での config 読みは読み直しと区別できず、落ちてよい。その旨をメッセージへ書いた |
| **L1**: `fn max_results` の再読みは `window_coordinator.rs` 内では検知器の母集団外 | **対応せず**（D2 の意図的設計。死角は `view.rs` の検査 doc が自認済み） |
| **W2**: `SPEC.md` の「行データと選択は消さず」と `on_enter` の flush 枝 | **対応せず**。主語はゲートであり「このゲートは行を消さない」は真。flush は別機構で、#1077 が §4.7 へ書いた同じ文言と揃っている。レビュア自身も到達可能性は未測定・低確信としている |
| **W4**: 検知器はコメント中のリテラルでも満たされる | **対応せず**（既知の死角として doc 化済み。現状コメントは括弧なしで、変異注入でも検知器は生きていた） |

#### 2 巡目（同じレビュアを SendMessage で継続）

1 巡目の 9 件は **7 件解消 / 2 件は未対応が妥当と検算された**（W2 の非対応根拠「#1077 が §4.7 へ書いた
同じ文言と揃っている」は、レビュア自身が `SPEC.md` を実読して**逐語一致**を確認）。新規は Critical 0・High 0。

| 新規所見 | 対応 |
|---|---|
| **N1（Medium）**: L2 の対応で書いた doc が `DriveResultsInputs` の「同名フィールド」を指すが、**その名前のフィールドは無い**（実際は `visible_rows`・型も違う） | **修正**。intra-doc link 形にして名前と型の不一致を明記した——**この 2 巡目が予測した失敗モード（修正が周辺に新しい誤りを生む）の実例である** |
| **N2（Low）**: `docs/architecture.md` の「構築子を**読み点へ**閉じた型」が機構より 1 段強い（実際に閉じているのはモジュール） | **修正**。既存の正確な言い回しへ揃えた |
| **N4（Low⚠️）**: `window_coordinator.rs` 側を維持した根拠を「`FrameIndexing` と同型」に置くのは弱い（既存を新規の基準にしてよいとは規約が言っていない） | **修正**。根拠を「名指しているのは呼び出し点ではなく**役割 2 つ**であり、その 2 つが同じ値を見ることがこの型の存在理由そのもの」へ置き直した |
| **N3 / N5（Low⚠️）**: 「上の行」という位置参照 / 同値テストが `n ∈ 1..=50` だけを回す | **対応せず**（前者は直近 5 行の指示語で誤読の余地が小さい・後者は式が単調ゆえ実害なし。どちらもレビュア自身が低確信としている） |

**1 巡目の H1 が新しい誤りを生んでいないことも検算された**——`layout.rs` の 3 つの doc がそれぞれ正しい対象に
付き、旧 doc が名指していた 4 参照も全部着地する。**むしろ強くなった**: 述語の doc が
`results_window_height` を intra-doc link 形で参照するようになり、**この 2 本は以後 `cargo doc` が
着地を検査する側に入った**（H1 が緑のまま通れたのは、素のバッククォートがリンクではなかったため）。
