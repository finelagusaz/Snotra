...
- [x] カテゴリ D の目視 6 項目を実施し、結果を下の「目視項目」表へ記録する（**6/6 PASS**。1〜4 はエージェント、5〜6 は人間が実施）
  - **当初「結果を PR 本文に残す」までを 1 項目にしていたが、それは計画が所有する作業ではない**——`gh pr create` は未チェック項目で塞がれるので、PR 本文を要求する項目を計画へ置くと循環して閉じられない（`.claude/rules/safety-nets.md`「CI の実測は計画の検証項目に置かず、PR 本文のチェックリストへ送る」と同型）。記録は本表が持ち、PR 本文への転記は PR 作成の手順に属する
...
| 5 | **サインアウト → サインインで実際に本体が起動すること**（この経路は誰もまだ測っていない） | **人間**——実行中のセッションを落とすため | **PASS（2026-09-01）** — 登録したパス（`target/debug/snotra.exe`）のプロセスが実行中。**対話ログオン 23:06:50 → プロセス起動 23:07:44（+53.9 秒）** で、ログオン処理が `Run` 値を読んで起動したことを時刻で裏づけた（debug ビルドゆえの立ち上がりの遅さを含む） |
| 6 | タスクマネージャーの「スタートアップ アプリ」に `Snotra` が現れること、および**そこで無効化したときにチェックボックスがどう見えるか**（残余 1 の実測を兼ねる） | **人間**——UI 操作の自動化手段が無い | **PASS（2026-09-01）** — 無効化しても `Run` 値は残り、`StartupApproved\Run` に `03`（無効・+ 無効化時刻）のレコードが作られた。有効へ戻すと `02` + ゼロ埋め。**残余 1 は予測どおり実測された**。この結果を受けて SPEC §7.7 / ADR 7 節 / `//!` の理由を「形式を測れない」から「そこは利用者の領分である」へ書き換えた |
