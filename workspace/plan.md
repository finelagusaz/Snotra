...
- [x] 変異注入（複製・`node_modules` は junction）で 3 種を実測。**(a) 純追記フィルタを外す → 赤（10 件中 1 件）／(b) 逆引き索引を 1 本の腕へ減らす → 赤（10 件中 2 件）／(c) hook の呼び出しを外す → 当初は緑のまま（0/96）**。(c) が配線の未検証を暴いたので統合テストを 1 本足し、同じ変異を再度当てて赤（1/97）になることと、**落ち方が「配線の欠落」由来である**こと（`buildEnvelope` が `null` を返し `JSON.parse` が落ちる）まで確かめた
- [x] 実測値を `docs/adr/ADR-dependents-reminder-at-edit-time.md` へ回収した（発火 44/80・中央 6 件・最大 37 件／hook の所要時間 386 ms 対 330〜351 ms）。**当初この項目は「PR 本文へ載せる」と書いていたが、コミット以降の作業を計画が所有してはならない**（`/start-issue`「Step 4 — 実装計画（workspace/plan.md）」）ので、計画が所有できる回収先へ替えた
