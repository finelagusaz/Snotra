...
- [x] **本 PR 自身の CI（`e2e.yml` の smoke job）が緑になることを確認し、ログの中身まで読む**（緑が「検査が走った」を意味しない・#686）。`e2e.yml` の `paths:` は個別ファイル名の列挙で、本 PR が触る 3 ファイルを含むため自動起動する（レビュー実測）
...
- [x] **(A1) `smoke-egui` の新設 2 検査が、遅い runner では沈黙のまま合格しうる配置だった** — `-RedirectStandardError` は**プロセス起動時に空のファイルを作る**ので `Test-Path` は真になり、本体がまだ 1 行も書いていない段階では `Select-String` が 0 件を返す（自分で実測）。固定待機 4,000ms の直後という配置は、CI で 3 回実測されている「起動後 12,000ms 経っても trace 0 行」を保証しない——**seed が壊れていても両検査が素通りし、赤は「`egui_results:show` 未観測」に化ける**（不変条件 4 の失敗様態そのものが、それを防ぐための検査で起きる）。**hotkey 観測（予算 25s）と `egui_show:done` 観測を通過した後・results 検査の直前**へ移し、ログが空なら赤にする guard を足した。不変条件 4 が要求するのは *results より前* という順序であって位置ではない。移動後に `smoke:egui` とフォールトインジェクション B を再実行して緑・赤を実測（`trace 行数: 11` ＝ proof-of-life を得てから判定している）。**`smoke-startup` にこの穴は無い**——待機ループの後で判定し、空ログは `$events.Count -eq 0` が独立に赤にする
