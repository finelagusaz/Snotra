...
## 目的【2026-08-27 の人間裁定で変更】

**当初**: `clamp_main_into_work_area` の抑止条件を「ポインタが押されていない」から「OS のモーダル移動ループ中でない」へ差し替える。

**現在**: **Q1 は取り下げた。** Phase 1 の計装と測定で拘束が再現せず、差し替えが no-op と判明したため。残る目的は次の 2 つである。

1. 1 度置いた計装で **Q2**（確定後に窓を動かしている経路の名指し）と **Q3**（逸脱の 2 仮説）に答える → **両方とも答えが出た**
2. **`SPEC.md` と rustdoc を実測へ直す**（裁定の逐語: 「Q1 を取り下げて SPEC と rustdoc を実測へ直して」）

**以下「設計」節と「変更ファイル一覧」は当初計画の記録であり、取り消し線の代わりに Phase 2 の但し書きで上書きしてある。** 実際に入った差分は Phase 2〜4 の作業項目が正本である。
...
### ⚠️ Q1 の「1 度だけ戻る」は、述語を直すだけでは閉じないかもしれない【的中したが、向きが逆だった】

**この懸念は当たっていたが、外れ方が予想と逆だった。** 「述語を直しても閉じない」ではなく
**「述語を直す必要が無かった」**——測定で拘束そのものが再現せず、`Enter` 後の 1 度だけの復帰は
**既に成立していた**。Q2 の答えが Q1 の真偽を決める、という依存の読みは正しかった。
...
> **【取り下げ】以下の「判定は `any_down()` を置換する」と「`SPEC.md` は…一般化する」は実装していない。**
> Phase 1 の測定で no-op と判明し、人間裁定で取り下げた（Phase 2 の但し書きが正本）。
> **記録として残す理由**は、`GetGUIThreadInfo` の実装可能性をここまで確かめた事実そのものが
> `ADR-modal-move-loop-clamp-suppression` 却下 1 の根拠だからである。
> **計装の設計（次々節）は実装済みで、いまも正本である。**

### 判定は `any_down()` を**置換**する【取り下げ】
...
> **【当初計画の記録】実際に入った差分は下表と違う。** 取り下げにより `view.rs` は無変更、
> `mod.rs` の再 export は不要（述語の消費者が同一モジュール内に閉じた）、`SPEC.md` は `:478` の
> 1 項のみ、`src-tauri/CLAUDE.md` と `scripts/manual-smoke.ps1` は**無変更**（ガードを変えないので
> 「ポインタ非押下のフレームに限る」は今も真）。**実際に触ったファイルは 4 つ**:
> `window_coordinator.rs` / `SPEC.md` / 新 ADR / `workspace/`。
...
- [x] 測定スクリプトを**リポジトリの外**（`C:/tmp/snotra-1194/measure-move-loop.ps1`）に書いた。`SnotraSmoke.psm1` の関数だけで組んだ
  - **実装中に判明**: `Initialize-SnotraDpiAwareness` は **export されていない**。exported な入口は `Get-SnotraWindowDpi`（内部で awareness を確立してから DPI を返す）
  - **実装中に判明**: 窓は不可視で作られるので、**ホットキーで出すまで `Wait-SnotraWindow` は通らない**（120s タイムアウトを踏んだ）。`hotkey:registered` を待って `Alt+Q` → `egui_show:done` を待つ順が要る
- [x] **対照ビルドは不要になった**——`egui_main:clamp` の `fired` 欄が「クランプが撃ったか」を直接答えるので、#1173 が対照で分離していた「頭打ちはクランプかカーソル限界か」を**計器 1 つで分離できる**。代わりに 3 条件（既定 / toast / `↓`+`↑`）13 反復を測った。**ゆえに 1 行パッチは当てておらず、戻し忘れの危険も生じていない**
- [x] **U1 を裁定する** — **純平行移動で正しい**。高さは 64 物理 px のまま不変（1130−1066 = 1106−1042 = 64）。#1173 が測っていなかった前提を確認した
- [x] **Q2 に答える** — **クランプでも製品コードでもない（＝ OS 側）**。クランプが観測した `pre` は 1042 で、移動前の 1066 を見たサンプルが 1 つも無い。加えてその区間のフレームが 0 枚
- [x] **Q3 に答える** — **この条件では「競り合い」は成立しない**（モーダルループ中のフレームが 0 枚）。ただし #1173 の「出入りを 2 度繰り返す」条件を再現できておらず、**あちらの逸脱の機序を棄却したことにはしない**
- [x] **U3 を測る** — 確定後にフレームは来る（クランプが 1 度だけ発火する）。**副産物として、armed 期限の `request_repaint_after` の鎖はモーダルループを越えないことも実測**（toast 条件でもループ中 0 枚）——U3 の対処候補 (a) はこれで潰れた
- [x] 生ログは `C:/tmp/snotra-1194/` に留め、リポジトリへは派生表（`workspace/measurement-1194.md`）だけを入れた
- [x] 測定条件（機体 GPDWINMINI・dpi 120 ＝ 125%・seed した `config.toml` の内容・ビルドの mtime）を派生表に併記した

### Phase 2 — Q1 の取り下げと、実測への同期【人間裁定 2026-08-27 で方針変更】

**当初計画（ガードを `GUI_INMOVESIZE` へ差し替える）は取り下げた。** Phase 1 の測定で拘束が
13 反復 3 条件すべてで再現せず、差し替えても挙動が 1 ビットも変わらないため（クランプが
モーダルループ中に走らない）。裁定の逐語は「Q1 を取り下げて SPEC と rustdoc を実測へ直して」。

**`SPEC.md` を先に直す**（`AGENTS.md`「開発ワークフロー」1: 仕様変更は `SPEC.md` → コード → ドキュメントの順）。

- [x] `SPEC.md` §8.2 `:478` を実測へ直す——「キーボード移動は拘束される」を「移動中は拘束せず、確定後の最初のフレームで 1 度だけ戻る」へ。**`:476` / `:477` / `:481` は変えない**（ポインタ主体の記述は今も真である＝ガードを変えないため）
- [x] **全称にしない**——「フレームが回らない」は機構の保証ではなくイベント駆動の帰結であり、ループ中にフレームが走る条件が生じれば移動中も引き戻されうる。#1173 が逆を記録していることと、条件差が未特定であることを添える
- [x] `main_in_modal_move_loop` の呼び出しを **`trace_enabled()` の内側へ移す**——観測専用になった以上、trace を切った実運用で毎フレーム Win32 を 1 回払う理由が無い
- [x] `view.rs:1279` のガードは**変えない**（`!any_down()` のまま）
