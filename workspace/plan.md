...
- [x] **受け入れ条件 3（下端に収まらないとき）は人間の目視に残る** — 窓をドラッグして作業領域の最下端へ置く操作が要り、自動化していない。**下の「残タスク」で PR 本文へ振り分け済み**（項目 7 を本変更に合わせて書き換えてある）
...
- [x] **H2 — `visible_rows` を作業領域より大きくすると、下端の行がどこへ動かしても見えずスクロールでも到達できない。** 撤去したクランプは「窓が画面を超えない」という絶対上限も担っていた。**人間裁定（2026-08-04）: 問い「1 このまま受容 / 2 設定 UI の上限を実用域へ下げる / 3 設定画面で警告を出す」→ 回答「1 受容。ユーザーの作業領域は千差万別なため」。** 却下した案 2 とその理由（収まる値は解像度・DPI・タスクバー配置で変わり単一の定数にできない）は `ADR-results-fixed-height` の却下 6 へ、受容残余は `SPEC.md`「4.5 最大列挙数」へ記録済み。**issue は起票しない**——受容の決定であって未完の作業ではない
- [x] **受け入れ条件 3（下端に収まらないときの挙動）の目視** → **PR 本文のチェックリストへ送った**（下記）。窓のドラッグと人間の判定が要り自動化していない

### PR 本文へ入れる項目

**チェックボックスの記法はここに書けない**——`pre-bash.mjs` の `countUnchecked` は行頭の `-`/`*` + `[ ]` を数え、**コードブロック内も区別しない**（同関数の doc が「過剰検出として受容する」と明記）。ここへ書くと `gh pr create` が自分自身の草案でブロックされる。PR 本文を書くときにチェックボックスへ起こすこと。

> カテゴリ D の目視（下端）: `npm run smoke:manual -- -Only 7` — 件数でも表示位置でも results の高さが変わらないこと。下端で作業領域からはみ出すのは仕様（#835）
