...
## code-reviewer（Step 4b）の記録

**同じエージェントを 2 ラウンド**（2 巡目は新規起動でなく `SendMessage` で再開——自分が出した
指摘の解消を本人が検算でき、⚠️ として残した争点も追える）。

### ラウンド 1 — Critical 0 / High 1 / Medium 2 / Low 5 / ⚠️ 3

- **H1（採用・本質的）**: 契約 doc が「read guard を握ったまま `engine.lock()`」の害を**加算的な
  遅延**として書いていたが、実体は **ABBA デッドロック**である。`config_watcher` の
  `state.engine.lock().unwrap().update_config(..)` は `MutexGuard` が文全体で生きたまま内側で
  `config.write()` を取るので、製品に `engine → config-write` の順序が実在する。
  **機序を一次証拠で自分でも裁定してから採った。** 逆向きの監査が「持ち手の見つからない不変
  条件」として挙げた唯一の 1 件でもある。
- **M2（採用）**: 消えた文言を逐語で引用する doc が 2 件（`instant.rs` / `launcher_controller.rs`）。
  **どちらも本差分が触ったファイルで、その隣の行が直っていなかった**（#977 / #1056 の型）。
  新 ADR が数えた「散文 9 か所」の外側。
- M3 / L4 / L5 / L6 / L8 も採用。**L7 は採らず**（3 か所は読者と目的が違うと反論 → 2 巡目で
  レビュア自身が取り下げた）。
- 争点 A・B・C はいずれも著者の判断が支持された。**争点 A には著者が挙げていなかった 4 点目**が
  付いた——`launch_item_with_state` は `resolve_opener` の直後に `record_and_save`（engine 錠）を
  呼ぶので、**生 guard で書いていたらデッドロックの形がその関数の中に自然に生まれていた**。
  クロージャ形の選択は様式ではなく correctness の話だった。

### ラウンド 2（修正差分が対象）— Medium 1 / Low 3 / ⚠️ 3

- **R1（採用・私が修正で作った退行）**: H1 を直すとき、閉じた肯定形の規則（「`read` へ渡すのは
  純 CPU だけにする」）を**開いた禁止 2 件**（`engine.lock()` / I/O）へ置き換えてしまい、
  `src-tauri` に実在する engine 以外の `Mutex`（`IconCacheState` / `SettingsProcessState` /
  updater 等）を規則が禁じなくなっていた。**しかも委譲側の `mod.rs:419` が正本より広い契約を
  宣言したままだった**（触らなかったファイルが正しく、触ったファイルが狭まった）。
  → 規則を戻し、デッドロックを「とくに」の具体例として下に置く形へ。
- **R2 / R3 / R4（採用）**: M2 の修正が逐語でない引用を再生産（`grep -c` で 0 件と実測）／
  M3 の但し書きが写しへ伝播していない／括弧の連続。
- **⚠️ の裁定**: W1 は「名指しは正しいが規則を消したのは誤り」＝ R1 に含まれる。W2・W3 は
  著者が正しい（W3 は「現存しない危険を doc へ書くのは規範の希釈」と支持）。

### ラウンド 3 を行わない判断

memory `review-frameworks-not-rounds`: **自分の修正差分が対象だと同じ枠組みは収束しない**。
所見の在り処で判断する——ラウンド 1 は機構の誤り（H1）、ラウンド 2 は散文の言い回し
（R2 / R3 / R4）と 1 件の実質（R1）へ移った。R1・R2・R4 の解消は**主エージェントが自分の
道具で測った**（`grep -c` / 委譲側と正本の突き合わせ / `）（` の出現数）。
