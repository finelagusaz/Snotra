...
- **`SPEC.md` は更新しない。** 判定は AGENTS.md の 2 参照で行う: (a) `SPEC.md` §4.2.1
  ...
  - **残余（本変更以前からの欠落）**: §4.2.1 の「以下の全条件を満たすこと」という列挙に
    `has_path_sep` と `!prev_candidates.is_empty()` は入っていない。**この差分が入れた退行ではない**が、
    本変更で前者が read/write の要石になったので、SPEC へ足すかは別途の判断とする
