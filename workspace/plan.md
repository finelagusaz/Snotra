...
| # | 項目 | 実施者 |
|---|---|---|
| 1 | 設定アプリを開き、チェックボックスが**未チェック**であること（この機体は現在未登録・research.md §2 で実測済み） | エージェント |
| 2 | チェックを入れる → `HKCU\...\Run` に `Snotra` が現れ、値が引用符付きの `snotra.exe` 絶対パスであること | エージェント |
| 3 | チェックを外す → 値が消えること | エージェント |
| 4 | 値が無い状態で外す操作をしてもエラーが出ないこと（レジストリから手で消してから設定アプリを開き直す） | エージェント |
| 5 | **サインアウト → サインインで実際に本体が起動すること**（この経路は誰もまだ測っていない） | **人間**——実行中のセッションを落とすため |
| 6 | タスクマネージャーの「スタートアップ アプリ」に `Snotra` が現れること、および**そこで無効化したときにチェックボックスがどう見えるか**（残余 1 の実測を兼ねる） | **人間**——UI 操作の自動化手段が無い |
