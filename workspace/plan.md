...
- [ ] 走査元から外す接頭辞（`docs/superpowers/` / `workspace/` / `docs/adr/`）を**述語 1 つへ括り出す**。両腕が同じ契約（履歴資料・作業バッファ・凍結された歴史は走査元から外す）を共有していることを構造で示す。`.rs` では現状 0 件マッチ＝ inert ゆえ、効いていることは fixture でしか固定できない（Phase 2 種 7）
- [ ] `headingRefSourceDocs(snapshot)` を追加（`.rs` かつ上記の接頭辞に当たらない）。doc コメントに以下を書く
...
- [ ] **種 7（不混入・双方向）**: `.ts` / `.mjs` / `.ps1` / `.toml` は `.rs` の腕に入らない。かつ `workspace/x.rs` / `docs/adr/x.rs` / `docs/superpowers/x.rs` は除外される（Phase 1 で括り出した接頭辞述語が `.rs` にも効くことの固定）
...
- [ ] (b) `monitor.rs:80` を書き換える。参照は `` `SPEC.md` §4.7「4.7 結果表示制御（2 窓構成）」``（**番号を `「」` の内側に含める**——`.rs` に既に 7 件・生きた `.md`（`docs/architecture.md:82` 他）にも在る優勢な形で、改番でも赤くなる）。**本文の言明はこのコメントの要点なので落とさず、正準形の外に残す**（`/plan-review` 独立導出 §3.2 の案 B。両検査で finding 0 を実測済み）
...
- 差異（採らなかった提案）: `headingRefDocs` を「2 母集団の和」にする案は採らず、`headingRefDocs` を md の腕のまま残して兄弟を足す。既存テスト :860 の主張が偽にならず、名前も嘘にならない
