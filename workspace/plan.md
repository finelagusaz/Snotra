...
- [ ] **B16** `docs/adr/ADR-stale-identifier-detector-scope.md` へ追記節を足す（既存の「その後（#735 完了後・射程を広げた）」と**同じ形**。**原文は 1 文字も書き換えない**）:
  ...
  - (f) **`docs/design/` を入れた理由**（U1 の決定）。**基準を「もう成り立たないことを書く場所か」として書き、ディレクトリの列挙として書かない**——日付スラグの有無で分けたと読まれると、次の読者が `docs/design/` を「歴史記録」として外しにいく。`docs/adr/` は却下案、`docs/superpowers/` は #589 で非規範化された当時の設計。`docs/design/…-coherence-staleset.md` は `status: Agreed` で `docs/architecture.md:99` が現在形で指す先ゆえ、**外すと G-references が守るポインタの指し先だけが黙って腐る**
