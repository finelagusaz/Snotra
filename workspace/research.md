...
**当初測れていなかったこと（2026-09-01 の目視 6 で決着）**: タスクマネージャーの「無効化」が `Run` 値を消すのか、`StartupApproved` 側にマークを置くだけなのか。調査時点では手元の 2 件がどちらも `02`（有効）で無効側の標本が無かった。

**実測結果**: **`Run` 値は消えない。** 無効化すると `StartupApproved\Run` に `Snotra` のレコードが作られ、先頭バイトが `03`（無効）になる（後続 8 バイトは無効化時刻に見える）。有効へ戻すと `02` + ゼロ埋めに戻り、これは調査時に観測した既存 2 件と同じ形である。**`Run` 値はどちらの操作でも変化しない。**

この実測により、§7 の未解決 1 と 4 は両方とも解消した。ただし**採る対処は変わらない**——追随させない理由が「形式を測れない」から「そこは利用者の領分である」へ変わった（`docs/adr/ADR-autostart-state-ownership.md` の 7 節が正本）。
