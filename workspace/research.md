...
**2026-08-14 追記（ユーザーからの一次情報）: Zed は測定窓の間ずっと起動していた**（測定後に終了）。
敵対枠が「測定窓の間 Zed が一度も RA を起動しなかったことは検証していない」と指摘した点は、
これで**推測ではなく既知の事実**になった。そして**結論は弱まらず、むしろ強くなる**。理由は 2 つ。

1. **推論の向きが安全である。** M1 の主張は「mtime が**動かなかった**」であり、別クライアントが
   増えても mtime は**動く方向にしか**効かない。第三者の書き込みは我々の RA の書き込みを隠せない。
2. **ずっと起動していたなら、それは A/B を通じた定数である。** M1b は同じ `target/` を共有させた
   2 条件の対照であり、**両方の腕に等しく効く定数は、腕の間の差を説明できない**。実際 B 条件
   （RA 既定）でだけ 2 回とも mtime が動き（`14:33:38` / `14:39:37`）、その直前後の A 条件では
   2 回とも動かなかった。断続的な第三者よりも、**常時起動している第三者のほうが交絡として弱い**。
