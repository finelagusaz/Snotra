...
| ~~`SPEC.md` は剪定を規定していない（§3.4 は遅延ロードと FIFO cap のみ）~~ | ~~`SPEC.md:96-97` を読んだ~~ | **偽。2026-08-09 に code-reviewer が訂正**（下記） |
...
### 訂正: `SPEC.md` は剪定を規定していた（＝これは仕様変更である）

**`SPEC.md:95` に「インデックス再構築時はキャッシュをクリア（次回検索時に再抽出）」が在る。**
`git show 50205b5` が、この SPEC 行と `indexing.rs` の drain 側 `clear()` を**同一コミットで**
入れており（コミットメッセージも「インデックス再構築時に show_icons 設定を反映し無効時は
キャッシュ破棄」）、指しているのは `invalidate_icon_cache` ではなく drain 側である。

**なぜ見落としたか**: §3.4 を `grep アイコン` で読んだため、**その語を含まない当の 1 行が
母集団から落ちた**。`AGENTS.md`「照合は SSOT に対して行う」の失敗形——節を読むと言いながら、
実際には語で絞った部分列を読んでいた。

**採否への影響は無い**（SPEC の記述は実装が全消し `clear()` から選択的剪定へ縮んだ時点で既に
ずれており、撤去はそのずれを最後まで進めただけである）。**手順への影響は在る**——仕様変更
として `SPEC.md` を同期する（`AGENTS.md`「『fix』でも文書化された挙動を変えたら仕様変更」）。
...
1. **実測: 測ったセッションでは剪定が 1 件も落としていない**（650 件のキャッシュに対し、索引に
   無いキーは PATH 併合を勘定すると 0）。撤去してもキャッシュの中身は変わらなかった
